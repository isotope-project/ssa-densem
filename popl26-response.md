...
The exponential modality in linear logic is a monoidal comonad, which essentially means that it functorially assigns a comonoid structure to every type !A, to model contraction (the comultiplication) and weakening (the counit).
...
* In Lean: 
  ...
  1. The type system of lambda-iter, its refinement theory, and the
     soundness of (directed) substitution are mechanized in Lean. 
  2. The categorical interpretation of lambda-iter, and the soundness
     of this interpretation (including both the soundness of substitution
     and the soudness of the refinement relation) are mechanized in Lean. 
  3. Some, but not all, of the concrete models are proved to be Elgot monads
     in Lean. [FIXME: Which ones?]

* On paper: 

  1. The completeness proof is proved on paper (we used string diagrams,
     which are painful to mechanize). 
  2. The equivalence of lambda-ssa and lambda-iter is proved on paper, 
     since it uses completeness. 

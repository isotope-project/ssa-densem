...
Verification of compiler optimizations typically follows one of two broad semantic paradigms:
operational and denotational. Operational semantics verifies transformations by constructing
...
specific model. However, many practical compiler optimizations, such as loop hoisting,
\texttt{mem2reg}, and sparse conditional constant propagation, seem not to depend upon the specific
details of the model, instead relying on high level information such as which operations are
pure/constant. In particular, we observe that we often only require information about:
\begin{enumerate}
  ...
  $\ms{store}\;a\;b ; \ms{get}\;a \tref \ms{store}\;a\;b ; b$, where $\tref$ indicates refinement.
  ...
  be duplicated ($\letexpr{x}{\ms{maybeloop}}{(x, x)} \teqv (\ms{maybeloop}, \ms{maybeloop})$), i.e.
  is \emph{relevant}, while an effect like nondeterminism can be discarded
  ($\letexpr{\cdot}{\ms{nondet}}{5} \teqv 5$), i.e. is \emph{affine}. Again, linearity is sometimes
  a refinement rather than an equation; for example, $(\ms{nondet}, \ms{nondet}) \tref
  \letexpr{x}{\ms{nondet}}{(x, x)}$, making nondeterminism \emph{fusable}, but not relevant.
\end{enumerate}

\TODO{the talk covers:}

\TODO{a calculus for reasoning about what equations can be derived from info 1 2 3}

\TODO{a sound and complete denotational semantics for this calculus}

\TODO{an equivalence between this calculus and (substructural) SSA, which we also drive-by-define}

\TODO{fun models which support fun optimizations, like weak memory and separation logic}

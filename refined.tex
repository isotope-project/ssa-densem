...
!_{\etoty{A^q}}$ and $\Delta_{A^q} = \Delta_{\etoty{A^q}}$, where the \emph{effective type} of an annotated type $A^q$, $\etoty{A^q}$, is $\mb{1}$ if $q = 0$, and $A$ otherwise. 
Likewise, we proceed to define the semantics of an annotated context $\dnt{\Gamma^{\mb{q}}} : |\mc{C}|$ as $\dnt{\etoty{\Gamma^{\mb{q}}}}$.
...
By soundness (since all members of an equivalence class with the same variable $x$ are equivalent) and semantic substitution (to allow us to rename variables, since a renaming has the identity as
...
\begin{gather*}
  ...
  \prftree[r]{\rle{cons}}
    {\lwk{\Gamma}{\ms{L}^{\mb{Q}}}{\ms{K}^{\mb{Q}}}}
    {\cwk{\Gamma^{\mb{q}'}}{\Gamma^{\mb{q}}}}
    {\lwk{\Gamma}{\ms{L}^{\mb{Q}}, \lhyp{\ell}{A}^{\mb{q}}_{\epsilon}}
    {\ms{K}^{\mb{Q}}, \lhyp{\ell}{A}^{\mb{q}'}_{\epsilon'}}} \qquad
  \prftree[r]{\rle{skip}}
    {\lwk{\Gamma}{\ms{L}^{\mb{Q}}}{\ms{K}^{\mb{Q}}}}
    {|\Gamma| = |\mb{q}|}
    {\lwk{\Gamma}{\ms{L}^{\mb{Q}}}{\ms{K}^{\mb{Q}}, \lhyp{\ell}{A}^{\mb{q}}_{\epsilon}}}
\end{gather*}
  ...
  \begin{gather*}
    ...
    \prftree[r]{\rle{where$_{\ms{nonrec}}$}}
      ...
      {\forall \lhyp{\ell_i}{A_i}^{\mb{q}_i} \in \ms{R}^{\mb{Q}'} .
        \haslb{\Gamma^{\mb{q}}, x_i : A_i}{\epsilon}{t_i}{\ms{L}^{\mb{Q}}}
      }
    ...
    {\einf{\epsilon}}
    {\forall \lhyp{\ell_i}{A_i}^{\mb{q}_i} \in \ms{R}^{\mb{Q}'} .
      \haslb{\Gamma^{\mb{q}}, x_i : A_i}{\epsilon}{t_i}{
        \ms{L}^{\mb{Q}}, \ms{R}^{\mb{Q}'}}
    }
...
\subiterexp{} model of our signature. 

\TODO{rework label context narrative to define and use ltoty}

We begin by giving a denotational semantics for label contexts and label
weakenings in Figure~\ref{fig:lwk-densem}, which are both w.r.t. a variable context $\Gamma$.
It is easy to verify that we can reassociate 
$\alpha^+: \mc{C}_\bot(\dnt{\ms{L}^{\zeroqv{\mb{Q}}}}(\Gamma, x : A), \dnt{\ms{L}^{\mb{Q}}}(\Gamma))
$ and $\alpha^+ : \mc{C}_\bot(
  \dnt{\ms{L}^{\mb{Q}}, \ms{R}^{\mb{Q}'}}(\Gamma),
  \dnt{\ms{L}^{\mb{Q}}}(\Gamma) + \dnt{\ms{R}^{\mb{Q}'}}(\Gamma)
)
$; we will use these freely in our semantics. 
...
\begin{itemize}
  ...
  $\ms{R}^{\mb{Q}'}$. The exit labels are then passed through as is, while the sublabels $\ell_i$ are passed to the denotation of the appropriate subregion $t_i$.
...
\begin{figure}
  \begin{gather*}
    \boxed{\dnt{\ms{L}^{\mb{Q}}}(\Gamma) : |\mc{C}|}  
    \\
    \dnt{\cdot}(\Gamma) = \mb{0} \qquad
    \dnt{\ms{L}^{\mb{Q}}, \ell(A)^{\mb{q}}_{\epsilon}}(\Gamma)
    = \dnt{\ms{L}^{\mb{Q}}}(\Gamma) + \dnt{\Gamma^{\mb{q}}} \otimes \dnt{A}
    \\
    ...
    \dnt
      {\lwk{\Gamma}{\ms{L}^{\mb{Q}}, \lhyp{\ell}{A}^{\mb{q}}_{\epsilon}}
      {\ms{K}^{\mb{Q}'}, \lhyp{\ell}{A}^{\mb{q}'}_{\epsilon'}}}
...
\begin{figure}
  \begin{gather*}
    ...
    : \mc{C}_\epsilon(\dnt{\Gamma^{\mb{q}}}, \dnt{\ms{L}^{\mb{Q}}}(\Gamma))} \\
    \dnt
    {\haslb{\Gamma^{\mb{q}}}{\epsilon}{\letstmt{x}{o}{t}}{\ms{L}^{\mb{Q}}}}
    =
    \dnt{\qsp{\Gamma}{\mb{q}}{\mb{q}_l}{\mb{q}_r}}
    ...
    \\ 
    \qquad ; [
      \ms{id}_{\dnt{\ms{L}^{\mb{Q}}}(\Gamma)},
      [
        \dnt{\hasty{\Gamma^{\mb{q}_i}, x_i : A_i}{\epsilon_i}{t_i}
          {\ms{L}^{\zeroqv{\mb{Q}}}}},
      ]_{\ell_i(A_i)^{\mb{q}_i}_{\epsilon_i} \in \ms{R}^{\mb{Q}'}}
    ]
    \\
    \dnt
    {\haslb{\Gamma^{\mb{q}}}{\epsilon}{\cwhere{\kappa}{(\wbranch{\ell_i}{x_i}{t_i},)_i}}
    ...
    \qquad ; [
      ...
      [
        \dnt{\hasty{\Gamma^{\mb{q}_i}, x_i : A_i}{\epsilon_i}{t_i}
          {\ms{L}^{\mb{Q}}, \ms{R}^{\mb{Q}'}}},
      ]_{\ell_i(A_i)^{\mb{q}_i}_{\epsilon_i} \in \ms{R}^{\mb{Q}'}}^\dagger
    ]
  \end{gather*}
...
\end{figure}
We will consider two \isotopessa{} programs equivalent under the relation $\mc{R}$...

\TODO{program equivalence}

We now wish to show that \isotopessa{} and \subiterexp{} are equivalent

\TODO{SSA to Expr, Expr to SSA, bijection up to equivalence}

; to show this, it suffices
to show that there are semantics-preserving translations between them. For the than explicitly
translate \isotopessa{} to \subiterexp{}, it suffices to note that the denotational semantics of
\isotopessa{} using the syntactic \subiterexp{}-model can be viewed as mapping \isotopessa{} regions
to (equivalence classes of) \subiterexp{} expressions satisfying
$$
\forall (x, a) \in \dnt{\haslb{\Gamma^{\mb{q}}}{\epsilon}{r}{\ms{L}^{\mb{Q}}}}_{\ms{Tm}(\mc{R})}, \qquad
  \hasty{x : \etoty{\Gamma^{\mb{q}}}}{\epsilon}{a}{\ltoty{\Gamma}{\ms{L}^{\mb{Q}}}}
$$
It hence suffices to show that
\begin{equation}
\forall (x, a) \in \dnt{\haslb{\Gamma^{\mb{q}}}{\epsilon}{r}{\ms{L}^{\mb{Q}}}}_{\ms{Tm}(\mc{R})},
  \dnt{\hasty{x : \etoty{\Gamma^{\mb{q}}}}{\epsilon}{a}{\ltoty{\Gamma}{\ms{L}^{\mb{Q}}}}}
  = \rho^{-1}_{\dnt{\Gamma^{\mb{q}}}} 
  ; \dnt{\haslb{\Gamma^{\mb{q}}}{\epsilon}{r}{\ms{L}^{\mb{Q}}}}
  \label{eqn:ssa-to-expr}
\end{equation}
For the other direction, we require a semantics-preserving function $\ms{SSA}$ compiling well-typed
\subiterexp{} terms to SSA (with a single output label corresponding to the output value), i.e., such that
\begin{equation}
  \dnt{\haslb{\Gamma^{\mb{q}}}{\epsilon}{\ms{SSA}(\hasty{\Gamma^{\mb{q}}}{\epsilon}{a}{A})}{\ell(A)^0}} = \dnt{\hasty{\Gamma^{\mb{q}}}{\epsilon}{a}{A}} ; \alpha^+
  \label{eqn:expr-to-ssa}
\end{equation}
We leave the definition of $\ms{SSA}$ and the proof of Equations~\ref{eqn:ssa-to-expr} and \ref{eqn:expr-to-ssa} to Appendix~\ref{apx:ssa-roundtrip}.

\TODO{\citet{bohm-jacopini} lore}
...
\TODO{load, store, alloc, free, haul legacy}

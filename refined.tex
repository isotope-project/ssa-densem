...
of let-bindings and our other language constructs. We don't provide rules for every construct; for
example, rules for pairs are notably omitted, since these can be derived from $\beta$-reduction.
...
\brle{let$_1$-$\beta^p$}), which are \emph{directed}. We begin by defining,
\begin{equation*}
  \tmlep{\Gamma^{\mb{q}}}{\mc{R}}{a}{b}{A}{+} \iff \tmeq{\Gamma^{\mb{q}}}{\mc{R}}{a}{b}{A} \qquad
  \tmlep{\Gamma^{\mb{q}}}{\mc{R}}{a}{b}{A}{-} \iff \tmeq{\Gamma^{\mb{q}}}{\mc{R}}{b}{a}{A}
\end{equation*}
In particular, \brle{let$_1$-$\beta^+$} says
...
\end{equation}
which of course obviously holds for pure expressions with $\epsilon = \bot$.

Finally, we give the rules for iteration in Figure~\ref{fig:iteration-rules}. These correspond
roughly to the properties of a Conway iteration operator; in particular, we require our iteration
operator to satisfy:
\begin{itemize}
  \item \emph{Fixpoint}: the rule \brle{iter-$\beta$} \todo{rest}
  \item \emph{Naturality}: \todo{this}
  \item \emph{Dinaturality}: \todo{this}
  \item \emph{Uniformity}: \todo{this}
\end{itemize}

...
\brle{let$_1$-$\beta^+$} and \brle{let$_1$-$\beta^-$}, which are \emph{directed}. In particular,
\brle{let$_1$-$\beta^+$} says that $\beta$-reducing a let-binding is a \emph{refinement} if the
bound value has an effect which is a right-mover w.r.t. the effect of the body and is used a number
of times compatible with both the effect and the context it is typed in. \brle{let$_1$-$\beta^-$},
on the other hand, says that factoring out a let-binding from a substitution is a refinement if the
bound value has an effect which is a \emph{left-mover} w.r.t the effect of the body. In particular, 
this means that the following more standard equation is \emph{derivable}:
\begin{equation}
...
Finally, we give the rules for iteration in Figure~\ref{fig:iteration-rules}.

\TODO{these correspond exactly to the axioms for a Conway iteration operator}
...
\begin{figure}
  \begin{gather*}
    ...
    \prftree[r]{\rle{let$_1$-$\beta^+$}}
      ...
      {q \leq \alquant(\Gamma^{\mb{q}_r}) \sqcap \alquant^p(\epsilon)}
      {
        \tmle{\Gamma^{\mb{q}}}{\mc{R}}{\letexpr{x}{a}{b}}{[x/a]b}{B}
      } \\
    \prftree[r]{\rle{let$_1$-$\beta^-$}}
      {\qsp{\Gamma}{\mb{q}}{\mb{q}_l}{\mb{q}_r}}
      {\hasty{\Gamma^{\mb{q}_l}}{\epsilon}{a}{A}}
      {\hasty{\Gamma^{\mb{q}_r}, x : A^q}{\eta}{b}{B}}
      {\epsilon \leftmove \eta}
      {q \leq \alquant(\Gamma^{\mb{q}_r}) \sqcap \alquant^p(\epsilon)}
      {
        \tmle{\Gamma^{\mb{q}}}{\mc{R}}{[x/a]b}{\letexpr{x}{a}{b}}{B}
      }
  \end{gather*}

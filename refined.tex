...
= \mb{0} + \dnt{\Gamma^0} \otimes \dnt{A}$. As this transformation is complicated to state and
verify for general expressions, we will instead define \emph{ANF}, and, after showing how every
expression can be compiled to an equivalent expression in ANF, define $\ms{SSA}_\ell$ by induction
...
\begin{itemize}
  \item (Valid operations $\hasty{\Gamma^{\mb{q}}}{\epsilon}{o}{A}$): \todo{don't stop}
  \item ($\hasty{\Gamma^{\mb{q}}}{\epsilon}{\letexpr{x}{o}{P}}{A}$): \todo{till you drop}
  \item ($\hasty{\Gamma^{\mb{q}}}{\epsilon}{\letexpr{(x, y)}{o}{P}}{A}$): \todo{POPL}
  \item ($\hasty{\Gamma^{\mb{q}}}{\epsilon}{\letexpr{x}{\caseexpr{o}{x}{P}{y}{Q}}{R}}{C}$): 
  \todo{till you}
  \item ($\hasty{\Gamma^{\mb{q}}}{\epsilon}{\letexpr{x}{\liter{o}{x}{P}}{Q}}{B}$): \todo{DROPL}
\end{itemize}
...
\TODO{introduction, working over $\mb{Set}$}
...
\TODO{in particular, \emph{every} monad is poset-enriched with the identity order on $T\;A$}

\TODO{simplest poset-enriched monad is $\top$-monad}

\TODO{a good example of a nontrivial poset-enriched monad is nondeterminism $\mc{P}$, $\mc{P}^+$}

\TODO{every poset-enriched monad yields a distributive effectful category}
 
\TODO{Elgot monad; poset-enrichment is the obvious thing}

\TODO{simplest Elgot monad is nontermination}

\TODO{nondeterminism is a poset-enriched Elgot monad}

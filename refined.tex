...
Premonoidal categories were introduced in \citet{power-premonoidal-97} to model side-effectful
computations with first-order binding. A premonoidal category can be viewed as a generalization of a
monoidal category, in which \emph{sliding} does not necessarily hold; i.e., we have that, in
general,
$$
...
\TODO{for branching with coproducts, we can represent case, $1 + 1$ for if; but we want our coproducts to be
distributive, which allows us to interpret the rule for cases where variables can be used in either
branch}
...
\TODO{for iteration, introduce Conway operator; point back to refinement rules}
...
\TODO{give higher-order presentation (i.e. ML or Haskell code), rules are obvious bro trust me}

\TODO{note that a refinement rule for dinaturality is missing; that is because it is derivable given
uniformity}

\TODO{we need it to be \emph{strong} again for variable scoping purposes; look at the ML}

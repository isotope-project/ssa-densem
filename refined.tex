...
The last thing that remains is to treat iteration. Our rules for iteration are based on the
properties of a Conway iteration operator as given in
\citet{coinductive-resumption-levy-goncharov-19}. In particular, we require our operator to satisfy
...
this justifies flattening nested loops into one with a more complex continuation, and algebraically
it reflects that iteration is a monoid in the Kleisli category.
%
\paragraph{Uniformity}, via the rule \textsf{unif$^p$}, allows us to effectively commute effectful
operations with a loop body; in particular, if a command $s$ commutes with the loop body $b$, then

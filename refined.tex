...
Static single assignment form, or SSA, rapidly became the dominant compiler intermediate representation (IR) after its introduction by \citet{alpern} and \citet{rosen} in the 1980s. The fundamental idea behind SSA is one which is very familiar to functional programmers: if a variable is defined exactly once and never reassigned, then substitution is then a valid program transformation. This both simplifies the implementation and improves the performance of many compiler optimizations.

The correctness of SSA transformations has mostly been handled informally, because it was originally intended to be a simple, first-order imperative programming language. Unfortunately, in the decades since its introduction, computers have become much less simple, and the optimizations that compiler writers want to do have become much more ambitious. Modern hardware is highly concurrent, and exhibits user-visible non-sequentially-consistent behaviour:  \emph{weak memory}~\cite{batty}. This means memory can no longer be correctly modelled as a global array of bytes. Furthermore, compilers like LLVM and GCC now make very aggressive optimizations exploiting knowledge of memory aliasing, undefined behaviour, and nondeterminism. Because all these optimizations are all done in the presence of nondeterminism, the transformations compilers want to do are not usually progarm equivalences, but merely refinements: the possible behaviours of the optimized program must be a subset of the possible behaviours of the original program. 
...
\begin{definition}[Effectful category]
  ...
  category.} (symmetric premonoidal) subcategories $\mc{C}_\epsilon \subseteq \mc{E}$ such that,
  given $\epsilon, \eta \in \mc{E}$, $f \in \mc{C}_\epsilon(A, B)$, and $g \in \mc{C}_\eta(A', B')$, 
  \begin{itemize}
    \item $\epsilon \rightmove \eta \implies f \ltimes g \cref f \rtimes g$
    \item $\epsilon \leftmove \eta \implies f \ltimes g \anticref f \rtimes g$
  \end{itemize}
  \footnote{
...
on the set of live variables $\Gamma$. To extend the set of live variables, we need to zero-pad
the quantity matrix, an operation which we call \emph{lifting}, and define as follows:
\begin{equation*}
...
using \brle{cons}. It is easy to prove that label weakening is compatible with lifting, and in
particular, that the following rule is derivable:
\begin{equation*}
  \prftree[r]{\rle{lift}}
...
In all the models in this section, every type has unrestricted linearity; see Appendix~\ref{apx:sep}
for a model with linear types.
...
$\{\infty, \ubeff, ?, \ms{r}, \ms{w}\}$
generated by inclusions:
\begin{itemize}
  \item $\ms{Heap}_\bot(A, B) \approx A \to B$
  \item $\ms{Heap}_\infty(A, B) \approx \ms{UB}_\infty(A, B)$ and 
        $\ms{Heap}_\ubeff(A, B) \approx \ms{UB}_\ubeff(A, B)$
\end{itemize}
and primitive effectful instructions
\begin{itemize}
  \item $\ms{load} : \ms{Heap}_{\ms{r}\ubeff}(\nats, \nats) := 
    \lambda p\; s.
      \ms{if}\;p \in s\;\ms{then}\;\ms{pure}_{\ms{ND}}(s\;p, s)\;\ms{else}\;\ubeff
    $
  \item $\ms{store} : \ms{Heap}_{\ms{w}\ubeff}(\nats \otimes \nats, \mb{1}) :=
    \lambda (p, x)\; s.
      \ms{if}\;p \in s\;\ms{then}\;\ms{pure}_{\ms{ND}}((), [p \mapsto x]s)\;\ms{else}\;\ubeff
    $
  \item $\ms{alloc} : \ms{Heap}_{\ms{w}?}(\nats, \nats) :=
    \lambda x\; s. \{(p, [p \mapsto x]s) \mid p \notin s\}
    $
  \item $\ms{free} : \ms{Heap}_{\ms{w}\ubeff}(\nats, ()) :=
...
\TODO{separation logic pointer}

\subsection{Release-Acquire Weak Memory}

\TODO{this}

\TODO{a trace is one possible execution sequence; events in the trace are what can influence other threads}

\TODO{our traces are reads and writes quotiented by a relation $R$ (with example, e.g. mumble), for relation + bind + Elgot see appendix}

\TODO{all the heap peepholes still work}

\TODO{conjecture mem2reg}

...
\newcommand{\loopstmt}[3]{\ms{loop}\;#1(#2) \lto \;\{#3\}}
...
Computer programs are written for two audiences: the \emph{computer}, which executes them, and the
\emph{programmer}, who writes, interfaces with, modifies, and maintains them. The needs of these two
audiences are often in tension, hence, the need for \emph{compilers}, which translate programs from
high-level, human-readable languages to low-level languages amenable to execution. A single
high-level program, however, can compile to a vast variety of low-level programs with the same
\emph{semantics} but vastly different \emph{performance characteristics}. The job of an
\emph{optimizing compiler} is to generate a ``\emph{good}'' low-level program from a high-level
source file, where ``good'' most commonly refers to fast execution time, but often includes low
binary size and memory usage as well, especially when targeting embedded systems and web platforms. 

The issue is that neither high-level source code nor low-level machine code is particularly amenable
to machine analysis and rewriting: high-level source code is too varied, while low-level
instructions are often complex and highly side-effectful, with both often having extremely
context-dependent semantics for the same operations. Hence, compilers typically transform programs
through a series of \emph{intermediate representations} specifically designed to be amenable to
automated reasoning.

The most basic form of intermediate representation, variants of which are still commonly used as a
high-level intermediate representation at the start of modern compiler pipelines (e.g. Rust's MIR),
is \emph{register-transfer level (RTL)} code, often known as \emph{3-address code}. RTL programs are
represented as a \emph{control-flow graph} of \emph{basic blocks}, which are sequences of simple
instructions having only straight-line control flow followed by a single \emph{terminator} which
can jump to other basic blocks in the graph.
...
Since variables may be redefined in RTL code, algebraic reasoning about programs can be quite
difficult, especially in the presence of complex control flow. SSA fixes this by borrowing an idea
from functional programming: it enforces the property that every variable has exactly one
definition. To deal with the case where a variable can have a different value depending on
control-flow, we extend basic blocks with \emph{arguments} (or equivalently, $\phi$-nodes). Since
every variable has exactly one definition, we can now, regardless of control-flow, conveniently
perform algebraic-reasoning based on a variable's value.
...
One way of viewing this is to note that, since every variable has exactly one definition, SSA makes
every usage of a pure variable \emph{referentially transparent}, in that we can simply replace it
with its definition to do algebra. \citet{ghalayini-24-ssa-densem-arxiv} use this property to
develop a sound and complete equational theory for SSA, based on the insight that substitution of
pure expressions is therefore unconditionally valid, i.e., the following programs are equivalent:
\begin{equation*}
  a\text{ pure} \implies \letexpr{x}{a}{b} \teqv [a/x]b
\end{equation*}
By combining this with a theory of control-flow, and treating impure expressions opaquely, they
manage to justify a variety of complex optimizations in models with complex side-effects such as
weak memory accesses.

Unfortunately, not all the algebraic reasoning optimizing compilers want to do is equational
reasoning about pure expressions. For example, consider an operation like \ms{div}, which exhibits
\emph{undefined behavior} if we divide by zero; such an operation has a potential side-effect, and
so is not pure, but we still would like to be able to do algebra on expressions containing it. Let's
say we have a program like $\letexpr{x}{\ms{div}\;y\;z}{e}$, where the optimizer cannot deduce
whether $z$ is nonzero.  In general, this program is \emph{not} equivalent to $[\ms{div}\;y\;z/x]e$,
since, for example, if we choose $e = ()$ and $z = 0$, the former has UB whereas the latter does
nothing. However, it turns out it is always safe to rewrite $\letexpr{x}{\ms{div}\;y\;z}{e}$ to
...
We say such a refinement is \emph{strict}.
...
which is fusable, but not duplicable, is nondeterminism: the refinement $(\ms{nondet}, \ms{nondet})
\tref \letexpr{x}{\ms{nondet}}{(x, x)}$ is not an equation since the right-hand side will only
evaluate to pairs $(x, x)$ with equal components, whereas the right-hand side can evaluate to
arbitrary pairs $(x, y)$.
...
\TODO{can't lattice-ify substitution due to $\bot$-parameter}
...
\TODO{note that the above does \emph{not} compose, but this is fine...}
...
\begin{definition}[Effectful Elgot category]
  \;\TODO{this}
\end{definition}

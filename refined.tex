...
\begin{itemize}
  ...
  \end{itemize}
  \item On the other hand, it is obviously not safe to optimize $[\ms{div}\;y\;z/x]e$ to
  $\letexpr{x}{\ms{div}\;y\;z}{e}$ in the general case, since, for example, if $e$ does not contain
  $x$, then we might \emph{introduce} UB whereas previously there was none!
  \item This is an example of a \emph{refinement}, rather than an \emph{equation}: we have
  $\letexpr{x}{\ms{div}\;y\;z}{e} \tref [\ms{div}\;y\;z/x]e$, meaning that the left-hand side
  has fewer equations than the right-hand side.
  \item 
\end{itemize}

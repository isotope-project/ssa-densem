...
\begin{itemize}
  ...
  = f^\dagger ; g
  \end{equation*}
  \item \emph{Codiagonal}: given $f: A \to (B + A) + A$, define $g = f ; [\ms{id}_(B + A),
  \iota_r]$ and $h = f^\dagger$. We have
  \begin{align*}
    h_{i + 1}
    & = f^\dagger ; [\ms{id}, h_i]
  ...
  = h_{i + 1}
  \end{align*}
  We therefore have that $g^\dagger \subseteq h^\dagger$. On the other hand, 
  \TODO{direction}

...
the current state of memory. A program's denotation is then given by a \emph{set} of pairs
$\tret{\xi}{r}$ of a trace $\xi$ and return value $r$. We interpret such traces as
...
then returns control to the program with the system in state $\mu_2$, and so on.

\TODO{discuss the semantics of a read or a write?}
...
requiring our set of traces to satisfy certain \emph{closure properties}. In particular...

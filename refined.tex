...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \Gamma^{\mb{q}}, y : A^q & \vdash_\bot
      ...
      &\approx ((), y)
    \end{align*}
    \item ($\cwk{\Gamma^{\mb{q}}, x : B^q}{y : A^1}$): we have that
    \begin{align*}
    \dnt{\cwk{\Gamma^{\mb{q}}, x : B^q}{y : A^1}}
    ...
    It follows that
    \begin{align*}
    \Gamma^{\mb{q}}, x : B^q & \vdash_\bot
    \letexpr{x}{\ms{pack}(\Gamma^{\mb{q}}, x)}  
      {\dnt{\cwk{\Gamma^{\mb{q}}, y : A^q}{y : A^1}}(x)} \\
    ...
    \end{align*}
    \TODO{for lo, the enemies of mankind are many, but the greatest of these is the left strike --
    Github Copilot}
  \end{itemize}

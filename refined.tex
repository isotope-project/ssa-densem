...
$$
\boxed{\haslb{\Gamma^{\mb{q}}}{\epsilon}{r}{\ms{L}^{\mb{Q}}_{\mb{e}}}}
$$
which means that
\begin{itemize}
...
Figure~\ref{fig:label-grammar}. We call $\mb{Q}$ the \emph{quantity matrix}, and $\mb{e}$ the
\emph{effect vector}.
...
\begin{itemize}
  ...
  expression of type $A$, and $\ell(A)^{\mb{q}_l}_\epsilon$ weakens to the label context 
  $\ms{L}^{\mb{Q}}_{\mb{e}}$, where $\mb{q}_l$ is the quantities left over after typing $o$, then
  $\brb{\ell}{o}$ is a valid branch into $\ms{L}^{\mb{Q}}_{\mb{e}}$
  ...
  the compound label-context $\ms{L}^{\mb{Q}}_{\mb{e}}, \ms{R}^{\mb{Q}'}_{\mb{e}'}$, and,
  for each label in $\ell_i(A_i)^{\mb{q_i}}_{\epsilon_i}$ in $\ms{R}^{\mb{Q}'}_{\mb{e}'}$, an 
  associated region $t_i$ which, with variables $\Gamma^{\mb{q}_i}_{\mb{e}}, x_i : A_i$ live
  on entry, targets the exit labels $\ms{L}^{\mb{Q}}_{\mb{e}}$ with effect $\epsilon_i$. 
  This is non-recursive since the $\ms{R}^{\mb{Q}'}_{\mb{e}'}$ cannot call themselves, but we can 
  encode any DAG since the inner subtree $\kappa$ can contain other basic blocks.

  \brle{where$_\ms{rec}$} simply requires the effect $\epsilon$ to be iterative, and in exchange 
  allows the $t_i$ to make recursive calls to $\ms{R}^{\mb{Q}'}_{\mb{e}'}$ as well.
\end{itemize}
...
\begin{figure}
  \begin{gather*}
    \prftree[r]{\rle{br}}
      {\qsp{\Gamma}{\mb{q}}{\mb{q}_l}{\mb{q}_r}}
      {\hasty{\Gamma^{\mb{q}_r}_{\mb{e}}}{\bot}{o}{A}}
      {\lwk{\Gamma}{\lhyp{\ell}{A}_\epsilon^{\mb{q}_l}}{\ms{L}^{\mb{Q}}_{\mb{e}}}}
      {\haslb{\Gamma^{\mb{q}}_{\mb{e}}}{\epsilon}{\brb{\ell}{o}}{\ms{L}^{\mb{Q}}_{\mb{e}}}}
    \\

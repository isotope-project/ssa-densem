...
refinement. To do so, we note that $\dnt{\hasty{\Gamma^{\mb{q}}}{\epsilon}{a}{B}}_{\ms{Tm}(\mc{R})}$
is, up to equivalence, a term of the form $\hasty{x : [\Gamma^{\mb{q}}]}{\epsilon}{\bar{a}}{B}$.
Hence, our strategy for proving completeness is simply to relate the terms $a$ and $\bar{a}$ such
that $\bar{a} \tref \bar{b} \implies a \tref b$. We begin by relating the contexts $\Gamma^{\mb{q}}$
and $x : [\Gamma^{\mb{q}}]$ via the \emph{packing lemma}

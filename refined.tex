...
% \leq \dnt{\haslb{\Gamma^{\mb{q}}}{\epsilon}{r'}{\ms{L}^{\mb{Q}}}}_{\mc{M}}
...
% The astute reader will note that these functions are not \emph{quite} mutually inverse, since the 
% output of a round-trip  $\ms{SSA}(\ms{Expr}(\haslb{\Gamma^{\mb{q}}}{\epsilon}{r}{\ms{L}^{\mb{Q}}}))$ 
% now targets a \emph{single} label.
% TODO: expr + case is mutually inverse
% TODO: 
...
hand, the function $\ms{SSA}$ is essentially just a standard inductive ``compiler pass;''
we leave it's definition and verification to Appendix~\ref{apx:ssa-roundtrip}; we note in passing
that the result of this function will always use only structured control-flow (since \subiterexp{}
only supports structured control flow). 

% Hence, the round-trip 
% $\ms{SSA}(\ms{Expr}(\haslb{\Gamma^{\mb{q}}}{\epsilon}{r}{\ms{L}^{\mb{Q}}}))$ can be viewed as a
% somewhat crude implementation of the transformation to structured control-flow described in
% \citet{bohm-jacopini}, except that the output labels are changed to a single coproduct. Recovering
% the original output labels, and hence implementing the transformation on SSA in full, should not

...
$\mc{R}$.'' Equivalence of terms is derived from this refinement relation in the usual manner;
namely, by defining
\begin{equation}
...
\end{equation}
For brevity when introducing pairs of rules with opposite polarity, we also introduce
...
Here $\mc{R}$ is a set of rewrites of the form $\tmle{\Gamma^{\mb{q}}}{}{a}{b}{A}$; we
...
\end{equation*}
Our goal is to construct a refinement relation $\tref$ at every context and type
...
\begin{enumerate}
  ...
  \item $\tref$ abstracts away the associativity of let-bindings.
  \label{item:abstracts-syntax}
  \item $\tref$ contains the $\beta$ and $\eta$-rules of our language constructs
  \label{item:does-computation}
  \item $\tref$ encodes the properties of iteration
  \label{item:does-iteration}
\end{enumerate}
The rules in Figure~\ref{fig:congruence-refinement} guarantee the first two properties. In
particular, property~\ref{item:is-congruence}, also means that equivalence of terms $\approx$ is a
congruence.

To satisfy property \ref{item:abstracts-syntax}, we introduce a set of \emph{binding rules} in
Figure~\ref{fig:binding-rules}. The conclusions of these rules are \emph{equivalences} of the form
$\tmeq{\Gamma^{\mb{q}}}{\mc{R}}{a}{b}{A}$, rather than refinements; we could also express this by
providing one rule for each direction of the equivalence. We don't need to provide rules for every
construct; for example, binding rules for pairs and injections are notably omitted, since these can
be derived from $\beta$-reduction.

We proceed to give the $\beta$ and $\eta$ rules for our calculus in
Figure~\ref{fig:reduction-rules}. These are all standard equations, except for
\brle{let$_1$-$\beta^+$} and \brle{let$_1$-$\beta^-$} (which we write as a single rule
\brle{let$_1$-$\beta^p$}), which are \emph{directed}. In particular, \brle{let$_1$-$\beta^+$} says
that $\beta$-reducing a let-binding is a \emph{refinement} if the bound value has an effect which is
a right-mover w.r.t. the effect of the body and is used a number of times compatible with both the
effect and the context it is typed in. \brle{let$_1$-$\beta^-$}, on the other hand, says that
factoring out a let-binding from a substitution is a refinement if the bound value has an effect
which is a \emph{left-mover} w.r.t the effect of the body. In particular, this means that the
following more standard equation is \emph{derivable}:
\begin{equation}
...
\end{equation}
which of course obviously holds for pure expressions with $\epsilon = \bot$, as we would normally
expect in an effectful language.

...
To prove completeness, it suffices need to show that $\dnt{\cdot}_{\ms{Tm}(\mc{R})}$ \emph{reflects}
refinement. To do so, we begin by noting that by soundness (since all members of an equivalence
class with the same variable $x$ are equivalent) and semantic substitution (to allow us to rename
variables, since a renaming has the identity as denotation), for any $f :
\ms{Tm}_\epsilon(\mc{R})(A, B)$, we have that, for any model $\mc{M} \models \mc{R}$,
$$
\forall (x, a), (y, b) \in f, \dnt{\hasty{x : A^\top}{\epsilon}{a}{B}}_{\mc{M}} 
  = \dnt{\hasty{x : A^\top}{\epsilon}{b}{B}}_{\mc{M}}
$$
It follows that we can lift $\dnt{\cdot}_{\mc{M}}$ to $\ms{Tm}_\epsilon(\mc{R})$ by defining, for
arbitrary $(x, a)$ in the equivalence class,
$
\dnt{f}_{\mc{M}} = \dnt{\hasty{x : A^\top}{\epsilon}{a}{B}}_{\mc{M}} 
  : \mc{C}_\epsilon(\dnt{A}, \dnt{B})
$. 
In particular, we can hence state the \emph{flattening lemma}:
\begin{lemma}[name=Flattening, restate=flatteninglemma]
We have that
$
  \dnt{\dnt{\hasty{\Gamma}{}{a}{A}}_{\ms{Tm}(\mc{R})}}_{\mc{M}} 
  = \lambda ; \dnt{\hasty{\Gamma}{}{a}{A}}_{\mc{M}}
$
\label{lem:flattening}
\end{lemma}
\TODO{segue to packing}
\begin{lemma}[name=Packing, restate=packinglemma]
  ...
  $\hasty{\Gamma^{\mb{q}}, \Delta^{\mb{q}'}}{\epsilon}{b}{A}$ such that
  \begin{itemize}
...
\begin{equation}
  ...
  \label{eqn:unpacking}
\end{equation}
Details of the definition of $\ms{pack}$ and $\ms{unpack}$ and the proofs of
Lemmas~\ref{lem:flattening} and \ref{lem:packing} are given in Appendix~\ref{apx:packing}.

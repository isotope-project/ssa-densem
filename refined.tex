...
\begin{definition}[Poset-Enriched Monad]
  ...
  partial order compatible with $\mbind{T}{\cdot}{\cdot}$, i.e., such that, for $a \leq b$ and $f
  \leq g$, we have $\mbind{T}{a}{f} \leq \mbind{T}{b}{g}$, where functions $f, g : A \to T(B)$ are
  given the pointwise partial order.
\end{definition}
...
\begin{definition}[Poset-Enriched Elgot Monad]
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item $h^\uparrow\;f \leq g ; B + h^\uparrow \implies h^\uparrow ; f^\dagger \leq g^\dagger$
      \item$h^\uparrow\;f \geq g ; B + h^\uparrow \implies h^\uparrow ; f^\dagger \geq g^\dagger$
    \end{itemize}
...
the state transformer to the UB monad from Appendix~\ref{ssec:ub}, with state $S := \nats
\pto_{\ms{fin}} \nats$.

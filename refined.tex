...
In the previous section, the most natural way to define an effect system was as the lattice 
generated by a set of primitive effects: morphisms which \emph{can} loop forever ($\infty$) or 
\emph{can} exhibit UB ($\ubeff$). Interestingly, we can take the opposite perspective, and think
of the \emph{negation} of effects: note that $\infty$ is defined precisely as the morphisms which
\emph{cannot} exhibit UB ($\lnot\ubeff$) and $\ubeff$ as the morphisms which \emph{cannot}

...
$$
Here, the associator $\alpha$ takes $\dnt{A}$ to $\dnt{\Gamma^0} \otimes \dnt{A}$ (the latter being
a tensor product of monoidal units), which $\iota_r$ then takes to $\dnt{\ltoty{\Gamma}{\ell(A)^0}}
= \mb{0} + \dnt{\Gamma^0} \otimes \dnt{A}$.

We define $\ms{SSA}_\ell$ by induction on derivations $\hasty{\Gamma^{\mb{q}}}{\epsilon}{a}{A}$,
proving each branch correct as we define it:
\begin{itemize}
  \item ($\hasty{\Gamma^{\mb{q}}}{\epsilon}{o}{A}$, for a valid operation $o$): we define
  \begin{equation*}
    \ms{SSA}_\ell(\hasty{\Gamma^{\mb{q}}}{\epsilon}{o}{A}) 
      := \haslb{\Gamma^{\mb{q}}}{\epsilon}{\brb{\ell}{o}}{\ell(A)^0}
  \end{equation*}
  Note that this case includes variables $x$, as well as, e.g., pairs of variables $(x, y)$. This
  case satisfies the desired property, since
  \begin{align*}
    \dnt{\haslb{\Gamma^{\mb{q}}}{\epsilon}{\brb{\ell}{o}}{\ell(A)^0}}
    &= 
    \dnt{\qsp{\Gamma}{\mb{q}}{0}{\mb{q}}} 
    ; \dnt{\Gamma^{0}} \otimes \dnt{\hasty{\Gamma^{\mb{q}}}{\epsilon}{o}{A}}
    ; \iota_r
    ; \cancel{\dnt{\lwk{\Gamma}{\ell(A)^0}{\ell(A)^0}}} \\
    &=
  ...
  \end{align*}
  as desired.
  \item ($\hasty{\Gamma^{\mb{q}}}{\epsilon}{f\;a}{B}$): we define
  \begin{equation*}
    \ms{SSA}_\ell(\hasty{\Gamma^{\mb{q}}}{\epsilon}{f\;a}{B}) 
      := \haslb{\Gamma^{\mb{q}}}{\epsilon}{
          \letstmt{x}{a}{\ms{SSA}_\ell(\hasty{\Gamma^0, x : A}{\epsilon}{f\;x}{B}})}{\ell^0(B)}
  \end{equation*}
  We verify by induction that this satisfies the desired property:
  \begin{align*}
  & \dnt{\haslb{\Gamma^{\mb{q}}}{\epsilon}{
          \letstmt{x}{a}{\ms{SSA}_\ell(\hasty{\Gamma^0, x : A}{\epsilon}{f\;x}{B}})}{\ell^0(B)}} \\
  &= \dnt{\qsp{\Gamma}{\mb{q}}{0}{\mb{q}}} 
    ; \dnt{\Gamma^0} \otimes \dnt{\hasty{\Gamma^{\mb{q}}}{\epsilon}{a}{A}}
    ; \dnt{\haslb{\Gamma^{\mb{q}}}{\epsilon}
        {\ms{SSA}_\ell(\hasty{\Gamma^0, x : A}{\epsilon}{f\;x}{B})}{\ell^0(B)}} \\
  &= \dnt{\hasty{\Gamma^{\mb{q}}}{\epsilon}{a}{A}} 
  ; \alpha
  ; \dnt{\hasty{\Gamma^0, x : A}{\epsilon}{f\;x}{B}}
  ; \alpha
  ; \iota_r \\
  &= \dnt{\hasty{\Gamma^{\mb{q}}}{\epsilon}{a}{A}} 
  ; \alpha
  ; \dnt{\hasty{\Gamma^0, x : A}{\epsilon}{x}{A}}
  ; \dnt{f : A \to_\epsilon B}
  ; \alpha
  ; \iota_r \\
  &= \dnt{\hasty{\Gamma^{\mb{q}}}{\epsilon}{a}{A}} ; \alpha ; \iota_r
  \end{align*}
  as desired.
  \item ($\hasty{\Gamma^{\mb{q}}}{\epsilon}{\letexpr{x}{a}{b}}{A}$): \todo{tock}
  \item ($\hasty{\Gamma^{\mb{q}}}{\epsilon}{()}{\mb{1}}$): \todo{on the}
  \item ($\hasty{\Gamma^{\mb{q}}}{\epsilon}{(a, b)}{A \otimes B}$): \todo{on the}
  \item ($\hasty{\Gamma^{\mb{q}}}{\epsilon}{\letexpr{(x, y)}{a}{c}}{C}$): \todo{stutter}
  \item ($\hasty{\Gamma^{\mb{q}}}{\epsilon}{\iota_l(a)}{A + B}$): \todo{mumble}
  \item ($\hasty{\Gamma^{\mb{q}}}{\epsilon}{\iota_r(b)}{A + B}$): \todo{mumble}
  \item ($\hasty{\Gamma^{\mb{q}}}{\epsilon}{\caseexpr{e}{x}{a}{y}{b}}{C}$): \todo{but don't}
  \item ($\hasty{\Gamma^{\mb{q}}}{\epsilon}{\labort(a)}{A}$): \todo{but don't}
  \item ($\hasty{\Gamma^{\mb{q}}}{\epsilon}{\liter{a}{x}{b}}{B}$): \todo{metastutter}
\end{itemize}

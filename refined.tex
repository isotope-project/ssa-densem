...
build up our SSA formalism.
\subiterssa{} is, at first glance, a standard first-order language with branching and iteration.
% segue
We equip \subiterssa{} with a type-and-effect system, with every expression having as \emph{effect}
an element of a bounded meet-semilattice $\mc{E}$. We will represent pure expressions as having the
bottom effect $\bot$, whereas expressions with effect $\top$ have ``arbitrary'' effect.

\TODO{talk about a signature at a high-level: \emph{types, instructions, and effects}}

\subsection{Syntax}

\TODO{types are parametrized by \emph{base types}, effects and instructions are parameters}

\TODO{we will discuss the grammar of types}

\TODO{now: we will discuss what effect are}

The core purpose of our type system is to determine which substitutions are valid so as to use them
as building blocks for more complex optimizations, building on the observation in
\citet{ghalayini-24-ssa-densem-arxiv} that substitution can be used to build up a complete
equational theory for SSA. Since we are building a \emph{refinement} theory, however, we need to
track \emph{directed} substitution validity, which is determined by:
\begin{itemize}
  \item Whether an effect $\epsilon$ is a \emph{left-mover} (i.e. can be moved before) or
  \emph{right-mover} (i.e. can be moved after) an effect $\eta$.
  \item Whether an effect $\epsilon$ is \emph{duplicable}, i.e., it is a refinement to replace it
  with two copies of itself, or \emph{fusable}, i.e., it is a refinement to fuse two copies of it
  into one
  \item Whether an effect $\epsilon$ is \emph{eliminable}, i.e., it is a refinement to remove it,
  or \emph{introducable}, i.e., it is a refinement to introduce it
  \item Whether an effect $\epsilon$ is \emph{iterable}, i.e., if doing it in a (potentially
  infinite) loop does not introduce any new effects; in particular, all iterable effects must have
  nontermination as a \emph{subeffect}.
\end{itemize}
To be able to do this, we need to track how often variables are used. While we could use a natural
number for this purpose, the number of times a variable is used may be control-flow dependent
...
drawn from an abstract lattice mirroring the basic usage patterns described above. In particular,
...
\end{itemize}
While it would be more natural to have an element corresponding to being used (strictly) multiple
times, this would make our semantics more complex.

We will call the set $Q^0 = \{\zeroq, \oneq, \cpyq, \delq, \topq\}$ the \emph{extended} set of
quantities, and $Q = \{\oneq, \cpyq, \delq, \topq\}$ the set of (\emph{nonzero}) quantities. We can
represent the data of how an effect can be deleted/duplicated by simply assigning each effect a
\emph{pair} of quantities $\alquant^+(e), \alquant^-(e) \in Q$, such that
\begin{itemize}
...
$\alquant^p(\bot) = \topq$ for $p \in \{+, -\}$.

\TODO{just like we can give an effect a quantity, we can give a type a quantity}

Combining the above with tracking of commutativity and whether effects are iterative leads us to the
following definition of an \emph{effect system}:
...
\TODO{if we add some instructions which get an effect, we get the definition of a \subiterssa{}
signature}
...
\TODO{segue into grammar; reference figures}
...
\TODO{introduce contexts, every variable is annotated with a quantity $q$}

\TODO{remove the effects from variables since changed that in formalization}

\TODO{discuss how a whole context can also be given a quantity using the quantity of its variables}
...
\TODO{discuss weakening, use this to further discuss quantities}

\TODO{contrast with \emph{splitting}, which uses duplicability rather than discardability}
...
\TODO{typing rules for monoidal subtheory; very basic first-order system}
...
\TODO{extension with coproducts; corresponds to adding branching control-flow}
...
\TODO{extension with Elgot structure; corresponds to adding loops}

\TODO{explain: all variables used in a loop must be \emph{nonlinear} (not just relevant!) because
the loop may execute 0, 1, \emph{or} many times}

\TODO{the effect of a loop must be \emph{iterative} for obvious reasons (since the loop may execute
...
\begin{align*}
  \anflet{w}{x}{e} &:= (\letexpr{w}{x}{e}) \\
  \anflet{w}{f\;a}{e} &:= \anflet{w_a}{a}{\anflet{w}{f\;w_a}{e}} \\
  \anflet{w}{(a, b)}{e} &:= \anflet{w_a}{a}{\anflet{w_b}{b}{(\letexpr{w}{(w_a, w_b)}{e})}} \\
  \anflet{w}{}{} &:=
\end{align*}

\TODO{and this preserves equations, yo}
...
\TODO{and now we actually define SSA and show it is the same thing}
...
\begin{gather*}
  ...
  \prftree[r]{\rle{let$_1$}}
    ...
    {\haslb{\Gamma^{\mb{q}}_{\mb{e}}}{\epsilon}{\letstmt{x}{a}{t}}{\ms{L}^{\mb{Q}}_{\mb{e}}}}
  \\
  \prftree[r]{\rle{let$_2$}}
    {\qsp{\Gamma}{\mb{q}}{\mb{q}_l}{\mb{q}_r}}
    {\hasty{\Gamma^{\mb{q}_r}_{\mb{e}}}{\epsilon}{a}{A \otimes B}}
    {\haslb{\Gamma^{\mb{q}_l}_{\mb{e}}, x : A, y : B}{\epsilon}{t}{\ms{L}^{\mb{Q}}_{\mb{e}}}}
    {\haslb{\Gamma^{\mb{q}}_{\mb{e}}}{\epsilon}{\letstmt{(x, y)}{a}{t}}{\ms{L}^{\mb{Q}}_{\mb{e}}}}
  \\
  \prftree[r]{\rle{case}}
    {\qsp{\Gamma}{\mb{q}}{\mb{q}_l}{\mb{q}_r}}
    {\hasty{\Gamma^{\mb{q}_r}_{\mb{e}}}{\epsilon}{a}{A + B}}
    {\haslb{\Gamma^{\mb{q}_l}_{\mb{e}}, x : A}{\epsilon}{s}{\ms{L}^{\mb{Q}}_{\mb{e}}}}
    {\haslb{\Gamma^{\mb{q}_l}_{\mb{e}}, y : B}{\epsilon}{t}{\ms{L}^{\mb{Q}}_{\mb{e}}}}
    {\haslb{\Gamma^{\mb{q}}_{\mb{e}}}{\epsilon}{\casestmt{a}{x}{s}{y}{t}}{\ms{L}^{\mb{Q}}_{\mb{e}}}}
  \\
  \prftree[r]{\rle{loop}}
    {\qsp{\Gamma}{\mb{q}}{\mb{q}_l}{\mb{q}_r}}
    {\hasty{\Gamma^{\mb{q}_r}_{\mb{e}}}{\epsilon}{a}{A}}
    {\haslb{\Gamma^{\mb{q}_l}_{\mb{e}}, x : A}{\epsilon}{t}
      {\ms{L}^{\qbc{\mb{Q}}{0}}_{\mb{e}}}, \lhyp{\ell}{A}^{\mb{q}_l, 0}_\epsilon}
    {\einf{\epsilon}}
    {\haslb{\Gamma^{\mb{q}}_{\mb{e}}}{\epsilon}{\loopstmt{a}{\ell}{x}{t}}{\ms{L}^{\mb{Q}}_{\mb{e}}}}
  \\
  \prftree[r]{\rle{where$_{\ms{nonrec}}$}}
    {\haslb{\Gamma^{\mb{q}}_{\mb{e}}}{\epsilon}{r}
    ...
    {\haslb{\Gamma^{\mb{q}}_{\mb{e}}}{\epsilon}{\awhere{r}{(\wbranch{\ell_i}{x_i}{t_i},)_i}}
      {\ms{L}^{\mb{Q}}_{\mb{e}}}}
  \\
  ...
  {\haslb{\Gamma^{\mb{q}}_{\mb{e}}}{\epsilon}{\cwhere{r}{(\wbranch{\ell_i}{x_i}{t_i},)_i}}
    {\ms{L}^{\mb{Q}}_{\mb{e}}}}
\end{gather*}

\TODO{this}

\subsection{Compiling Terms to SSA}

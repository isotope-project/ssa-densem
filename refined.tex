...
Most major compilers today use \emph{static single assignment form}, or \emph{SSA}, as their intermediate representation. 
%
Unlike traditional 3-address code, SSA makes it much easier to perform algebraic, equational reasoning on values. 
%
\citet{ghalayini-24-ssa-densem-arxiv} give an equational theory of SSA designed to enable reasoning about such algebraic rewrites in a variety of different settings. 
%
Compiler writers, however, are often interested in rewritings which are not \emph{equivalences}, but rather \emph{refinements}. 
%
We would like to extend the equational theory of SSA to a \emph{refinement theory} to be able to reason about such rewrites axiomatically.

To be able to build up such a theory without tying ourselves to a fixed set of primitives, 
%
we need to \emph{classify} effects into different kinds. 
%
We begin with \citet{fuhrmann-direct-1999}'s notions \emph{copyable} and \emph{discardable} effects. 
%
An operation with \emph{discardable} effect is one for which, if the output is unused, it is safe to discard the operation entirely.
%
An example of such an effect is \emph{randomness}: 
%
while probabilistic operations are \emph{impure}, we can safely discard the result of an unused coin-flip.
%
\TODO{example program here: discarding a coin flip is valid, duplicating one is not}
%
An operation with \emph{copyable} effect is one in which performing the operation once and then using the output twice is the same as performing the operation twice on copied inputs.
%
An example of such an effect is \emph{nontermination}: 
%
while we cannot totally eliminate a call to a potentially nonterminating function, we can freely duplicate such a call.
%
\TODO{example program here: collatz step function duplication is valid, but is it truly valid to discard multiplication by 0?}
%
\TODO{systems example, UB is also copyable}
%
\TODO{but UB is \emph{not} discardable}
%
\TODO{we can remove it}
%
\TODO{but we can't add it}
%
\TODO{segue...}

%
To introduce refinement into the mix,
%
we can generalize discardable effects into \emph{erasable} and \emph{NAME GOES HERE (introducable?)},
%
and copyable effects into \emph{fusable} and \emph{duplicable}.
%
\TODO{UB is the canonical example of an erasable effect: you can remove potential UB, but its
dangerous to introduce!}
%
\TODO{similarly, while nondeterminism is not copyable, it is \emph{fusable}}
%
\TODO{what about the other way around: \emph{duplicable} and \emph{introducable?}?}
%
\TODO{\emph{memory fences} are introducable and copyable, but not erasable}
%
\TODO{think of an effect which is duplicable but not fusable...}
...
\begin{enumerate}
  ...
  \begin{itemize}
      \item \citet{fuhrmann-direct-1999}'s \emph{central/copyable/discardable}

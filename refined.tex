...
can jump to other basic blocks in the graph. For example, the program
\TODO{a program with a for-loop, say, factorial, as an lstlisting in Rust-like pseudocode}
compiles to the RTL code
\TODO{the RTL code}

Since variables may be redefined in RTL code, algebraic reasoning about programs can be quite
difficult, especially in the presence of complex control-flow.
\TODO{SSA fixes this by borrowing an idea from functional programming} %
\TODO{we can add arguments to basic blocks (or equivalently, $\phi$-nodes) to deal with control-flow
dependent values} %
\TODO{the program above becomes this} %
\TODO{we can now do algebra} %
\TODO{in \emph{particular}, substitution of pure expressions is unconditionally valid} %
\TODO{\citet{ghalayini-24-ssa-densem-arxiv} use this to give a sound and complete equational theory
for SSA} %
\TODO{pure expressions can be substituted and we can do arbitrary algebra} %
\TODO{we can also do arbitrary control-flow optimizations like B\"ohm-Jacopini} %
\TODO{combined with algebra we get Fun Stuff (TM) like Sparse Conditional Constant Propagation} %
\TODO{impure expressions are treated opaquely} %
\TODO{but not all the algebra we want to do is equational} %
\TODO{consider division; division by zero is UB} %
\TODO{but division isn't just some opaque IO operation} %
\TODO{substitution of division is not an equation, but a \emph{refinement}, since...} 
it is always safe to rewrite $\letexpr{x}{\ms{div}\;y\;z}{e}$ to

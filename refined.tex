...
\begin{definition}[\subiterexp-model]
  ...
  \begin{itemize}
    ...
    \item If $\zeroq \leq \alquant^+(\epsilon)$ and $A, B$ affine, $f ; !_B \tref !_A$
    \item If $\zeroq \leq \alquant^+(\epsilon)$ and $A$ relevant, $B$ affine, 
      $\dmor{A} ; (f ; !_B) \otimes \dnt{A} \tref \rho^{-1}$
    \item If $\cpyq \leq \alquant^+(\epsilon)$ and $A, B$ relevant, 
    $f ; \dmor{B} \cref \dmor{A} ; f \ltimes f = \dmor{A} ; f \rtimes f$
    \item If $\zeroq \leq \alquant^-(\epsilon)$ and $A, B$ affine, $f ; !_B \anticref !_A$
    \item If $\zeroq \leq \alquant^-(\epsilon)$ and $A$ relevant, $B$ affine, 
      $\dmor{A} ; (f ; !_B) \otimes \dnt{A} \anticref \rho^{-1}$
    \item If $\cpyq \leq \alquant^-(\epsilon)$ and $A, B$ relevant, 
    $f ; \dmor{B} \anticref \dmor{A} ; f \ltimes f = \dmor{A} ; f \rtimes f$
  \end{itemize}
  where
  \begin{itemize}
...
well-defined. The rest of the structure of a \subiterexp{}-model is given in Appendix~\ref{apx:syn-model}.

By soundness (since all members of an equivalence class with the same variable $x$ are equivalent) 
and semantic substitution (to allow us to rename variables, since a renaming has the identity as
denotation), for any $f : \ms{Tm}_\epsilon(\mc{R})(A, B)$, we have that, for any model 
$\mc{M} \models \mc{R}$,
$$
\forall (x, a), (y, b) \in f, \dnt{\hasty{x : A^\top}{\epsilon}{a}{B}}_{\mc{M}} 
  = \dnt{\hasty{x : A^\top}{\epsilon}{b}{B}}_{\mc{M}}
$$
It follows that we can lift $\dnt{\cdot}_{\mc{M}}$ to $\ms{Tm}_\epsilon(\mc{R})$ by defining, for arbitrary
$(x, a)$ in the equivalence class,
$
\dnt{f}_{\mc{M}} = \dnt{\hasty{x : A^\top}{\epsilon}{a}{B}}_{\mc{M}} 
  : \mc{C}_\epsilon(\dnt{A}, \dnt{B})
$. It therefore makes sense to ask whether we can relate 
$\dnt{\dnt{\hasty{\Gamma^{\mb{q}}}{\epsilon}{a}{A}}_{\ms{Tm}(\mc{R})}}_{\mc{M}}$ 
(where $\dnt{\hasty{\Gamma^{\mb{q}}}{\epsilon}{a}{A}}_{\ms{Tm}(\mc{R})} 
  : \ms{Tm}(\mc{R})(\etoty{\Gamma^{\mb{q}}}, A)$)
and
$\dnt{\hasty{\Gamma^{\mb{q}}}{\epsilon}{a}{A}}_{\mc{M}}$; it turns out we can via the 
\emph{packing lemma}:
\begin{lemma}[Packing]
  We can define functions $\ms{pack}$, $\ms{unpack}$ mapping derivations 
  $\hasty{x : \etoty{\Gamma^{\mb{q}}}}{\epsilon}{a}{A}$, $\hasty{\Gamma^{\mb{q}}}{\epsilon}{b}{A}$
  such that
  \begin{itemize}
    \item $\hasty{\Gamma^{\mb{q}}}{\epsilon}{
        \ms{pack}(\hasty{x : \etoty{\Gamma^{\mb{q}}}}{\epsilon}{a}{A})
        }{A}$ with
        $$
        \dnt{\hasty{\Gamma^{\mb{q}}}{\epsilon}{
          \ms{pack}(\hasty{x : \etoty{\Gamma^{\mb{q}}}}{\epsilon}{a}{A})
          }{A}}
          = \lambda^{-1} ; \dnt{\hasty{x : \etoty{\Gamma^{\mb{q}}}}{\epsilon}{a}{A}}
        $$
    \item $\hasty{x : \etoty{\Gamma^{\mb{q}}}}{\epsilon}{
      \ms{unpack}_x(\hasty{\Gamma^{\mb{q}}}{\epsilon}{b}{A})
      }{A}$ with
      $$
      \dnt{\hasty{x : \etoty{\Gamma^{\mb{q}}}}{\epsilon}{
        \ms{unpack}_x(\hasty{\Gamma^{\mb{q}}}{\epsilon}{b}{A})
        }{A}}
        = \lambda ; \dnt{\hasty{\Gamma^{\mb{q}}}{\epsilon}{b}{A}}
      $$
    \item $\tmeq{x : \etoty{\Gamma^{\mb{q}}}}{\mc{R}}
        {\ms{unpack}_x(\ms{pack}(\hasty{x : \etoty{\Gamma^{\mb{q}}}}{\epsilon}{a}{A}))}
        {a}
        {A}$
    \item $\tmeq{\Gamma^{\mb{q}}}{\mc{R}}
        {\ms{pack}(\ms{unpack}_x(\hasty{\Gamma^{\mb{q}}}{\epsilon}{b}{A}))}
        {b}
        {A}$
  \end{itemize}
  \label{lem:packing}
\end{lemma} 
Completeness immediately follows from the fact that
\begin{equation}
  (x, \ms{unpack}_x(\hasty{\Gamma^{\mb{q}}}{\epsilon}{a}{A})) \in
  \dnt{\hasty{\Gamma^{\mb{q}}}{\epsilon}{a}{A}}_{\ms{Tm}(\mc{R})}
  \label{eqn:unpacking}
\end{equation}
Details of the definition of $\ms{pack}$ and $\ms{unpack}$ and the proofs of Lemma~\ref{lem:packing} 
and Equation~\ref{eqn:unpacking} are given in Appendix~\ref{apx:packing}.
...
$$
\ms{Expr}(\haslb{\Gamma^{\mb{q}}}{\epsilon}{r}{\ms{L}^{\mb{Q}}}) := 
  \ms{pack}(\hasty{x : \etoty{\Gamma^{\mb{q}}}}{\epsilon}{a}{\ltoty{\Gamma}{\ms{L}^{\mb{Q}}}})
$$
It is straightforward to verify that this indeed satisfies the desired properties. On the other 
hand, the function $\ms{SSA}$ is essentially just the standard expression compilation presented 
inductively; we leave it's definition and verification to  Appendix~\ref{apx:ssa-roundtrip}. 
...
which they enable. For reasons of space, full details of each model are given in the appendix. We
note that, in all the models given here, every type has unrestricted linearity. Hence, we will often
write out our typing rules using \emph{un-annotated} contexts and variables $\Gamma, x: A$,
continuing the convention that a missing quantity annotation $\mb{q}$ corresponds to every variable
having unrestricted quantity $\topq$. We give a more sophisticated model taking advantage of
substructural types in Appendix~\ref{apx:sep}.

\subsection{UB}

Probably the simplest possible model which exercises all the features of our refinement calculus is
functions with nondeterminism, undefined behavior and nontermination, which we can represent 
using the monad $\ms{UB}\;A = \mc{P}^+(A \cup \{\infty\}) \cup \{\ubeff\}$ over sets, with $\infty$ 
a sentinel value for nontermination and $\ubeff$ for UB. We can equip this monad with the inclusion 
order on nonempty sets $\mc{P}^+(A \cup \{\infty\})$ extended with a top element $\ubeff$. 
Our monadic bind is given by
$$
...
\end{cases}
$$
where we define $f\;\infty = \infty$. We give a full description of this monad and its associated 
Elgot structure in Appendix~\ref{apx:ub}.
%
We can naturally equip the Kleisli category of $\ms{UB}$ with the lattice of effects generated by 
the nontermination effect $\infty$, the UB effect $\ubeff$, and the nondeterminism effect $?$.
Each subcategory is most naturally described as the image under inclusion from the appropriate 
monad, as shown, along with commutativity and linearity, in Figure~\ref{fig:ub-submonads}.
This model allows us to interpret instructions $\ubeff_A: \mb{1} \to_\ubeff A$ analogous to LLVM's
\texttt{unreachable} intrinsic. We simply write $\ubeff$ as shorthand for the operation 
$\ubeff_A\;()$. We can use this instruction to define \emph{unsafe unwrap} operators
$\rho_l : A + B \to_\ubeff A$ and $\rho_r : A + B \to_\ubeff B$
which simply take the other branch to $\ubeff$.
%
Our model can be shown to validate some of the usual rewrite rules for UB, given in 
...
Our effect system will be over the bounded lattice generated by 
$\{\infty, \ubeff, ?, \ms{r}, \ms{w}, \ms{a}\}$ (with $\ms{r}, \ms{w}, \ms{a}$ corresponding to 
``read'', ``write'', and ``allocate'' respectively) generated by inclusions, 
for all $\epsilon \in \{\infty, \ubeff, ?\}$,
$\ms{Heap}_\epsilon(A, B) \approx A \to \ms{UB}_\epsilon(B)$ and primitive effectful instructions
\begin{itemize}
  ...
  \item $\ms{rm} : \ms{Heap}_{\ms{w}\ubeff}(\nats, ()) :=
    ...
    $
\end{itemize}
This model allows us to validate a few useful properties of heaps, given in 
Figure~\ref{fig:heap-eqns}. These, combined with the fact that the allocation effect is central
and that reads are relevant and eliminable (but not affine, since \emph{introducing} a read may
introduce UB!) is enough to implement a simple \texttt{mem2reg}, details of which are given in
Appendix~\ref{apx:mem2reg}.

We note that so far, none of our models make use of our ability to handle substructural 
\emph{types}, rather than merely \emph{effects}. It turns out that our model of heaps is rich enough 
to \emph{build} a simple, higher-level model implementing \emph{separation logic}; for reasons of
space, this is relegated to Appendix~\ref{apx:sep}.
...
In Brookes's \cite{brookes-full-abstraction-96} semantics for concurrency, a program term's
effect is denoted by a trace consisting of a finite sequence of transitions 
$\langle \mu, \rho \rangle$, where $\mu$ and $\rho$ are maps from locations to values, representing
the current state of memory. We interpret such traces as
\emph{interrupted} program executions: for example, the trace $\langle \mu_1, \rho_1 \rangle ;
\langle \mu_2, \rho_2 \rangle$ can be viewed as the program, starting in condition $\mu_1$,
executing up until condition $\rho_1$, at which point control is returned to the environment, which
then returns control to the program with the system in state $\mu_2$, and so on. As a concrete
example, the program $\ms{st}\;\ell\;5 ; \ms{st}\;\ell\;3$ might have semantics
$
\langle \mu, [\ell \mapsto 5]\mu \rangle ; \langle \mu', [\ell \mapsto 3]\mu' \rangle
$
for some $\mu, \mu'$. We can think of the left-hand side of each pair as a \emph{requirement} the
program makes of the environment, and the right-hand side as a \emph{guarantee} the program gives
the environment if that requirement is fulfilled. A program's denotation is then given by a
\emph{set} of pairs $\tret{\xi}{r}$ of a trace $\xi$ and return value $r$.

The key observation of \citet{brookes-full-abstraction-96} is that we can achieve \emph{abstraction}
of a concurrent memory model (in this case, sequentially consistent concurrent execution) by
requiring our set of traces to satisfy certain \emph{closure properties}. In particular, we want to
allow our program to ``\emph{stutter},'' i.e., insert additional steps where it does nothing, and
``\emph{mumble},'' i.e. omit steps where the environment does nothing.

\TODO{SC closure rules are stutter (the system does nothing; or, a program can always guarantee what
it relies on) and mumble (the environment does nothing; or, a program can always omit a guarantee to
the environment, and rely on that omitted guarantee rather than the environment)}

\TODO{we can use this to make a monad exposing SC as a side effect}

\TODO{Simplest possible Elgot structure just ignores divergent executions; do this for now}

\TODO{We can validate fun rules like write-write elimination}

\TODO{full details in Appendix~\ref{apx:sc}}

\TODO{have RA version using special RA memory + extra closure rules, full details in
Appendix~\ref{apx:rel-acq}}

\TODO{``eliminations and others''; note these are all \emph{equations} in heapland}

\TODO{
  memory is weaker so we can validate more things; hard to do disjoint re-ordering though since
  disjunction is !FUN!...
}

\TODO{pointer to \citet{jagadeesan-brookes-relaxed-12} senpai}
...
\TODO{initial objects (syntactic models)}

\TODO{coproducts}

\section{\texttt{mem2reg}}

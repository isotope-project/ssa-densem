...
As a concrete example, consider the interaction of undefined behaviour and other effects. For
example, consider an operation like \ms{div}, which exhibits \emph{undefined behavior} in both C and
LLVM if we divide by zero. Let's say we have a program like $\letexpr{x}{\ms{div}\;y\;z}{e}$ (with
$e$ having arbitrary effect), where the optimizer cannot deduce whether $z$ is nonzero; since
...
\end{itemize}
Such a substitution has the effect of moving an effect \emph{forward} in the program's execution, so
we call UB a \emph{right-mover} with respect to all other effects. Since it is also always a
refinement both to eliminate and to duplicate UB, it is \emph{eliminable} and \emph{duplicable}. In
fact, we have that $(\ms{div}\;y\;z, \ms{div}\;y\;z) \approx \letexpr{x}{\ms{div}\;y\;z}{(x, x)}$,
so UB is also \emph{fusable} and hence \emph{relevant}. On the other hand, UB is obviously not
\emph{introducable}, since $() \not\tref \letexpr{x}{\ms{div}\;y\;z}{()}$, and so is therefore not
\emph{affine}.

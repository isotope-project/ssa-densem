...
\begin{itemize}
  ...
  for each label in $\ell_i(A_i)^{\mb{q_i}}$ in $\ms{R}^{\mb{Q}'}$, an 
  associated region $t_i$ which, with variables $\Gamma^{\mb{q}_i}$ plus an argument $x_i : A_i$ 
  live on entry, targets the exit labels $\ms{L}^{\mb{Q}}$. 
  ...
  allows the $t_i$ to make recursive calls to $\ms{R}^{\mb{Q}'}$ as well.
\end{itemize}
...
expressions.
\begin{itemize}
  \item \todo{br semantics}
  \item \todo{let and case semantics}
  \item \todo{where nonrec semantics}
  \item \todo{where semantics}
\end{itemize}

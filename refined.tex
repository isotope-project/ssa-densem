...
In Brookes's \cite{brookes-full-abstraction-96} semantics for concurrency, a program term's effect 
is denoted by a trace consisting of a finite sequence of transitions $\langle \mu, \rho \rangle$ 
between memory states $\mu$ and $\rho$.
%
We interpret such traces as \emph{interrupted} program executions: a transition $\langle \mu, \rho
\rangle$ represents a single step in which the program, \emph{relying on} a state $\mu$ by the
environment, \emph{guarantees} a state $\rho$ upon returning control to the environment.
%
A program's denotation is then given by a \emph{set} of pairs $\tret{\xi}{r}$ of a trace $\xi$ and
corresponding return value $r$; each such pair corresponds to a possible execution of the program
given potential environmental interference (e.g., some external thread performing reads and writes);
for simplicity, we will simply discard any executions which do not terminate with a return value
$r$.

The key observation of \citet{brookes-full-abstraction-96} is that we can achieve \emph{abstraction}
of a concurrent memory model by requiring our set of traces to satisfy certain \emph{closure
properties}. For example, we may require traces to be closed under ``\emph{stuttering}'' (adding
transitions of the form $\langle \mu, \mu \rangle$, corresponding to the program doing nothing) and
``\emph{mumbling}'' (merging $\langle \mu, \rho \rangle\langle \rho, \theta \rangle \tref \langle
\mu, \theta \rangle$, corresponding to the environment doing nothing). 

\TODO{define the monad}

By choosing the appropriate state types, closure properties, and operations on states, it is
possible to model a wide variety of memory models in this fashion.

\TODO{SC example, simplest possible Elgot structure just ignores divergent executions; do this for
now}

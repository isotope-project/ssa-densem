...
Our goal is to model our programming language with our categorical structures; thus, we want to
require only structures which correspond closely to the basic type-formers of \subiterexp{}, such
that the $\beta$ and $\eta$ rules for those type formers correspond to universal properties.

As is standard in categorical semantics, we will want to model the denotation of a judgement
$\dnt{\hasty{\Gamma}{\epsilon}{a}{A}}$ as a morphism $\dnt{\Gamma} \to \dnt{A}$, with contexts and
types both modeled by objects. Equivalent programs should then have equal denotations, and the
semantics of programs should be composition w.r.t. their denotations.

To define our mapping from programs to morphisms, we must start by defining the notion of a
\emph{premonoidal category}, which was introduced in \todo{this} to model sequencing and binding in
first-order imperative languages.
...
\TODO{give semantics of types here, or leave it to the end?}

We can use the tensor unit $I$ and tensor product $\otimes$ to give a denotation for the unit type
and product types respectively. We can further model contexts as tensors of their components, i.e.,
we take
$$
\dnt{\mb{1}} = I \qquad
\dnt{A \otimes B} = \dnt{A} \otimes \dnt{B} \qquad
\dnt{\cdot} = I \qquad
\dnt{\Gamma, x : A} = \dnt{\Gamma} \otimes \dnt{A}
$$

Unfortunately, premonoidal structure is not quite enough to model all the features we need in our language. Specifically, we need to:
\begin{itemize}
  \item Handle variables which are unused, or used multiple times (when the type's linearity allows this)
  \item Handle \emph{control-flow}: branching and looping.
\end{itemize}

For the latter, we can represent branching control-flow using coproducts. We require, however, that
our coproducts interact nicely with the premonoidal structure; that is, we want
\emph{distributivity}:
...
To model looping, we need to introduce a fixpoint operator, which we do by requiring the existence of a \emph{Conway iteration operator} as follows:
\begin{definition}[Conway Iteration Operator]
  ...
  \end{itemize}
\end{definition}
We'd like our Conway iteration operator to play nicely with our premonoidal structure; in particular, we require our Conway iteration operator to be \emph{strong}, defined as follows:
...
\TODO{poset-enrichment}

\TODO{effectful structure}

\TODO{duplicate-delete structure}

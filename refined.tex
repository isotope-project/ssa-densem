...
\begin{definition}[\subiterssa-signature]
  ...
  \begin{itemize}
    ...
    \item A bounded lattice of \emph{effects} $\epsilon \in \mc{E}$
    \item An upwards-closed set of \emph{looping effects} $\mc{E}^\infty \subseteq \mc{E}$
    containing $\top$
  \end{itemize}
  such that every instruction is associated with a source type $\ms{src}(f) = A$, a target type
  $\ms{trg}(f) = B$, and an effect $\ms{eff}(f) = e$ (where types are given by the grammar in
  Figure~\ref{fig:subiterssa-syntax}).
\end{definition}

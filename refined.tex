...
\begin{itemize}
  \item (Valid operations $\hasty{\Gamma^{\mb{q}_r}}{\epsilon}{o}{A}$): we define
  ...
  $(\tossa{\ell}{\letexpr{x}{o}{P}}) := \letstmt{x}{o}{\tossa{\ell}{P}}$
  \TODO{not so trivial anymore, buster}
...
\TODO{example: power-set monad, nonempty}

\TODO{define an Elgot monad}

\TODO{the simplest Elgot monad is...}

\TODO{also, see the power set monad}

\TODO{but NOT the nonempty power set monad; non-iterative effect}
...
\begin{equation*}
  f^\dagger(a) := \ms{c}_B(\bigcup_{i \in \nats}f_i(a))
\end{equation*}
where $f_i : A \to \ms{B}_{\mc{C}}(B)$ are the \emph{iterates} of $f$, defined inductively as follows
\begin{equation*}
  f_0 := (\lambda x . \varnothing) \qquad
  f_{i + 1} := f ; [\ms{id}_B, f_{i}]
\end{equation*}
We can think of analysis using this structure as corresponding to \emph{partial correctness}, since
any infinite traces are simply discarded (a program which always diverges will have denotation
$\varnothing$). It is straightforward to check that this is indeed an Elgot structure, since it
satisfies:
\begin{itemize}
  \item \emph{Fixpoint}: we have, since $f_0(a) = \varnothing$,
  \begin{align*}
      f^\dagger(a)
      & = \ms{c}_B(\bigcup_{i \in \nats}f_i(a))
        = \ms{c}_B(\bigcup_{i \in \nats}f_{i + 1}(a)) \\
      & = \ms{c}_B(\bigcup_{i \in \nats}(f ; [\ms{id}_B, f_i])(a)) \\
      & = \ms{c}_B(\bigcup_{i \in \nats}\ms{c}_B(\{
        \tret{t \cdot t'}{b} \mid 
          \exists c . \tret{t}{c} \in f(a), \tret{t'}{b} \in [\ms{id}_B, f_i](c)
      \})) \\
      & = \ms{c}_B(\{
        \tret{t \cdot t'}{b} \mid 
          \exists c . \tret{t}{c} \in f(a), \tret{t'}{b} \in [\ms{id}_B, \bigcup_{i \in \nats}f_i](c)
      \}) \\
      & = \ms{c}_B(\{
        \tret{t \cdot t'}{b} \mid 
          \exists c . \tret{t}{c} \in f(a), \tret{t'}{b} \in [\ms{id}_B, f^\dagger](c)
      \}) \\
      & = f ; [\ms{id}_B, f^\dagger](a)
  \end{align*}
  as desired.
  \item \emph{Naturality}: we have
  \TODO{this}
  \item \emph{Codiagonal}: we have
  \TODO{this}
  \item \emph{Uniformity}: we have
  \TODO{this}
\end{itemize}
...
\TODO{our state is partial functions from locations to values}

Given a set of rewrites $a \tref b$ for $a, b \in T$ closed under monoid multiplication (i.e., $a
\tref b \implies l \cdot a \cdot r \tref l \cdot b \cdot r$), we can verify that $\ms{c}(X) := \{t'
\mid \exists t \in X, t \tref^* t'\}$ is a valid closure operator.

\TODO{our closure operator is given by stuttering and mumbling; intuition}

\subsection{Weak Memory Models}

\TODO{wave-instantiate for release-acquire following \citet{release-acquire}}

\TODO{wave-instantiate for TSO following \citet{jagadeesan-brookes-relaxed-12}}

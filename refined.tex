...
using the monad $\ms{UB}\;A = \mc{P}^+(A \cup \{\infty\}) \cup \{\ubeff\}$, with $\infty$ a sentinel 
value for nontermination and $\ubeff$ for UB. We can equip this monad with the inclusion order on 
nonempty  sets $\mc{P}^+(A \cup \{\infty\})$ extended with a top element $\ubeff$. Our monadic bind 
is given by
$$
...
the nontermination effect $\infty$, the UB effect $\ubeff$, and the nondeterminism effect $?$, with
\begin{itemize}
  \item $\ms{UB}_\infty(A, B) \approx A \to B \cup \{\infty\}$
  \item $\ms{UB}_\ubeff(A, B) \approx A \to B \cup \{\ubeff\}$
  \item $\ms{UB}_?(A, B) \approx A \to \mc{P}^+(B)$
\end{itemize}

\TODO{make a table?}

\TODO{$?$ is central, affine, and not relevant \emph{but} is duplicable}

\TODO{$\infty$ is commutative, relevant, but not affine}

\TODO{$\ubeff$ is commutative and a right-mover, relevant, and not affine \emph{but} is eliminable}

\TODO{$\ubeff?$ is like $\ubeff$ but not relevant, only duplicable maybe?}

\TODO{$\infty?$ is like $\infty$ maybe?}

\TODO{$\infty\ubeff$ is?}

\TODO{operations: $\ms{undef}$, $\ms{choose}$, etc.}
...
our previously defined monad $\ms{ND}$, we can use this to define a heap monad 
$\ms{Heap}\;A = S \to \ms{ND}\;(A \times S)$, with the natural pointwise partial order.
...
\begin{itemize}
  ...
  \item $\ms{Heap}_\infty(A, B) \approx \ms{ND}_\infty(A, B)$ and 
        $\ms{Heap}_\ubeff(A, B) \approx \ms{ND}_\ubeff(A, B)$
\end{itemize}

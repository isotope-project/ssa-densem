...
the grammar given in Figure~\ref{fig:ssa-syntax}.

\TODO{in particular, we give a grammar for \emph{regions} $r, s, t$, which are:
\begin{itemize}
  \item A sequence of let-bindings for operations $o$, followed by;
  \item A terminator, making up a \emph{basic block}, wrapped by
  \item A \emph{where-block}, which encodes the children dominated by that basic block. We
  distinguish between recursive/non-recursive where for type-theoretic/effect system purposes, but
  the semantics are the same.
\end{itemize}
}

\TODO{i.e. you can squint and see this is SSA due to the data in Figure~\ref{fig:ssa-data}}

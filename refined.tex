...
We now give the semantics of each of our term formers in Figure~\ref{fig:expr-densem}. Each of them
corresponds quite closely to the the underlying categorical structure. In particular,
\begin{itemize}
  \item \todo{variables are weakenings}
  \item \todo{let-binding and pairs are sequencing}
  \item \todo{units are discards}
  \item \todo{case statements are the discriminator, and then the coproduct of the branches. we use
  the distributor to pass variables in, as discussed.}
  \item \todo{injections are just injections, and aborts are the zero morphism}
  \item \todo{iteration expressions are the initial value followed by the fixpoint of the body;
  again, we use the distributor to pass variables into the recursive branch}.
\end{itemize}

\TODO{ throughout, we use the notation $\dnt{\hasty{\Gamma^{\mb{q}}}{\epsilon}{a}{A}}$ to stand in
  for what is actually an induction on derivations. but, this notation is in general justified by
  \emph{coherence}: }
...
\TODO{ we also introduce the notation $\dnt{\hasty{\Gamma^{\mb{q}}}{}{a}{A}}$ since the effect
  $\epsilon$ doesn't affect the denotation, either. }
...
\TODO{as a first sanity-check, our semantics satisfies \emph{weakening}, stated as follows}
...
\TODO{the semantics of substitutions is given pointwise; unused variables are discarded}
...
\TODO{soundness is the usual statement, but directed}
...
\TODO{soundness means that if $\mc{M}$ models $\mc{R}$, it validates the whole theory of $\mc{R}$}

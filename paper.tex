...
Converting directly to strict SSA can be unwieldy. Therefore, we start by considering A-normal form
(ANF), extended with mutually recursive \ms{where}-bindings, which we define as a subset of our
language. Extending the work of \citet{chakravarty-functional-ssa-2003}, we provide an algorithm for
converting between \isotopessa{} and ANF, which we show is equivalence preserving. We will then
provide an algorithm for converting from ANF to strict SSA, as desired.
...
function application, injection, or pair of variables, or the unit expression. We specifically

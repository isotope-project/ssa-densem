...
Claim~\ref{claim:ssa-wf} establishes an equivalence, up to our equational theory, via lexical SSA
and traditional SSA. In particular, we treat a lexical SSA region as an SSA program equipped with a
\emph{semantically irrelevant} choice of dominator tree, represented by our \ms{where}-bracketing,
which serves only to make our typing rules syntax-directed.

Converting directly to lexical SSA can be unwieldy. Therefore, we break the transformation down into
two lowering passes: 
\begin{enumerate}
...
\TODO{kill typing rules; we only need a sub-grammar here. Adjust proof as needed.}
...
an injection, a pair of variables, or the unit expression; these form a (strict) subset of the
syntactic category $o$ of operations. In particular, we specifically forbid \ms{case}-expressions
and \ms{let}-bindings at this level; they are required to appear only at the region level. It is
obvious that $\ahasty{\Gamma}{\epsilon}{a}{A} \implies \hasty{\Gamma}{\epsilon}{a}{A}$. 
...
\TODO{lexical SSA region; not strict region}

\TODO{kill rules; all we need is the grammar (already in intro, unlike ANF)}

\TODO{we can probably simplify this dramatically; do we need the BB function...}

\TODO{kill struct; we already gave it in intro}

\TODO{kill standard SSA grammar; we already gave it in intro}

We now wish to give typing rules for strict regions. Since SSA consists of \emph{basic blocks}, that
is, linear sequences of instructions followed by a terminator, we will introduce two mutually

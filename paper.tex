...
\begin{itemize}
  \item The unit type $\mb{1}$ is modeled as the terminal object
  \item The empty type $\mb{0}$ is modeled as the initial object
...
\begin{itemize}
  \item \todo{variables}
  \item \todo{primitive instructions}
  \item \todo{let-bindings}
  \item \todo{pairs, unit}
  \item \todo{let-pairs}
  \item \todo{injections, abort}
...
We can now interpret \isotopessa regions $\haslb{\Gamma}{r}{\ms{L}}$ as morphisms 
$\dnt{\Gamma} \to_\epsilon \dnt{\ms{L}}$; branching to $\ms{L}$ with a parameter of type $A$ being
interpreted as ``landing'' in the appropriate summand of the coproduct $\dnt{\ms{L}}$. Other than
the semantics of \ms{where}-blocks, all the rules given in Figure~\ref{fig:ssa-reg-sem} are quite

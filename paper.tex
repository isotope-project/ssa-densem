...
\TODO{note the signposted argument above when doing grammar stuff}
...
\TODO{give grammar for standard-\isotopessa{} here}

\TODO{justify full \isotopessa{} here}
...
\TODO{give grammar for full \isotopessa{} \emph{here} instead of below}
...
\TODO{rework this example, since it is confusing. Maybe remove?}
...
\TODO{it was a \emph{big} mistake to annotate variables with effects. Should we remove this
throughout? Will this require re-doing the formalization?}
...
\begin{itemize}
  ...
  \item For each pair $A, B \in \ms{Ty}(X)$ and effect $\epsilon \in \{\bot, \top\}$, a set of
  \textit{primitive instructions} $f \in \mc{I}_\epsilon(A, B)$, where $\epsilon \leq \epsilon'
  \implies \mc{I}_\epsilon(A, B) \subseteq \mc{I}_{\epsilon'}(A, B)$. 
...
$\epsilon \in \{\top, \bot\}$ is the effect of using that variable (used when filling holes with
effectful expressions). If $\epsilon = \bot$, we often omit it, writing $\bhyp{x}{A}$. Similarly, we
define a \textit{label-context} to be a list of \textit{labels} $\lhyp{\ell}{A}$, where $A$ is the
parameter type that must be passed on a jump to the label $\ell$.
...
\begin{itemize}
  ...
  (B, \epsilon') \iff A = B \land \epsilon \leq \epsilon'$.
...
regions. In particular, an \isotopessa{} expression model consists of:
\begin{itemize}
...
More precisely, we can define the \emph{effect} of a context using the lattice structure on $\{\top,
\bot\}$ as follows
\begin{equation}

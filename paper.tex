...
Many of the design choices in \citet{leaky-semicolon} are motivated by the desire for sequential
composition to be associative; i.e., to have $\dnt{(S_1 ; S_2) ; S_3} = \dnt{S_1 ; (S_2 ; S_3)}$:
earlier attempts at compositional weak memory models, such as that in \citet{jagadeesan-pwp-20},
often failed to maintain even this basic invariant. More complex properties, such as distributivity,
which corresponds to the equation
\begin{equation*}
  \dnt{S ; \ite{b}{T_1}{T_2}} = \dnt{\ite{b}{S ; T_1}{S ; T_2}}
\end{equation*}
are similarly unlikely to hold unless the model is specifically designed with them in mind, and
loops are often not handled at all, making them unsuitable as models of SSA without extensive
adaptation. One of the easiest ways to build a model with ``nice'' properties is to make the model
...
support loops, as is done in \citet{xia-itrees-20}. Our work simply pares away the minimal
conditions required to have a compositional model of general \emph{first-order} side effects
compatible with the standard interpretation of SSA; the higher-order analogue is precisely an Elgot
monad.

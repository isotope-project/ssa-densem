...
$
on terms $a, a'$ for each context $\Gamma$, effect $\epsilon$, and type $A$. This will be presented
as a set of simple \emph{rewriting rules} along with a \emph{congruence rules}, given in
Figure~\ref{fig:ssa-expr-congr-rules}, which merely ensure we can perform equational reasoning as
normal.

For each of the rules we will present, we assume the rule is valid if and only if \emph{both sides}
of the rule are well-typed. We also assume that variables are $\alpha$-converted as appropriate to
avoid shadowing; our actual formalization uses de-Bruijn indices, but we stick with a named
...
for $\ms{let}_2$ given in \ref{fig:ssa-binary-let-expr}; we have:
\begin{itemize}
...
\begin{figure}
  ...
  \end{gather*}
  \Description{}
  \caption{Rewriting rules for \isotopessa binary \ms{let} expressions}
  \label{fig:ssa-binary-let-expr}
\end{figure}

Similarly, it is enough to give $\eta$, $\beta$, and binding rules for \brle{case} expressions,
as in Figure \ref{fig:ssa-case-expr}. In particular, we have that
\begin{itemize}
...
\begin{figure}
  ...
  \label{fig:ssa-case-expr}
\end{figure}

The case for other the other constructors is even more convenient: no additional rules are required
at all to handle operations, pairs, and injections. For example, we can derive a the expected
...
\begin{align*}
  ...
  \teqv (\letexpr{x}{a}{f\;x})
\end{align*}
To have aborts and units behave as we expect; namely, that $\mb{0}$ acts as the empty type and
$\mb{1}$ the unit type, we need only introduce the following rules, given in
Figure~\ref{fig:ssa-initial-terminal-expr}:
\begin{itemize}
  \item \brle{initial}, which says that \emph{all} terms are equated in any context containing the
  empty type $\mb{0}$, since such we will take such a context to be unreachable by program execution
  (e.g. after an infinite loop, or an instruction or function call which never returns). 

  This can be combined with \brle{let1-$\beta$} to yield the more convenient rule
  \brle{initial-expr}, which states that any context in which a \emph{pure} term of empty type
  can be constructed equates all terms. Note in particular that this means divergent instructions
  must have impure effect!

  \item \brle{terminal}, which says that all \emph{pure} terms of type $\mb{1}$ are equal in any
  context. Note that \emph{impure} terms may be disequal, since while their result values are the
  same, their side effects may differ!
\end{itemize}
...
\begin{figure}
  ...
  \label{fig:ssa-initial-terminal-expr}
\end{figure}
...
We now come to the equational theory for \isotopessa \emph{regions}, which is somewhat more
complicated, due to needing to support arbitrary control-flow graphs in the form of
\ms{where}-statements. It is presented in a similar format, with a set of rewriting rules and a
congruence, the latter given in Figure~\ref{fig:ssa-reg-congr-rules}.
...
\TODO{
  let1 theory is the same as above: just need to commute with each expression constructor, +
  an extra rule for let1-expressions; should move \brle{initial} to the proper place later and
  restate \brle{initial-expr}
}
...
\TODO{let2 theory is again a reduction, eta, and bind}
...
\begin{figure}
  ...
  \end{gather*}
  \Description{}
  \caption{Rewriting rules for \isotopessa binary \ms{let}-statements}
\end{figure}

\TODO{case theory is again a reduction, eta, and bind}
...
\begin{figure}
  ...
  \end{gather*}
  \Description{}
  \caption{Rewriting rules for \isotopessa \ms{case}-statements}
\end{figure}

\TODO{
  basic where theory: simple reduction, binding rules for let12 and case, self-binding
}
...
\TODO{advanced where lore: lsubst-reduction, uniformity, and codiagonal}
...
\begin{figure}
  \begin{gather*}
      ...
      % but these are trivially equal by cfg-<
      \prftree[r]{\rle{uni}}
        {
          \prfStackPremises
  ...
  \end{gather*}
  \Description{}
  \caption{Rewriting rules for \isotopessa \ms{where}-statements}
\end{figure}

\subsection{Metatheory}

\TODO{congruence relation on substitutions, label-substitutions}
...
\begin{lemma}[Substitution Congruence]
...
\begin{lemma}[Label Substitution Congruence]
...
\begin{definition}[Pomset] 
  ...
  \to \mc{A}\) quotiented over the removal of \(\delta\) such that cardinality
  is preserved. A pomset is \textbf{finite} if its carrier set is. We will write
  unordered pomsets using multiset notation, e.g. \(\{a, a, b\}\), and linearly
...
\(\ms{trim}(R)\) removes all elements of \(R\) with infinitely many
...
% pomset of the events that would have be generated by a given execution (in the
...
% and later post-filter to obtain a set of valid executions. By doing so, our
% semantics remains compositional, allowing us to reason about each program
% fragment individually, while still allowing other program fragments to perform
% concurrent operations affecting our potential executions. 
% On that note, 
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
It's tempting to try to use this to define a tensor product of morphisms to
obtain a monoidal (rather than preomonoidal) category, but unfortunately,
sliding still fails: \((W_x^{\ms{PO}} || \ms{id}) ; (\ms{id} || W_y^{\ms{PO}})\)
emits the pomset \([x := a, y := b]\), while \((\ms{id} || W_y^{\ms{PO}}) ;
(W_x^{\ms{PO}} || \ms{id})\) emits the pomset \([y := b, x := a]\). 
...
pomset. At any point, we may choose to \textit{flush} some of the buffered
writes. We introduce the set \(\mc{A}_b = \{(\bufloc{x} := v)\}\) of
...
\mc{A}_b\); a pomset over this set is called a \textbf{TSO pomset}. A buffer
will be defined to be a list of write actions \(\ms{Buf} = \mc{A}_b^*\), which
we will interpret as linear pomsets over \(\mc{A}_{\ms{TSO}}\) ordered by index
(with the empty list corresponding to \(\{\delta\}\)). In particular, we define
the monad \(\ms{TSO} = \ms{StateT}\;\ms{Buf}\;(\ms{Trace}\;\Sigma)\), where
\(\Sigma\) is taken as a the stream action of finite TSO pomsets on TSO pomsets.
We can view \(\ms{PO}\) as a submonad of this monad.
...
\TODO{\ms{pflush}-sandwich subcategory; Elgot lore?}

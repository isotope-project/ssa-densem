...
\TODO{Introduce a figure with SSA and BBA side-by-side. Suggestion: Add arrows showing how the arguments and phi-functions correspond, maybe with arrows connecting the two (use \texttt{tikzmark} package). }
\begin{align*}
  \ms{start}:\quad & \ms{let}\;n = 10; \\
  & \ms{br}\;\ms{loop}(1, 1) \\
...
\TODO{After introducing BBA, recall Appel's SSA is FP paper, and then
  note that there is a mismatch wrt scoping. Then explain dominance
  based scoping.}

\TODO{After introducing where and showing how it turns the dominator
  tree into lexical scoping, then show how \texttt{e where f = e'} is the
  same as \texttt{let f = e' in e} and then talk about the SSA-ANF
  paper. Here also talk about full if-expressions.}

\TODO{Once we have ANF, observe that ANF has good scoping properties,
  but lacks a good substitution property, since substituting a value
  for a variable can take you out of ANF. So we relax things again to
...
\begin{itemize}
  ...
  \item Examples: \begin{itemize}
    ...
    body
  \end{itemize}
  \item In general, dominance always forms a tree (explain? cite? \TODO{Cytron paper})

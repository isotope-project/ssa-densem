...
and
label-substitutions $\lbsubst{\cdot}{\ms{pack}_\ell(\ms{L})}{\ms{L}}{\ell([\ms{L}])}$,
$\lbsubst{\cdot}{\ms{unpack}_\ell(\ms{L})}{\ell([\ms{L}])}{\ms{L}}$, which we will show to
be mutually inverse

We will prove this somewhat indirectly, via the following:
\begin{itemize}
  \item 
  \item 
\end{itemize}
...
substitutions are pointwise equal, then...

...
\emph{constant propagation}; \citet{cytron-ssa-intro-91} provided a foundational algorithm for
lowering programs from \emph{3-address code}, or \emph{register transfer language (RTL)}, into SSA
form with a minimal number of $\phi$-nodes, based on the graph-theoretic notion of a \emph{dominance
frontier}. Since then, SSA has become a cornerstone of modern compiler design, forming the basis of
intermediate representations in production-grade compilers, notably the LLVM compiler
infrastructure~\cite{llvm}. 

\TODO{cite GCC? Cranelift and friends?}

The key property that makes SSA effective for compiler implementation is that each variable is
assigned exactly once, simplifying the tracking of variable values throughout the program. For
...
perspective implies that SSA effectively exposes the functional program hidden inside the imperative
one. This functional interpretation of SSA highlights why it is so effective for optimization:
since, just like in functional programming, variables are immutable in SSA, it enables algebraic
reasoning techniques traditionally used in functional programming. Optimizations that are natural in
the functional paradigm, such as inlining, dead code elimination, and constant propagation, become
more accessible in SSA form.

\TODO{expand, segue}

The relationship between SSA and Continuation-Passing Style (CPS) was explored by
...
transformations avoid such features. Kelsey observed that many optimizations requiring flow analysis
in CPS could be performed directly in SSA, largely because CPS programs are predominantly functional
in nature.

\TODO{expand, segue}

Building on these insights, \citet{chakravarty-functional-ssa-2003} formalized a mapping from SSA to
A-normal form (ANF). They argued that ANF provides a clearer operational interpretation than general
lambda calculus, facilitating advanced functional optimizations like 0CFA (Zero-Control-Flow
Analysis). Their work also referenced \citet{flanagan-93-anf}, who advocated for ANF over CPS as an
intermediate representation, citing its efficiency and the reduced need for complex flow analyses.

\TODO{mention $\beta\eta$ optimizations?}

\TODO{Thorin \citet{thorin-12} stuff}

Despite these advancements, earlier works primarily focused on establishing correspondences between
SSA and functional representations without providing explicit semantics or comprehensive equational
theories. \citet{benton-kennedy-99} addressed this gap by introducing the Monadic Intermediate
Language (MIL). MIL is equipped with an operational semantics from which a rich equational theory is
derived, allowing for refinement through observational equivalence. Their approach differs from
Moggi's equational metalanguage \cite{moggi-91-monad} by emphasizing values and computations without
nesting computations within computations, akin to the distinction between Kleisli and Freyd
categories.

\TODO{reflow}

Further exploring the theoretical underpinnings of SSA, \citet{garbuzov-structural-cfg-2018}
...
derived an equational theory to justify compiler optimizations. However, their operational semantics
is effect-free, which means it cannot justify optimizations involving side effects, such as
eliminating redundant writes.

\TODO{segue nicer}

The work by \citet{goncharov-metalang-21} is particularly relevant as it presents a metalanguage for
\emph{guarded iteration} using guarded Elgot monads, closely resembling our own approach. Their
metalanguage, based on labeled iteration concepts from \citet{geron-iteration-16}, employs
fine-grained call-by-value semantics within a distributive category and a strong guarded
pre-iterative monad for denotational semantics. 

\TODO{properly compare and contrast...}
...
\begin{enumerate}
  ...
  % \item The coinductive resumption monad from \citet{coinductive-resumption-levy-goncharov-19}
  % could be a useful place to look for freeish \isotopessa{} models 
...
\TODO{put future work lore from \citet{goncharov-metalang-21} w.r.t. hybrid lore
\citet{goncharov-hybrid-18} here} 

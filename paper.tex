...
\TODO{clarify the transformation steps}
...
and reasoning about optimizations. By topologically sorting the basic blocks in the CFG according to
the dominance order and inserting brackets based on the dominance tree, we can convert
...
be recovered by removing the inserted \ms{where}-blocks. We give the resulting grammar for
\emph{lexical SSA} in Figure~\ref{fig:lex-ssa}:

\TODO{!NEW! object: regions}

\TODO{!NEW! object: generalized terminators.}

\TODO{C analogy...}

\TODO{where is the distinguished entry block? it's a nameless region!}
...
\TODO{we want to be able to easily and inductively:
\begin{itemize}

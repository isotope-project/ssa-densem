...
\TODO{rework} Similarly, we can interpret \isotopessa regions $\haslb{\Gamma}{r}{\ms{L}}$ as
morphisms $\dnt{\Gamma} \to_\epsilon \dnt{\ms{L}}$. As $\ms{L}$ is a coproduct of its constituent
labels, we can consider the result of $r$ to be returning both a value and a label to jump to, the
latter being encoded by ``landing" in the appropriate branch of the coproduct; this is compared to
an expression, which only returns value information without control-flow information. In particular,
our denotational semantics, given in Figure~\ref{fig:ssa-reg-sem}, is as follows:
\begin{itemize}
  \item \todo{branching to a label}
  \item \todo{letting a variable}

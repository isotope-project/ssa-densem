...
\end{definition}
We note in particular that every cartesian closed category with coproducts is distributive, as
is the Kleisli category of a monad over a CCC with coproducts. 
The last
...
distributor, so that we can soundly access variables in scope in the loop body. Formally, we
...
\begin{itemize}
  ...
  $E$ to wide subcategories of $\mc{C}$, such that $\mc{C}_\top = \mc{C}$, and $(\mc{C}, \mc{C}_\bot)$ is a Freyd category. Furthermore, we ask that each $\mc{C}_\epsilon$ is closed under
...
\end{itemize}
If an \isotopessa{} expression model additionally has an Elgot structure on $\mc{C}$, we will refer to it simply
...
\begin{enumerate}
  ...
  To do so, we will first need some notation to talk about the
  behaviour of the equivalence classes of quotients. Suppose $S$ and $T$ are sets of terms. Then
  we will write $\hasty{\Gamma}{\epsilon}{S}{A}$ to mean that for every $a \in S$, we
  have $\hasty{\Gamma}{\epsilon}{a}{A}$, and similarly we will write $\tmeq{\Gamma}{\epsilon}{S}{T}{A}$ when for all $a \in S$ and $b \in T$, we have $\tmeq{\Gamma}{\epsilon}{a}{b}{A}$. We generalize in the obvious fashion to regions as well as the case when only one side of an equivalence is a set of terms.

  \TODO{replace $[a]$ notation for packing with $\left\langle a \right\rangle$}

...
control-flow graphs and, in particular, interchange data-flow and control-flow, which we will
discuss further in Section~\ref{ssec:data-control}.
...
\subsection{Basic Data-flow and Control-flow Transformations}
...
\TODO{interpretation as state machine optimization}

\TODO{float B\"ohm-Jacopini stuff down}

\TODO{segue, or drag this definition down to Section~\ref{ssec:completeness}}
...
\begin{enumerate}
  ...
  \begin{equation}
    ...
    \end{gathered}
  \end{equation}
  where packing of contexts is defined as in Section~\ref{ssec:data-control}.
  ...
  from Section~\ref{ssec:data-control} is injective for pure contexts $\ms{eff}(\Gamma) = \bot$,
...
\begin{equation}
  ...
  \end{gathered}
\end{equation}
Note the latter equation holds since \TODO{check this...} Hence, it follows that if $\mc{C}$ is an
\isotopessa{} (expression) model, $d_A$ is idempotent, $d_{A + B} = d_A + d_B$, and $d_{A \otimes B}
= A \otimes d_B = d_A \otimes B$, then $\ms{Ide}(\mc{C}, d)$ is an \isotopessa{} (expression) model
as well. It turns out that all these properties hold for $d = \ms{pflush}$, giving us our final
categorical model for TSO weak memory, $\ms{PTSO} = \ms{Ide}(\ms{Set}_{\ms{TSO}}, \ms{pflush})$.

...
\TODO{
\begin{itemize}
  \item Many of the design choices in \citet{leaky-semicolon} are motivated by the desire to have
  $\dnt{(S_1 ; S_2) ; S_3} = \dnt{S_1 ; (S_2 ; S_3)}$.
  \item Other first attempts at compositional weak memory models, like \citet{jagadeesan-pwp-20}, do
  not satisfy even this condition
  \item On the other hand, monadic approaches, like \citet{release-acquire}, get these conditions
  ``for free'' from the monad laws; verifying the Conway iteration axioms is still important, like
  is done in \citet{xia-itrees-20}.
  \item What we've done is pare away the very minimal conditions required if you \emph{don't} have a
  monad; it is unlikely that a non-monadic compositional model not specifically designed to have
  our required properties will do so, but most weak memory models are not even compositional!
\end{itemize}
}
...
\TODO{
  The way we would mechanize this section is with locally nameless... but that time has passed.
  The way we \emph{did} work with things like this is with de-Bruijn indices, having a single hole.
}

...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    dataflow problems \todo{find specific citation for sparse conditional constant propagation; is
    it \citet{wegman-sccp-91}}
  ...
  \begin{itemize}
    ...
    \item Compare to \citet{kelsey-95-cps}; \citet{flanagan-93-anf} says there is no advantage to
    using CPS over direct-style representations such as ANF since it requires additional
    transformations and non-distributive flow analysis is too expensive; so prefer ANF
    \todo{check why, read \citet{flanagan-93-anf}}
    \item This presents a mapping from SSA to ANF. (Not ANF to SSA too?)
  ...
  add dominance-based scoping to CPS rather than add functional-style scoping to SSA \todo{collect
  more notes}
  ...
  \begin{itemize}
    ...
    \item Different to Moggi's equational metalanguage as we have values and computations but no
    computations of computations, i.e. $\ms{T}(\ms{T}\;A)$, much like a Kleisli category vs. a Freyd
    category
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \citet{plotkin-adequacy-01} \todo{read and note}
...
\begin{itemize}
  ...
  \item \todo{maybe not relevant, but worth reading} \emph{On the Operational Theory of the
  CPS-Calculus: Towards a Theoretical Foundation for IRs}
...
\TODO{future work lore from \citet{goncharov-metalang-21} w.r.t. hybrid lore} 

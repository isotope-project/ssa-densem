...
\newcommand{\readmor}[1]{\ms{rcase}(#1)}
...
Given a distributive Freyd category equipped with an Elgot structure, we will often want to consider
the fixpoint of a morphism $f: R \otimes A \to B + A$, where our ``context'' $R$ does not change
between iterations. To do this, we first need to build up a morphism
\begin{equation}
  \readmor{f} := \lmor{f} ; \pi_l \otimes (B + A) ; \delta^{-1} 
...
\begin{equation}
  \rfix{f} := (\readmor{f})^\dagger ; \pi_r : R \otimes A \to B
\end{equation}

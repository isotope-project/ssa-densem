...
expression is well-typed, while our lexical scoping ensures that variables are only visible in
basic blocks which are children of the basic blocks which define them in the dominance tree.
To complete our argument, it hence suffices to show that:
\begin{itemize}
  \item We can convert any well-formed SSA program back into a strict region by inserting
  \ms{where}-blocks
  \item All ways of doing so are equivalent
\end{itemize}
\TODO{sort according to topological order}
\TODO{construct the dominator tree, everyone is well-scoped}
\TODO{insert where accordingly}
\TODO{argument \#1: every toposort is equivalent}
\TODO{argument \#2: 
  all the strict SSA programs are the same as those which strictly reflect the 
  dominator tree. In particular, even if we postulate less relations, i.e. fold wheres,
  we still get an equivalent program
}
\TODO{idea: do by induction on length of CFGs?}

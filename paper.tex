...
\item Next, in Section~\ref{sec:densem}, we give a categorical semantics for this type theory, in
  ...
  We show that any category with this structure is a model of SSA. This establishes that all of the
  equations we give are sound with respect to the categorical structure.
...
\item We also show, in Section~\ref{ssec:completeness}, that syntax quotiented by the equational
  ...
  Theoretically, this leads us to the surprising discovery that SSA is actually a syntactic
  presentation of distributive Elgot categories, thereby establishing a surprising connection
  between compiler IRs and category theory. (In fact, our calculus is also the first syntactic
  presentation of distributive Elgot categories.) 
  
  Practically, it means that we can freely mix equational and semantic reasoning depending on what
  is convenient for the problem at hand: a semantic proof which holds in all \isotopessa{}-models
  guarantees the existence of a corresponding syntactic sequence of rewrites.

\item We proceed in Section~\ref{sec:concrete} to show that this denotational axiomatization is
  useful in practice. We give a model of TSO weak memory based on~\citet{sparky} in
  Section~\ref{ssec:tso}, We also give a family of concrete models based on Brookes-style
  traces~\cite{brookes-full-abstraction-96}, which can be instantiated to support the
  release/acquire model of~\citet{release-acquire}. This demonstrates that it is possible to give
  realistic weak memory models, in a variety of semantic styles, which do not disturb the structure
  of SSA in fundamental ways. Furthermore, our results constitute the first proof that SSA-based
  loop transformations are compatible with weak memory. 
...
%   \TODO{note this in reviewer response for ``soundness of substitution''}
\end{itemize}

% \TODO{mention refinement early on?}
...
\begin{itemize}
  ...
  \item We only support nullary tuples $()$ and binary tuples $(v, v')$ as composite values; $n$-ary
  tuples $(x, y, z)$ can be encoded as nested binary tuples $((x, y), z)$.
  
  \item We allow conditional branches $\ite{o}{\tau}{\tau'}$ to be \emph{nested}.  
  This allows us to emulate switch-statements $\ms{switch}(o) \{c_1 : \tau_1 ,
  ... , c_n : \tau_n\}$ as nested if-statements without introducing spurious basic blocks.
  %
  Likewise, for uniformity, a return-statement may appear in the branch of a conditional branch.
\end{itemize}
...
\begin{figure}[H]
  \begin{center}
    \begin{grammar}
      ...
      <\(\tau\)> ::= \(\ms{br}\;\ell\) \;|\; \(\ms{ret}\;v\) \;|\; $\ite{o}{\tau}{\tau'}$
...
\begin{itemize}
  ...
  blocks.
  \item Converting composite expressions like $a * (i + 1)$ into a sequence of definitions naming
  each subexpression. 
...
Additionally, they require us to adopt more complex scoping rules than simple dominance-based
scoping. For example, in the basic block \ms{loop} in Figure~\ref{fig:fact-ssa}, $i_0$ evaluates to
1 if we came from the entry block and to $i_1$ if we came from \ms{body}. Similarly, $a_0$ evaluates
to either 1 or $a_1$ based on the predecessor block. This does not obey dominance-based scoping,
since $i_0$ and $i_1$ are defined \emph{after} the $\phi$-nodes $i_0$, $a_0$ that reference them,
which seems counterintuitive -- after all, variables are typically used after they are defined. In
fact, since the value of a $\phi$-node is determined by which basic block is our immediate
predecessor, we instead need to use the rule that expressions in $\phi$-node branches with source
$S$ can use any variable $y$ defined at the \emph{end} of $S$. Note that this is a strict superset
of the variables visible for a normal instruction $x$, which can only use variables $y$ which
\emph{dominate} $x$ -- i.e., such that \emph{every} path from the entry block to the definition of
$x$ goes through $y$, rather than only those paths which also go through $S$.
  ...
  \begin{subfigure}[t]{.5\textwidth}
    \begin{align*}
      %\ms{entry}:\quad  
                        & \ms{let}\;n = 10; \\
                        & \ms{br}\;\ms{loop} \\
      \ms{loop}: \quad  & \ms{let}\;i_0 = \phi(\ms{entry}: 1, \ms{body}: i_1); \\
                        & \ms{let}\;a_0 = \phi(\ms{entry}: 1, \ms{body}: a_1); \\
                        & \ms{if}\;i_0 < n\;
                          \{\;\ms{br}\;\ms{body}\;\}\;
                          \ms{else}\;\{\;\ms{ret}\;a_0\;\}; \\
...
We give a formal grammar for basic blocks-with-arguments SSA in Figure~\ref{fig:bba-grammar}
\footnote{ Many variants of SSA do not allow variables to appear alone on the right-hand side of
assignments, such as $x = y; \beta$.  We do not incorporate this restriction, though we could by
normalizing even further and substituting $[y/x]\beta$ instead. }. One of the other changes we make
is replacing conditional branches $\ite{o}{\tau}{\tau'}$ with \emph{case-statements}
$\casestmt{o}{y}{\tau}{z}{\tau'}$. In particular, a case-statement $\ite{o}{\tau}{\tau'}$ may be
desugared into a case-statement on a Boolean value $o : \mb{1} + \mb{1}$.

Note that this grammar no longer needs a separate terminator for returns: we can treat the return
point as a distinguished label (with argument) that a program can jump to. 
...
\begin{itemize}
  \item The entry block strictly dominates all other blocks by definition; thus, the variable $n$ is
  visible in \ms{loop} and \ms{body}.
  ...
  \item \ms{body} does \emph{not} strictly dominate \ms{loop}, since there is a path from the entry
  block to \ms{loop} that does not pass through \ms{body}.
\end{itemize}
  ...
  \begin{subfigure}[t]{.5\textwidth}
    ...
    \begin{align*}
      ...
      \ms{loop}: \quad  & \begingroup \color{red}
                          \ms{let}\;i_0 = \phi(\ms{entry}: 1, \ms{body}: i_1);
                          \endgroup \\
...
% \TODO{
%   \begin{itemize}
%     \item What we want to do is \emph{substitution}: replacing a variable with it's definition
%     \item So fuse values $v$ and operations $o$ to get \emph{expressions}
%     \item But, we also want to represent things like \ms{abs}: so let-bindings and case-expressions
%     \item This adds no expressive power: we can lower in the obvious manner
%     \item Likewise, we fuse $r$ and $\tau$ to allow inlining regions
%     \item Specifically, consider Ye Olde Unconditional Jump. 
%       \begin{equation*}
%         \where{\letstmt{x}{a}{\brb{\ell}{x}}}{\wbranch{\ell}{y}{r}} 
%         \teqv \letstmt{y}{a}{r}
%       \end{equation*}
%       Want this to be unconditionally valid.
%   \end{itemize}
% }

% Our current syntax, however, makes it difficult to reason about \emph{substitution}. In particular,
% replacing a variable with its definition is not generally possible, even if that variable's
% definition is \emph{pure}, since a valid expression $f\;(x, y)$ might become an invalid expression
% $f\;(a + b, 0)$. This means that algebraic rewrites need to be able to look through variable
% definitions, inserting and removing them as necessary: this is may actually be \emph{beneficial} in
% practical implementations (where each node of an expression-tree can correspond one-to-one with an
% instruction), but makes it much more cumbersome to do theoretical work.

% To fix this, instead of stratifying the left-hand-side of our bindings into operations $o$ and
% values $v$, we can instead fuse these into a single syntactic category of \emph{expressions} $a$.
% Conceptually, what we're doing is replacing variables $x$ with arbitrary expressions $a$. This
% clearly adds no expressive power, since we can introduce temporary variable names for each
% sub-expression of $a$ to go back to standard SSA. We can justify allowing let-bindings in
% expressions $a$ in the same way: we get an equivalent program by ``floating'' the let-binding out of
% the expression and into the main lexical SSA program, $\alpha$-renaming as necessary to ensure the
% let-binding binds a fresh variable.

% To be able to effectively combine control-flow rewrites with algebraic rewriting, such as in our
% absolute-value example, it helps to be able to represent conditional control-flow in our expression
% language. We can do this by allowing expressions to contain case-statements, giving us the grammar
% for expressions $a$ in Figure~\ref{fig:ssa-grammar}. The justification that this adds no expressive
% power is slightly more fiddly, and resembles to some degree the transformation into
% continuation-passing style. In particular, given a program of the form
% $$
% \letstmt{x}{\caseexpr{a}{y}{b}{z}{c}}{r}
% $$
% and assuming without loss of generality that $a = t$ is a variable (we can first pull out $a$ into a
% let-binding, essentially converting our expressions to A-normal form) we can obtain an equivalent
% program with the case-expression removed by
% \begin{itemize}
%   \item Introducing a new label corresponding to the rest of the program taking the value of the
%   case-expression as argument, $\ell_{\ms{rest}}(x): r'$, where $r'$ is $r$ (inductively) converted
%   into standard SSA
%   \item Compiling $a$ to SSA and inserting a branch parametrized by the resulting value $t_a$ to
%   either:
%   \begin{itemize}
%     \item A new label $\ell_{\ms{left}}(y)$, which computes $b$, binding its value to $t_b$, and jumps
%     to $\ell_{\ms{rest}}(t_b)$, or
%     \item A new label $\ell_{\ms{right}}(z)$, which computes $c$, binding its value to $t_c$, and
%     jumps to $\ell_{\ms{rest}}(t_c)$
%   \end{itemize}
% \end{itemize}
% We'd like to be able to reason like this without fiddling with temporary labels. More generally,
% we'd like to perform \emph{label-substitution}, in which we replace an unconditional branch to a
% label $\ell$ with a region $r$; this allows us to directly reason about optimizations such as
% jump-threading, inlining, and loop-unrolling. The easiest way to do this is simply to fuse the
% syntactic categories $r, \tau$ of regions and terminators respectively, yielding our final
% \isotopessa{} language, whose grammar is given in Figure~\ref{fig:ssa-grammar}.
...
% We can view this change as allowing a case-statement, and the head of a where-block, to branch to
% anonymous basic blocks. For example, the program
% $$
% \where{\casestmt{e}{x}{s}{y}{t}}{L}
% $$
% is exactly equivalent to the lexical SSA program
% $$
% \where{\brb{\ell_1}(())}
% {
%   \wbranch{\ell_1}{-}{
%     \where{\casestmt{e}{t}{\brb{\ell_2}{t}}{\brb{\ell_3}{y}}{t}}{
%       \wbranch{\ell_2}{x}{s},
%       \wbranch{\ell_3}{y}{t},
%       L
%     }
...
% }
% $$
% It is hence easy to see that this change adds no additional expressive power; however, it makes it a
% lot easier to justify equivalences, like the one we used above, as simple step-by-step derivations:
% \begin{align*}
% \letstmt{x}{\caseexpr{a}{y}{b}{z}{c}}{r} 
% &\teqv \casestmt{a}{y}{\letstmt{x}{b}{r}}{z}{\letstmt{x}{c}{r}} \\
% &\teqv \where{\casestmt{a}{y}{\brb{\ell}{y}}{z}{\brb{\ell}{z}}}{\wbranch{\ell}{x}{r}}
% \end{align*}
...
\begin{itemize}
  ...
  \item Pattern matching on sum types, typed by \brle{case}. Operationally, we interpret this as
  executing the expression $e$, and then, if $e$ is a left injection $\iota_l\;x$, executing $r$
  with its value ($x$), otherwise executing $s$.
...
\begin{figure}
  \begin{gather*}
    ...
    \prftree[r]{\rle{cfg}}
      {\haslb{\Gamma}{r}{\ms{L}, (\lhyp{\ell_i}{A_i},)_{i \in I}}}
      {\forall i \in I. \haslb{\Gamma, \bhyp{x_i}{A_i}}{t_i}{\ms{L}, (\lhyp{\ell_j}{A_j},)_{j \in I}}}
      {\haslb{\Gamma}{\where{r}{(\wbranch{\ell_i}{x_i}{t_i},)_{i \in I}}}{\ms{L}}}
  \end{gather*}
...
something typechecks in a context $\Delta$, and $\Gamma$ contains all the variables of $\Delta$
(written $\Gamma \leq \Delta$, pronounced ``$\Gamma$ \emph{weakens} $\Delta$''), then it should
typecheck in the context $\Gamma$ as well. Here, the context with fewer variables appears on the
\emph{right}, allowing us to compose typing judgements likeso
\begin{equation*}
  \Gamma \leq \Delta \implies \haslb{\Delta}{r}{\ms{L}} \implies \haslb{\Gamma}{r}{\ms{L}}
\end{equation*}
As our theory has two types of context; we'd also like to define \emph{label-weakening} $\ms{L} \leq
\ms{K}$, which we should be able to apply in the same manner:
\begin{equation*}
  \haslb{\Gamma}{r}{\ms{L}} \implies \ms{L} \leq \ms{K} \implies \haslb{\Gamma}{r}{\ms{K}}
\end{equation*}
If a region $r$ typechecks with exit labels $\ms{L}$, and $\ms{K}$ contains every label in $\ms{L}$,
then $r$ should obviously also typecheck in $\ms{K}$. It follows that in the judgement $\ms{L} \leq
\ms{K}$ the context with fewer labels appears on the \emph{left}-hand side of the judgement: this
corresponds precisely to the fact that label-weakening (injection into a coproduct) is semantically
dual to variable-weakening (projection from a product), and hence the order is flipped.

We give the (standard) formal rules for weakening $\Gamma \leq \Delta$, and their duals, in the
first part of Figure \ref{fig:ssa-meta-rules}.
\begin{itemize}
  \item \brle{wk-nil} and \brle{lwk-nil} say that the empty (label) context weakens itself,
  \item \brle{wk-skip} says that if $\Gamma$ weakens $\Delta$, then $\Gamma, x: A$ also weakens
  $\Delta$ for arbitrary (fresh) $x$. Dually, \brle{lwk-skip} says that if $\ms{L}$ weakens
  $\ms{K}$, then $\ms{L}$ also weakens $\ms{K}, \lhyp{\ell}{A}$ for arbitrary (fresh) $\ell$.
  \item \brle{wk-cons} says that if $\Gamma$ weakens $\Delta$, then $\Gamma$ with $\bhyp{x}{A}$
  added weakens $\Delta, \bhyp{x}{A}$. Likewise, \brle{lwk-cons} says that if $\ms{L}$ weakens
  $\ms{K}$, then $\ms{L}$ with $\lhyp{\ell}{A}$ added weakens $\ms{K}, \lhyp{\ell}{A}$.
\end{itemize}
It is easy to see that (label) weakening defined in this manner induces a partial order on (label)
contexts. Our weakening lemma is then as follows:
\begin{lemma}[Weakening]
  Given $\Gamma \leq \Delta$, $\epsilon \leq \epsilon'$, we have that:
  \begin{enumerate}[label=(\alph*)]
    \item If $\hasty{\Delta}{\epsilon}{a}{A}$, then $\hasty{\Gamma}{\epsilon'}{a}{A}$
    \item If $\ms{L} \leq \ms{K}$ and $\haslb{\Delta}{r}{\ms{L}}$, then $\haslb{\Gamma}{r}{\ms{K}}$
    \item If $\issubst{\gamma}{\Delta}{\Xi}$, then $\issubst{\gamma}{\Gamma}{\Xi}$
    \item If $\lbsubst{\Delta}{\sigma}{\ms{L}}{\ms{K}}$, then
...
The validity of variable weakening hinges on the fact that all the variables in $\Delta$ are also
available with the same type in $\Gamma$, i.e., if $\hasty{\Delta}{\epsilon}{x}{A} \implies
\hasty{\Gamma}{\epsilon}{x}{A}$, then anything which can be typed in $\Delta$ can be typed in
$\Gamma$. So while weakening on \emph{terms} is just the identity, weakening on \emph{derivations}
is essentially replacing ``variables from $\Delta$'' with ``variables from $\Gamma$.'' Since none of
our typing rules, other than $\ms{var}$, make use of variable names, we might ask whether we can
repeat essentially the same reasoning to reason about the well-typedness of replacing variables in
$\Gamma$ with arbitrary pure expressions of the same type (i.e., perform a substitution).  
%
% More generally, we might instead ask whether things still type-check if every variable
% $\hasty{\Delta}{\epsilon}{x}{A}$ in $\Delta$ is replaced with an (arbitrary) \emph{pure} term
% $\hasty{\Gamma}{\bot}{\gamma_x}{A}$ which is well-typed in $\Gamma$, whether things still
% type-check.
An assignment of such variables $\gamma : x \mapsto \gamma_x$ is called a
\emph{substitution}, which we can type with the judgement $\issubst{\gamma}{\Gamma}{\Delta}$ as per
the rules given in Figure \ref{fig:ssa-meta-rules}. In particular,
\begin{itemize}
...
Figure \ref{fig:ssa-subst-def} in the appendix). Substitution satisfies the
...
\begin{lemma}[Substitution]
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \item $\issubst{\gamma_2}{\Delta}{\Xi} \implies \issubst{[\gamma]\gamma_2}{\Gamma}{\Xi}$
...
% ; sometimes, for convenience, we may even omit the
% $\rupg{\cdot}, \lupg{\cdot}$ operators entirely, due to the fact that for any $\gamma$, we have
% $[\gamma]a = [\lupg{\Gamma}_\Xi]a = [\rupg{\gamma}_\Xi]a$. 
...
% $\sigma$, we have $[\sigma]r = [\rupg{\sigma}_{\ms{K}}]r = [\lupg{\sigma}_{\ms{K}}]r$. 
...
\begin{figure}
  \begin{gather*}
    ...
    \\
    \prftree[r]{\rle{cfg}}
      {\lbeq{\Gamma}{r}{r'}{\ms{L}, (\lhyp{\ell_i}{A_i},)_{i \in I}}}
      {\forall i \in I. \lbeq{\Gamma, \bhyp{x_i}{A_i}}{t_i}{t_i'}
        {\ms{L}, (\lhyp{\ell_j}{A_j},)_{j \in I}}}
      {\lbeq{\Gamma}
        {\where{r}{(\wbranch{\ell_i}{x_i: A_i}{t_i},)_{i \in I}}}
        {\where{r'}{(\wbranch{\ell_i}{x_i: A_i}{t_i'},)_{i \in I}}}
        {\ms{L}}
      }
    \\
...
\begin{figure}
  \begin{gather*}
    \prftree[r]{\rle{let$_1$-$\beta$}}
      {\hasty{\Gamma}{\bot}{a}{A}}
      {\haslb{\Gamma, \bhyp{x}{A}}{r}{\ms{L}}}
      {\lbeq{\Gamma}{\letstmt{x}{a}{r}}{[a/x]r}{\ms{L}}}
    \\
      \prftree[r]{\rle{let$_1$-op}}
      {\isop{f}{A}{B}{\epsilon}}
      {\hasty{\Gamma}{\epsilon}{a}{A}}
...
\begin{equation}
  \prftree[r]{\rle{cfgs}}
    {\forall i \in I. \haslb{\Gamma, \bhyp{x_i}{A_i}}{t_i}{\ms{L}, (\lhyp{\ell_j}{A_j},)_{j \in I}}}
    {\lbsubst{\Gamma}
      {\cfgsubst{(\wbranch{\ell_i}{x_i}{t_i},)_{i \in I}}}{\ms{L}, (\lhyp{\ell_j}{A_j},)_{j \in I}}{\ms{L}}}
\end{equation}
...
follows:
\begin{multline}
  \where{\brb{\ell}{x}}{\wbranch{\ell}{y}
    {\letstmt{x'}{y + 1}{\ms{if}\;p\;3x'\;\{\ms{ret}\;3x'\}\;\ms{else}\;\{\brb{\ell}{x'}\}}}}
  \\ \teqv
  \where{\letstmt{y}{3x}{\brb{\kappa}{y}}}{\wbranch{\kappa}{y}
    {\letstmt{y'}{y + 3}{\ms{if}\;p\;y'\;\{\ms{ret}\;y'\}\;\ms{else}\;\{\brb{\kappa}{y'}\}}}}
  \label{eqn:loop-comm-ssa}
\end{multline}
...
\end{gathered}
\label{eqn:uni-variant}
\end{equation}
Going back to our concrete example from Equation~\ref{eqn:loop-comm-ssa}, if we first substitute the
let-binding $y = 3x$ on the RHS, we get
\begin{multline}
  \where{\brb{\ell}{x}}{\wbranch{\ell}{y}
    {\letstmt{x'}{y + 1}{\ms{if}\;p\;3x'\;\{\ms{ret}\;3x'\}\;\ms{else}\;\{\brb{\ell}{x'}\}}}}
  \\ \teqv
  \where{\brb{\kappa}{3x}}{\wbranch{\kappa}{y}
    {\letstmt{y'}{y + 3}{\ms{if}\;p\;y'\;\{\ms{ret}\;y'\}\;\ms{else}\;\{\brb{\kappa}{y'}\}}}}
  \label{eqn:loop-comm-red}
\end{multline}
Now, instantiate $\brle{uni}'$ (Equation~\ref{eqn:uni-variant}) by taking:
\begin{itemize}
  \item
    $s = \letexpr{y'}{y + 3}{\ms{if}\;p\;y'\;\{\ms{ret}\;y'\}\;\ms{else}\;\{\brb{\kappa}{y'}\}}$ 
    to be the loop body on the RHS
  \item $e = 3x$
  \item $r = \brb{\ell}{x}$
  \item $t = \letstmt{x'}{y + 1}{\ms{if}\;p\;3x'\;\{\ms{ret}\;3x'\}\;\ms{else}\;\{\brb{\ell}{x'}\}}$
  to be the loop body on the LHS
\end{itemize}
It's easy to see that $(\where{([\ell(x) \mapsto \brb{\kappa}{e}]r)}{\wbranch{\kappa}{y}{s}})$ and
$(\where{r}{t})$ are syntactically equal to the \emph{RHS} and \emph{LHS} of our desired result
(Equation~\ref{eqn:loop-comm-red}). So, it suffices to verify that
\begin{align*}
  \Gamma, \bhyp{x}{A} \vdash 
  & [e/y]s
  \teqv \letexpr{y'}{3x + 3}{\ms{if}\;p\;y'\;\{\ms{ret}\;y'\}\;\ms{else}\;\{\brb{\kappa}{y'}\}} \\
  & \teqv \letexpr{y'}{3(x + 1)}{\ms{if}\;p\;y'\;\{\ms{ret}\;y'\}\;\ms{else}\;\{\brb{\kappa}{y'}\}} \\
  & \teqv \letexpr{x'}{x + 1}{
      \letexpr{y'}{3x'}{\ms{if}\;p\;y'\;\{\ms{ret}\;y'\}\;\ms{else}\;\{\brb{\kappa}{y'}\}}} \\
  & \teqv \letexpr{x'}{x + 1}{
      \ms{if}\;p\;3x'\;\{\ms{ret}\;3x'\}\;\ms{else}\;\{\brb{\kappa}{3x'}\}} \\
  & \teqv [\ell(x) \mapsto \brb{\kappa}{e}]t
\end{align*}
as desired.
%
The reason why we require $e$ to be \emph{pure} in the uniformity rule is that impure expressions do
...
\begin{align}
  ...
  &\not\teqv
  \ms{loop} \{ x = x + 1 ;  \ms{if}\;x = y\;\{\ms{hi} ; \ms{ret}\;y\} \} ; \ms{hi}
\end{align}
...
\begin{figure}
  \begin{gather*}
      \prftree[r]{\rle{cfg-$\beta_1$}}
        ...
        {\forall i \in I. \haslb{\Gamma, \bhyp{x_i}{A_i}}{t_i}{\ms{L}, (\lhyp{\ell_j}{A_j},)_{j \in I}}}
        {\lbeq{\Gamma}
          {\where{\brb{\ell_k}{a}}{(\wbranch{\ell_i}{x_i}{t_i},)_{i \in I}}}
          {\where{(\letstmt{x_k}{a}{t_k})}{(\wbranch{\ell_i}{x_i}{t_i},)_{i \in I}}}
          {\ms{L}}}
      \\
      \prftree[r]{\rle{cfg-$\beta_2$}}
        ...
        {\forall i \in I. \haslb{\Gamma, \bhyp{x_i}{A_i}}{t_i}{\ms{L}, (\lhyp{\ell_j}{A_j},)_{j \in I}}}
        {\ms{L}\;\kappa = B}
        {\kappa \notin \{\ell_i \mid i \in I\}}
        {\lbeq{\Gamma}
          {\where{\brb{\kappa}{b}}{(\wbranch{\ell_i}{x_i}{t_i},)_{i \in I}}}
          {\brb{\kappa}{b}}
          {\ms{L}}}
      \\
        ...
        {
          \lbeq{\Gamma}
            {\where{r}{(\wbranch{\ell_i}{x_i}{t_i},)_{i \in I}}}
            {[\cfgsubst{(\wbranch{\ell_i}{x_i}{t_i},)_{i \in I}}]r}
            {\ms{L}}
        }
      \\
      ...
      \prftree[r]{\rle{dinat}}
        {\prfStackPremises{
          ...
          \lbsubst{\Gamma}{\sigma}{(\lhyp{\ell_i}{A_i},)_{i \in I}}{(\lhyp{\kappa_j}{B_j},)_{j \in J}}
        }
        }{
          \forall j \in J. \haslb{\Gamma, \bhyp{x_j}{B_j}}{t_j}{\ms{L}, (\lhyp{\ell_i}{A_i},)_{i \in I}}
        }{
          \prfStackPremises
            {\Gamma \vdash \where{([\lupg{\sigma}]r)}{(\wbranch{\kappa_j}{x_j}{[\lupg{\sigma}]t_j},)_{j \in J}}}
            {\hspace{8em} \teqv \where{r}
              {(\wbranch{\ell_i}{x_i}
                {[\lupg{(\kappa_j(x_j) \mapsto t_j,)_{j \in J}}](\sigma\;\ell_i\;x_i)},)_{i \in I}}}
        }
  \end{gather*}
  \Description{}
...
\end{figure}
The derivable rule \brle{uni'} (Equation~\ref{eqn:uni-variant}) illuminates a very important
potential use for uniformity; namely, formalizing rewrites like those in
Figure~\ref{fig:fact-dinat-rewrites}. In particular, consider a program of the form 
\begin{equation*}
...
where 
\begin{itemize}
  \item $\haslb{\Gamma}{r}{\ms{L}, \ell(A)}$
  \item $\haslb{\Gamma, y : B}{s}{\ms{L}, \ell(A)}$
  \item $\hasty{\Gamma, \bhyp{x}{A}}{\bot}{e}{B}$ is pure
\end{itemize} 
Then we have that
\begin{multline*}
...
Figure~\ref{fig:dinat-struct-cfg}. Unlike in the case of uniformity, however, this is true even when
the program fragment $P$ is \emph{impure}, since, unlike in the case of general uniformity, we do
not commute $P$ over an infinite number of iterations. Our final rewriting rule, \brle{dinat},
...
\begin{equation}
  \prftree[r]{\rle{cfg-fuse}}
    {\haslb{\Gamma}{r}{\ms{L}, (\lhyp{\ell_i}{A_i},)_{i \in I}, (\lhyp{\kappa_j}{B_j},)_{j \in I}}}
    {
      \prfStackPremises{
        \forall i \in I. \haslb{\Gamma, \bhyp{x_i}{A_i}}{t_i}{
        \ms{L}, (\lhyp{\ell_j}{A_j},)_{j \in I}}
      }{
        \forall i \in I. \haslb{\Gamma, \bhyp{y_i}{B_i}}{s_i}{
        \ms{L}, (\lhyp{\ell_j}{A_j},)_{j \in J}, (\lhyp{\kappa_k}{B_k},)_{k \in K}}
      }
    }
    {
      \prfStackPremises{
        \Gamma \vdash 
          \where{(\where{r}{(\wbranch{\kappa_k}{y_k}{s_k}),})_{k \in K}}
            {(\wbranch{\ell_i}{x_i}{t_i}),)_{i \in I}}
      }{
        \hspace{8em}
        \teqv \where{r}{(\wbranch{\kappa_k}{y_k}{s_k},)_{k \in K}, 
                (\wbranch{\ell_i}{x_i}{t_i}),)_{i \in I}}
        \rhd \ms{L}
      }
    }
...
\end{equation}
Rather than directly giving derivation trees for such auxilliary rules, it is more convenient to
give a denotational proof. However, the completeness of our equational theory (proved in
Section~\ref{ssec:completeness}) means that the semantic equality implies the existence of the
requisite derivation tree. A proof can be found in Lemma~\ref{lem:where-fusion} in the appendix.
...
% \TODO{this is not a point against our equational theory \emph{per-se}: we can use the semantics to
% prove \emph{new} rules sound, adding them to the set of valid principles, rather than having more
% rules to check at the beginning. This is rather a point \emph{for} completeness, since it lets us
% \emph{mix} equational and semantic reasoning as convenient. Link back to discussion about this.} 
...
\begin{equation}
  \prftree[r]{\rle{perm-cfg}}
    ...
    {\lbeq{\Gamma}
      {\where{r}{(\wbranch{\ell_i}{x_i}{t_i},)_{i \in I}}}
      {\where{r}{(\wbranch{\ell_{\sigma_i}}{x_{\sigma_i}}{t_{\sigma_i}},)_{i \in I}}}{\ms{L}}}
\end{equation}
...
\begin{lemma}[Weakening (Rewriting)]
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \item $\lbeq{\Delta}{r}{r'}{\ms{L}} \implies \lbeq{\Gamma}{r}{r'}{\ms{K}}$
  \end{enumerate}
\end{lemma}
\begin{proof}
  These are formalized as:
...
Figure~\ref{fig:ssa-subst-equiv} in the interests of explicitness.
...
\begin{equation}
  \begin{aligned}
    \letanf{x}{a}{r} &= (\letstmt{x}{a}{r}) \hspace{8em} \text{if}\;\isopex{a} \\
    ...
    \letanf{x}{\caseexpr{e}{y}{a}{z}{b}}{r} &= 
      \letanf{w}{e}{
        \where{\casestmt{w \\ & \qquad}
          {y}{\letanf{x}{a}{\brb{\ell}{x}}}
          {z}{\letanf{x}{b}{\brb{\ell}{x}}} \\ & \qquad}
        {\wbranch{\ell}{x}{\toanf{r}}}
      } 
    \\
    \letanf{x}{\labort{e}}{r} &= \letanf{y}{e}{(\letstmt{x}{\labort{y}}{r})}
  \end{aligned}
\end{equation}
We note that, to get to ANF, we don't actually need the new label, and can instead replace each
branch with $\letanf{x}{a}{r}$; introducing the label is only necessary to guarantee that the
transformation to ANF is \emph{linear-space}.
%
The specification of the functions we just defined is given by the following lemma:
...
We would now like to define a syntactic linear-space transformation from regions $r$ to equivalent
lexical SSA regions $\tossa{r}$. We do this by giving a pair of mutually syntactic transformations
$\tossa{r}$ converting a region to SSA, and $\ssawhere{r}{L}$ from ANF regions $r$ \emph{targeting}
labels in $L$ to lexical SSA.
\begin{equation}
  \begin{aligned}
...
We now come to the final part of our argument: that lexical SSA is equivalent to standard SSA, where
we take the basic-blocks-with-arguments dialect described in Figure~\ref{fig:bba-grammar} as
...
\begin{equation}
  \begin{aligned}
    ...
    \toentry{\where{\tau}{(\wbranch{\ell_i}{x_i}{t_i},)_i}} &= \tau
  \end{aligned}
\end{equation}
\begin{equation}
  \begin{aligned}
    ...
    \todom{\where{\tau}{(\wbranch{\ell_i}{x_i}{t_i},)_i}} &= [\wbranch{\ell_i}{x_i}{t_i},]_i
  \end{aligned}
\end{equation}
We can similarly define a function to construct a lexical SSA program from a basic block $\beta$ and
a set of children as follows:
\begin{equation}
  \begin{aligned}
  \adddom{\letstmt{x}{a}{\beta}}{L} &= \letstmt{x}{a}{\adddom{\beta}}{L} \\
  \adddom{\letstmt{(x, y)}{a}{\beta}}{L} &= \letstmt{(x, y)}{a}{\adddom{\beta}}{L} \\
  \adddom{\tau}{L} &= \where{\tau}{L}
  \end{aligned}
\end{equation}
...
%  $\wbranch{r}{(\wbranch{\ell_i}{x_i}{t_i},)_i} \teqv \adddom{r}{(\wbranch{\ell_i}{x_i}{t_i},)_i}$
  ...
  \end{equation}
\end{itemize}
We may hence define a function $\tocfg{\cdot}$ from lexical SSA programs $r$ to control-flow graphs
$G$ as follows:
\begin{equation}
...
\end{equation}
At the level of program text, all this function is doing is ``removing \ms{where}-blocks;'' so we'll
call the result \emph{reased}. An SSA program is \emph{well-formed} if:
\begin{itemize}
...
labels guarantees are dominated by $\beta$. 

On the other hand, we may give an algorithm to convert any well-formed SSA program $G$ into a
well-typed lexical SSA program $r = \toreg{G}$ as follows:
\begin{enumerate}
...
\end{enumerate}
We will write $G \simeq G'$ to mean ``$G$ is a permutation of $G'$'' (that is, $G$ and $G'$ have the
same entry block, but the other labels may be reordered). It is easy to see that this algorithm
yields a lexical SSA program which erases to a permutation of $G$, i.e., $\tocfg{\toreg{G}} \simeq
G$, as desired. To complete our argument, it hence suffices to show that, given lexical SSA regions
$\haslb{\Gamma}{r}{\ms{L}}$, $\haslb{\Gamma}{r'}{\ms{L}}$, such that $\tocfg{r} \simeq \tocfg{r'}$,
we have that $\lbeq{\Gamma}{r}{r'}{\ms{L}}$. We break this down into two lemmas:
\begin{lemma}[name=Permutation Invariance, restate=cfgperminvar]
  If $G \simeq G'$ and $\haslb{\Gamma}{\toreg{G}}{\ms{L}}$, then
    $\haslb{\Gamma}{\toreg{G'}}{\ms{L}}$ and $\lbeq{\Gamma}{\toreg{G}}{\toreg{G'}}{\ms{L}}$
\end{lemma}
\begin{proof}
  See Appendix~\ref{proof:cfg-perm-invar}
\end{proof}
\begin{lemma}[name=CFG Conversion, restate=cfgconversion]
  Given a lexical SSA region $r$ (i.e., assuming $\isssa{r}$) s.t. $\haslb{\Gamma}{r}{\ms{L}}$, we
  have that $\lbeq{\Gamma}{r}{\toreg{\tocfg{r}}}{\ms{L}}$.
\end{lemma}
\begin{proof}
  See Appendix~\ref{proof:cfg-conversion}
\end{proof}

It follows that, given lexical SSA regions $\haslb{\Gamma}{r}{\ms{L}}$ and
$\haslb{\Gamma}{r'}{\ms{L}}$, if $\tocfg{r} \simeq \tocfg{r'}$, we have that
$\lbeq{\Gamma}{r}{r'}{\ms{L}}$, as desired, since in particular
\begin{equation}
...
Now that we have given an equational theory for SSA, we would like to build a denotational semantics
for SSA: a mapping from well-typed programs $\haslb{\Gamma}{r}{\ms{L}}$ to mathematical objects
$\dnt{\haslb{\Gamma}{r}{\ms{L}}}$. This naturally gives us another way to reason about whether two
programs ``$r$, $r'$'' are ``the same:'' we can ask whether
$
\dnt{\haslb{\Gamma}{r}{\ms{L}}} = \dnt{\haslb{\Gamma}{r'}{\ms{L}}}
$.
Our goal is to build a denotational semantics which is both \emph{sound} and \emph{complete}; that
is, such that
$$
\lbeq{\Gamma}{r}{r'}{\ms{L}} \iff
\dnt{\haslb{\Gamma}{r}{\ms{L}}} = \dnt{\haslb{\Gamma}{r'}{\ms{L}}}
$$
More specifically, we'll begin by defining a notion of a \emph{model} $\mc{M}$ of SSA, and then
define the denotational semantics w.r.t. a model $\dnt{\haslb{\Gamma}{r}{\ms{L}}}_{\mc{M}}$. We'll
then prove that, for \emph{any} model of SSA, if $\lbeq{\Gamma}{r}{r'}{\ms{L}}$, then
$$
\dnt{\haslb{\Gamma}{r}{\ms{L}}}_{\mc{M}} = \dnt{\haslb{\Gamma}{r'}{\ms{L}}}_{\mc{M}}
$$
In other words, our equational theory is \emph{sound}, and so never allows us to prove an equation
which does not in fact hold. We'll then give an model $\mc{M}$ such that
$$
\dnt{\haslb{\Gamma}{r}{\ms{L}}}_{\mc{M}} = \dnt{\haslb{\Gamma}{r'}{\ms{L}}}_{\mc{M}} \implies
\lbeq{\Gamma}{r}{r'}{\ms{L}}
$$
showing that our equational theory is also \emph{complete}: if an equation holds in an
\emph{arbitrary} model of SSA, it must in fact be derivable via our theory.

We'd further like our denotational semantics to be \emph{compositional}: the denotation of a program
should be a fixed function of the denotations of its parts. To do this, we will give SSA a
\emph{categorical} semantics, interpreting (label) contexts as objects and programs as morphisms
between them in some target category $\mc{C}$. Different program structures, such as branching and
sequential composition, will correspond more-or-less directly to structures in our target category.

Before we begin, we fix some notational conventions:
\begin{itemize}
  \item We denote the \emph{composition} of two morphisms $f : A \to B$ and $g : B \to C$ as $f ; g
  : A \to C$
  \item We denote the coproduct of a morphism $f: B \to C$ with the identity $\ms{id}_A$ as $f + A:
  B + A \to C + A$ or $A + f: A + B \to A + C$.
\end{itemize}
...
$$
Instead, we will only require that our category be \emph{premonoidal}, as introduced by
\citet{power-97-premonoidal}: ``monoidal, without sliding.'' Indeed, the Kleisli category of a
strong monad on a CCC is not always monoidal, as demonstrated by the writer monad on $\ms{Set}$
(which exposes $\ms{print}$), but \emph{is} always premonoidal. We define a premonoidal category as
follows:
...
If we generalize this structure to an arbitrary premonoidal category, we get the notion of a
\emph{Freyd category}, as introduced in \citet{levy-03-environment}:
...
\begin{definition}[Freyd category]
  ...
  \begin{itemize}
    ...
    \item $I$ is a terminal object in $\mc{C}_\bot$. In particular, this implies the terminal
    morphisms $!_A: A \to I$ are pure.
  ...
  \end{itemize}
  Where $\mc{C}$ is clear from context, we will write $A \to_\bot B$ to denote a pure morphism
  $\mc{C}_\bot(A, B)$.
  ...
  \begin{itemize}
    ...
    \item For each $A$, there exists a pure morphism $\dmor{A} : A \to A \otimes A$ forming a
          comonoid with the (pure) terminal morphism $\tmor{A} : A \to I$, i.e: $\dmor{A} ; \tmor{A}
...
This gives us enough machinery to express any \emph{acyclic} control-flow graph, however, we still
have no way to model loops. What we would like is to equip our category with an \emph{iteration
operator} taking a morphism $f: A \to B + A$, representing a ``loop'' which, given input $A$, either
produces a result $B$ or continues to another iteration with a new $A$, to its \emph{fixpoint}
$f^\dagger: A \to B$. Naturally, we require that this operator satisfies various properties, and in
particular is \emph{strong}, that is, compatible with the distributor. Formally, we define a
\emph{(strong) Conway iteration operator} as follows:
...
(dashed red) ``state'' wire which all impure morphisms require as an input and output, as in
...
representing a pure morphism will simply have a dashed red ``stripe'' on the side. This gives us a
...
\begin{lemma}[name=(Label) Weakening, restate=weakeninglem]
  ...
  \begin{enumerate}[label=(\alph*)]
    \item For all $\Gamma \leq \Delta$, $\dnt{\Gamma \leq \Delta} = \dnt{\Gamma \leq
    \Gamma'};\dnt{\Gamma' \leq \Delta}$
      \label{itm:varwk}
    \item For all $\ms{L} \leq \ms{K}$, $\dnt{\ms{L}' \leq \ms{K}} = \dnt{\ms{L}' \leq
    \ms{L}};\dnt{\ms{L} \leq \ms{K}}$
      \label{itm:lbwk}
    ...
    \item $\dnt{\haslb{\Gamma}{r}{\ms{L}}}
      ...
      ; \dnt{\ms{L}' \leq \ms{L}}$
      \label{itm:regwk}
    \item For all $\issubst{\gamma}{\Gamma}{\Delta}$, $\dnt{\issubst{\gamma}{\Gamma}{\Delta}} =
      \dnt{\Gamma \leq \Gamma'};\dnt{\issubst{\gamma}{\Gamma'}{\Delta}}$
      \label{itm:substwk}
    \item For all $\lbsubst{\Gamma}{\sigma}{\ms{L}}{\ms{K}}$, $\dnt{\lbsubst{\Gamma}{\sigma}{\ms{L}}{\ms{K}}}
...
\begin{itemize}
  \item Objects $|\ms{Th}^\otimes(\Gamma)|$ are types $A, B, C$
...
\begin{itemize}
  ...
  \item Identity $\haslb{\Gamma, \bhyp{\invar}{A}}{\ms{ret}\;\invar}{\ms{L}, \outlb(A)}$ where
    $\ms{ret}\;a := \brb{\outlb}{a}$
...
\end{equation}
It follows that every strong Elgot monad over a CCC with all coproducts is an \isotopessa{} model
(since, in particular, every CCC with coproducts is distributive). This allows us to very quickly
amass a large collection of \isotopessa{} models from the literature on strong Elgot monads (see
e.g. \cite{goncharov-18-guarded-traced}, \cite{goncharov-21-uniform-elgot}).

Probably the simplest example of an \isotopessa{} model is given by the
\emph{option monad} $\ms{Option}\;A = A + \mb{1}$
\footnote{
  In a constructive setting, we may equivalently use the partial values monad 
  $\ms{Partial}\;A := \Sigma \phi : \ms{Prop} . \phi \to A$.
},
with fixpoint operation
$$
...
\isotopessa{} models, at least for relatively simple side-effects. The resulting family of models is
somewhat similar to \emph{interaction trees}, which were introduced by \citet{xia-20-itrees} to give
convenient denotational semantics to imperative programs: these also form an Elgot monad. Because they
form an Elgot monad, they immediately form a model of \isotopessa{}. Similar techniques can be used
to tackle much more complex side-effects; to demonstrate this, we will show how the model of TSO weak
given in \citet{sparky} can be adapted to our framework.

\subsection{TSO weak memory}

\label{ssec:tso}

Rather than a sequential trace consisting of a stream of events (here, \emph{actions}) $a_1, a_2,
a_3, ...$, \citet{sparky} model concurrency by considering a \emph{partially ordered multiset}, or
\emph{pomset}, of actions. The incomparable actions in our pomset are then precisely those which are
executed in parallel. More formally, we define:
...
For simplicity, we will consider an infinite set of named locations \(x, y, z \in \ms{Loc}\) which
are subject to concurrent modification by all threads via TSO reads, writes, and fences. In
particular, we define a \emph{program order pomset} to be a pomset with \(\mc{A}_{\ms{PO}} =
\mc{A}_w \cup \mc{A}_r \cup \{\delta\}\), where \(\mc{A}_r\) consists of \textit{reads} of the form
\(x = v\) and \(\mc{A}_w\) consists of \textit{writes} of the form \(x := v\) for locations \(x\)
and values \(v \in \ms{Word}\). 

We may now define the \emph{program order monad} \(\ms{PO}\;A = \ms{Traces}\;\Sigma\;A\), where
\(\Sigma\) is taken as a stream action on finite program order pomsets. This yields an \isotopessa{}
model with support for concurrent read and write operations \(\ms{read}_x:
...
Weak memory is very complicated, and it has been very difficult to build models which  satisfy
even ``obvious'' properties. For example, the model of \citet{jagadeesan-pwp-20} fails to even
satisfy the associativity of sequencing (i.e., $\dnt{(S_1 ; S_2) ; S_3} = \dnt{S_1 ; (S_2 ; S_3)}$),
and it took a nontrivial effort by \citet{leaky-semicolon} to establish this property. However,
properties like distributivity
\begin{equation*}
  \dnt{S ; \ite{b}{T_1}{T_2}} = \dnt{\ite{b}{S ; T_1}{S ; T_2}}
\end{equation*}
do not hold, and there is often no semantics of 
loops at all. This makes them impossible to apply to SSA without extensive
adaptation.

One of the easiest ways to build a model with ``nice'' properties is to make the model
monadic; indeed, all the models presented in the above section are an example of this technique.
This handles sequencing and distributivity ``for free'' via the monad laws and the fact that monads
always preserve coproducts; however, this still requires us to verify the Conway iteration axioms to
support loops, as is done in \citet{xia-itrees-20}. Our work identifies the minimal
conditions required to have a compositional model of general \emph{first-order} side effects
compatible with the standard interpretation of SSA; the higher-order analogue is precisely a
(complete) Elgot monad \cite{adamek-elgot-11}, as presented in \citet{goncharov-unguarded-18}.

\section{Discussion and Related Work}

\subsection{Lean Formalization}

We have formalized many of the lemmas and theorems presented in this paper in the Lean 4 proof
assistant; as of now, our main formalization, \texttt{debruijn-ssa}
(\url{https://github.com/imbrem/debruijn-ssa}), weighs about ~29 kloc. While our paper uses named
variables for simplicity, all our rules and syntax are formalized using de-Bruijn indices. Each
formalized theorem is tagged as such in the respective proof. It was also necessary to fork
Mathlib's monoidal category implementation to support Freyd categories, distributivity, and Elgot
structure: the resulting library, \texttt{discretion} (\url{https://github.com/imbrem/discretion}),
weighs about ~27 kloc. We are currently trying to upstream support for premonoidal categories into
Mathlib.
...
LLVM IR itself as part of the Vellvm project; \citet{li-20-kllvm} goes further and gives an
operational semantics for LLVM in the (sequentially consistent) multithreaded setting.
...
iteration structure of a program, which they use to better understand the loop-closing $\phi$-nodes
found both in the gated SSA representation as well as practical compilers such as GCC.
\citet{xia-20-itrees} describe a toolkit for building denotational semantics for imperative
languages, including the standard IMP language and a simple assembly language, using the
\emph{interaction tree monad} and \emph{continuation tree monad}, which they show to satisfy the
Elgot axioms. Recently, \citet{chappe-25-ctrees} introduced \emph{choice trees} (CTrees), which
extend ITrees to support weak memory concurrency, and use this to model a subset of LLVM IR.
...
naively lowered back to RTL for the rest of CompCert's machinery to work on. \citet{demange-ssa-15}
build on this work by providing realistic, verified implementations of Sparse Conditional Constant
Propagation (SCCP) \cite{wegman-sccp-91}, Common Subexpression Elimination (CSE), and Global Value
Numbering (GVN) \cite{rosen-gvn-1988} within a general framework of flow-insensitive static analysis
for CompCertSSA.
...
Most other formalizations oF SSA have given \emph{specific} semantics for it, whether operational or
denotational. One of the features which makes our work distinctive is that we give our type theory
semantics relative to a categorical axiomatization. Essentially, we have parameterized our
interpretation over any model satisfying the Freyd-Elgot axioms. First, this lets us show that many
different concrete semantics are valid models of SSA, as can be seen in section 6. This also let us
show that our equational theory is complete: we have exactly the equations valid in all Freyd-Elgot
models, no more and (crucially) no less. As a result, implementors and researchers proving things
about compiler optimizations do not have to deal with any of the messy details of (for example) weak
memory semantics, unless they are trying to study optimizations specifically involving weak memory.
All the usual  equations used in control- and data-flow optimizations are independent of those
details, and can be validated using the equational theory.

Conversely, researchers working on models of weak memory (and other strange models of computation
such as quantum computation), can use the Freyd-Elgot axioms as a target. If they ensure their
models satisfy these axioms, then using an SSA-based IR in their compiler is justified. 
...
functions, the former having effect $\bot$. \citet{fuhrmann-direct-1999} studies languages with
...
whose Kleisli categories are guarded Elgot), to support partial non-unique iteration -- in other words,
categories where the fixed point does not always exist. Generalizing
...
for example, one would find in a proof assistant) or productive processes. 
...
$\ms{T}\;X = (X \times \mathbb{N}^\ast) + \mathbb{N}^\omega$ over $\ms{Set}$. Notably, unlike our trace
...
\begin{equation}
  ...
  &= \casestmt{e}{x}{\ms{case}_{\ms{L}}\;x\;\{(\ell_i(A_i): t_i)_i\}}{y}{s}
  \end{aligned}
\end{equation}
with typing rule
\begin{equation}
...
\begin{equation}
  \prftree[r]{\rle{unpack$^+$}}
    {\hasty{\Gamma}{\epsilon}{e}{\ms{L}}}
    {\haslb{\Gamma}{\ms{unpack}^+(\ms{L})\;e}{\ms{L}}}
\end{equation}
defined as follows:
\begin{equation}
...
Sequencing can be expressed using a \ms{where}-block as follows (with $\ell$ fresh):
\begin{equation}
  \prftree[r]{\rle{seq}}
...
\begin{equation}
  \begin{aligned}
    \topwhile{\ms{L}}{\ms{br}\;\ell\;a} &= \brb{\outlb}{\iota_{\ms{L}, \ell}\;a} \\
...
\begin{equation}
  ...
  \ms{ua}_{\ms{L}, (\ms{R}, \ell(A))}\;e 
    = \caseexpr{e}{x}{
        ...
        {y}{\iota_r\;(\iota_r\;y)}
\end{equation}
Note that the base case $\topwhile{\ms{L}}{\ms{br}\;\ell\;a}$ is
%
The $\topwhile{\ms{L}}{r}$ function does the actual transformation, and we can see that most of the
cases are trivial except for: 
\begin{itemize}
  \item The base case, in which we simply replace a branch $\ms{br}\;\ell\;a$ with a branch to the
  output label $\outlb$ with the appropriate injection $\iota_{\ms{L}, \ell}\;a$ as parameter.
  \item The $\where{r}{(\wbranch{\ell_i}{x_i}{t_i},)_i}$ case, in which we replace the set of labels
  bound by the $\ms{where}$ clause with a tag and a while loop containing a case statement branching
  on the tag. (This uses the $\ms{ua}$ function, which implements associativity of $n$-ary
  coproducts. That is, given $\hasty{\Gamma}{\bot}{e}{[L, R]}$, we have that
...
\begin{theorem}[name=B\"ohm-Jacopini for SSA, restate=bohmjacopini]
  For all $\haslb{\Gamma}{r}{\ms{L}}$, we have that $\shaslb{\Gamma}{\towhile{\ms{L}}{r}}{\ms{L}}$
  is structured, and $\lbeq{\Gamma}{r}{\towhile{\ms{L}}{r}}{\ms{L}}$. In particular, we have that
...
\subsection{Lexical SSA and ANF}
...
\begin{proof}
  ...
  one permutation, so we are done. Otherwise, assume $G = \beta, (\wbranch{\ell_i}{x_i}{t_i},)_{i
  \in I}$. Furthermore, let:
  \begin{itemize}
    \item $\wbranch{\ell_i}{x_i}{\beta_i}$ be the children of $\beta$ in $G$ in order
    \item $G_i = (\wbranch{\kappa_{i, j}}{y_{i, j}}{t_{i, j}},)_{j \in I}$ be the CFG composed of
    the descendants of $\beta_i$ in $G$, with $\beta_i$ as entry block, in order
    \item $\wbranch{\ell_i'}{x_i'}{\beta_i'}$ be the children of $\beta$ in $G'$ in order
    \item $G_i'$ be the CFG composed of the descendants of $\beta_i'$ in $G'$, with $\beta_i'$ as
    entry block, in order
  \end{itemize}
  By assumption, there exists some permutation $\sigma: I \to I$ such that $G' = \beta,
  (\wbranch{\ell_{\sigma_i}}{x_{\sigma_i}}{t_{\sigma_i}},)_{i \in I}$. It follows that, since the
  dominance relation on labels is permutation-invariant, there exists some permutation $\rho$ such
  that $\forall i \in I, \wbranch{\ell_i'}{x_i'}{\beta_i'} =
  \wbranch{\ell_{\rho_i}}{x_{\rho_i}}{\beta_{\rho_i}}$, as well as permutations $\tau_i$ such that
  $G_i' = (\wbranch{\ell_{\rho_i, \tau_{ij}}}{x_{\rho_i, \tau_{ij}}}{t_{\rho_i, \tau_{ij}}},)_{j \in
  I}$, implying in particular that $G_i' \simeq G_{\rho_i}$. By induction, we hence have that, for
  all $i$, $\toreg{G_i'} \teqv \toreg{G_{\rho_i}}$, and hence that
  \begin{equation}
    \begin{aligned}
    \toreg{G'} &:= \adddom{\beta}{(\wbranch{\ell_{\rho_i}}{x_{\rho_i}}{\toreg{G_i'}},)_{i \in I}} \\
    &\teqv \adddom{\beta}{(\wbranch{\ell_{\rho_i}}{x_{\rho_i}}{\toreg{G_{\rho_i}}},)_{i \in I}} \\
    &\teqv \adddom{\beta}{(\wbranch{\ell_i}{x_i}{\toreg{G_i}},)_{i \in I}} \teqv \toreg{G}
    \end{aligned}
  \end{equation}
\end{proof}
...
\begin{proof}
  ...
  assume $r = \adddom{\beta}{(\wbranch{\ell_i}{x_i}{t_i},)_{i \in I}}$. Clearly, $\tocfg{r}$ will
  have entry block $\beta$; moreover, since every block in $\tocfg{r}$ other than those of the form
  ...
  \begin{equation}
    G \simeq \beta, (\wbranch{\kappa_i}{y_i}{G_i},)_{i \in I} \qquad
    \forall i \in I, G_i = \beta_i, (\wbranch{\kappa_{i, j}}{y_{i, j}}{\beta_{i, j}},)_{j \in I}, R_i
  \end{equation} 
...
\begin{lemma}[Label Substitution Splitting]
  ...
  \begin{equation}
    ...
    ; \delta^{-1}_{\Sigma}
    ; [\dnt{\haslb{\Gamma, \bhyp{x_i}{A_i}}{\sigma\;\ell_i\;x_i}{\ms{K}}},]_i
  \end{equation}
  and therefore
  \begin{equation}
    \delta^{-1}_{\Sigma}
...
\begin{proof}
  ...
  \begin{itemize}[leftmargin=*]
    ...
    \begin{equation}
      \begin{aligned}
        & \dnt{\haslb
          {\Gamma, \bhyp{x_i}{A_i}}{[\lupg{(\kappa_j(x_j) \mapsto t_j,)_j}](\sigma\;\ell_i\;x_i)}
          {\ms{L}, \ms{R}}} \\
        & = \lmor{\dnt{\haslb{\Gamma, \bhyp{x_i}{A_i}}{\sigma\;\ell_i\;x_i}{\ms{L}, \ms{R}'}}}
        ...
        & = \lmor{\dnt{\haslb{\Gamma, \bhyp{x_i}{A_i}}{\sigma\;\ell_i\;x_i}{\ms{L}, \ms{R}'}}}
          ...
          ; \dnt{\lbsubst{\Gamma}{\lupg{(\kappa_j(x_j) \mapsto t_j,)_j}}
            {\ms{L}, \ms{R}'}{\ms{L}, \ms{R}}} \\
        & = \rlmor{\dnt{\haslb{\Gamma, \bhyp{x_i}{A_i}}{\sigma\;\ell_i\;x_i}{\ms{L}, \ms{R}'}}}
        ; \dnt{\Gamma} \otimes \alpha^+_{\dnt{\ms{L}} + \dnt{\ms{R}'}}
        ; \delta^{-1}
        ; [\pi_r ; \iota_l ; \alpha^+_{\dnt{\ms{L}, \ms{R}}} , G]
         \\
        & = \rlmor{
          \dnt{\haslb{\Gamma, \bhyp{x_i}{A_i}}{\sigma\;\ell_i\;x_i}{\ms{L}, \ms{R}'}}
        ...
        ; [\pi_r ; \iota_l ; \alpha^+_{\dnt{\ms{L}, \ms{R}}} , G] \\
        & = \rcase{
          \dnt{\haslb{\Gamma, \bhyp{x_i}{A_i}}{\sigma\;\ell_i\;x_i}{\ms{R}'}}
          ; \iota_r
        }
        ; [\pi_r ; \iota_l ; \alpha^+_{\dnt{\ms{L}, \ms{R}}} , G] \\
        & = \rlmor{
    ...
    \begin{equation}
      \begin{aligned}
        & \loopmor{\Gamma}
          {(\wbranch{\ell_i}{x_i}{[\lupg{(\kappa_j(x_j) \mapsto t_j,)}](\sigma\;\ell_i\;x_i)},)_i}
          {\ms{L}}
        \\
        & = \delta^{-1}_\Sigma 
          ; [ \dnt{\haslb{\Gamma, \bhyp{x_i}{A_i}}
                  {[\lupg{(\kappa_j(x_j) \mapsto t_j,)_j}](\sigma\;\ell_i\;x_i)}{\ms{L}, \ms{R}}}, ]_i
          ; \alpha^+_{\dnt{\ms{L}} + \Sigma_i \dnt{A_i}}   \\
        & = \delta^{-1}_\Sigma 
          ; [ \rlmor{
                \dnt{\haslb{\Gamma, \bhyp{x_i}{A_i}}{\sigma\;\ell_i\;x_i}{\ms{R}'}}
        ...
        ; [
          ...
          \rfix{
            \loopmor{\Gamma}{(\wbranch{\ell_i}{x_i}
                    {[\lupg{(\kappa_j(x_j) \mapsto t_j,)}](\sigma\;\ell_i\;x_i)},)_i}{\ms{L}}}
        ] \\
        & = \dnt{\haslb{\Gamma}
          {\where{r}{[\lupg{(\kappa_j(x_j) \mapsto t_j,)}](\sigma\;\ell_i\;x_i)},)_i}{\ms{L}}}
      \end{aligned}
    \end{equation}
    as desired.
  \end{itemize}
...
\begin{lemma}[\ms{where}-fusion]
  The rewrite rules \brle{cfg-fuse$_1$} (Eqn.~\ref{eqn:where-fusion-1}) and \brle{cfg-fuse$_2$} are
  sound, where we define
  \begin{equation}
  \prftree[r]{\rle{cfg-fuse$_2$}}
    {\haslb{\Gamma}{r}{\ms{L}, (\lhyp{\ell_i}{A_i},)_i, \kappa(B)}}
    {
      \prfStackPremises{
        \forall i \in I. \haslb{\Gamma, \bhyp{x_i}{A_i}}{t_i}{
          \ms{L}, (\lhyp{\ell_j}{A_j},)_{j \in I}, \kappa(B)}
      }{
        \haslb{\Gamma, \bhyp{y}{B}}{s}{
          \ms{L}, (\lhyp{\ell_j}{A_j},)_{j \in I}, \kappa(B), 
            (\lhyp{\ell_{j'}'}{A_{j'}'},)_{j' \in I'},}
      }{
        \forall i' \in I'. \haslb{\Gamma, \bhyp{x_{i'}'}{A_{i'}'}}{t_{i'}'}{
          \ms{L}, (\lhyp{\ell_j}{A_j},)_{j \in I}, \kappa(B), 
            (\lhyp{\ell_{j'}'}{A_{j'}'},)_{j' \in I'}}
      }
    }
    {
      \prfStackPremises{
        \Gamma \vdash 
          \where{r}{(\wbranch{\ell_i}{x_i}{t_i},)_{i \in I}, 
            \wbranch{\kappa}{y}{\where{s}{(\wbranch{\ell_{i'}'}{x_{i'}'}{t_{i'}'},)_{i' \in I'}}}}
      }{
        \hspace{8em}
        \teqv \where{r}{(\wbranch{\ell_i}{x_i}{t_i},)_{i \in I}, 
            \wbranch{\kappa}{y}{s}, (\wbranch{\ell_{i'}'}{x_{i'}'}{t_{i'}'},)_{i' \in I'}}
        \rhd \ms{L}
      }
    }

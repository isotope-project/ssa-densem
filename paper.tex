...
is to, rather than treating the control-flow graph $C$ as a flat collection of basic blocks (with a
distinguished entry label $\ell$), to instead consider (subtrees of) the dominance tree $r$, with
the root of the tree implicitly being the entry block. We call such subtrees \emph{regions}: we note
that they have a single entry (the root) and multiple exits (the leaves), and so generalize the more
standard concept of a single-entry-single-exit region in the CFG.
...
(which can only be reached through $r$). We may then enforce dominance-based scoping simply by
making the variables defined in $r$ visible only in the $t_i$, which, as previously stated,
\emph{must} be dominated by $r$. Syntactically, this is just lexical scoping of \ms{where}-blocks.

It is easy to see (we demonstrate this more rigorously in Section~\ref{ssec:ssa-normal}) that, given
a CFG $C$ with a distinguished entry label $\ell$, there exists some way to annotate its topological
sort w.r.t. the dominance relation) with \ms{where}-blocks to obtain a region $r$ which is lexically
well-scoped if and only if $C$ is a valid SSA program. Conversely, erasing the \ms{where}-blocks
from a region $r$ and giving the root a name trivially yields a (topologically sorted!) SSA program

\TODO{explain where-statements}

\TODO{tie scoping for labels and variables; encodes dominance tree}

\TODO{see also, for a data structure, \ref{fig:ssa-data}}

\TODO{if well-scoped w.r.t. dominance, you can topologically sort and insert where's according to
the dominance tree}

\TODO{later, will show that any two where-bracketings are semantically equivalent}

\TODO{so this is the same as regular SSA}
...
\TODO{this explanation is redundant now that it is in Section~\ref{ssec:tt-ssa}; go merge it /
pull-up the end}

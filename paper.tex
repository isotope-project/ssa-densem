...
In particular, we can build up expressions as follows:
\begin{itemize}
  \item \todo{br rule lore} \todo{Note that ``returning'' is implemented by
  simply branching to a distinguished label $\ms{ret}$, with $\ms{ret}\;a$
  simply being an abbreviation for $\ms{br}\;\ms{ret}\;a$.}
...
\citet{moggi-91-monad} showed a cartesian closed category equipped with a strong monad interprets
effectful higher-order functional programs. SSA, however, is a first-order language. In particular,
since SSA does not support first-class functions, we have no need for a cartesian closure.
Similarly, first-class computations, represented by objects of the form $\ms{T}(\ms{T}(A))$, are not
usually supported by SSA. Freyd categories are to first order languages as strong monads on a CCC
are to higher-order languages. In particular, every strong monad on a CCC gives rise to a Freyd
category in a trivial manner. SSA, however, also has branching and loops. To represent these, we
need to add distributive coproducts and Elgot structure respectively.
...
\begin{definition}[Freyd category]
  ...
  \begin{itemize}
    ...
    \item For each $A_1, A_2$, there exist morphisms $\pi_i : A_1 \times A_2 \to_{\mc{C}_\bot} A_i$
    making $A_1 \otimes A_2$ into a cartesian product on $\mc{C}_\bot$
    \item $I$ is a terminal object in $\mc{C}_\bot$
    \item $\langle f, g \rangle 
      = \dmor{} ; f \otimes - ; g \otimes -  
      = \dmor{} ; g \otimes - ; f \otimes -$
  \end{itemize}
\end{definition}

\TODO{diagonal/projection presentation: in particular, we note that every category with $\dmor{}$...}
...
\begin{definition}[(Strong) Elgot structure]
  ...
  \emph{strong} monad is strong Elgot if its Kliesli category has a strong Elgot structure.
\end{definition}
...
\begin{theorem}[Substitution]
  ...
  \dnt{\Delta}$ (in particular, this is always the case when (todo: the effect
  ...
  \begin{enumerate}
    ...
    \item $\dnt{\issubst{[\gamma]\sigma}{\Delta}{\Xi}}
      = \dnt{\issubst{\gamma}{\Gamma}{\Delta}};\dnt{\issubst{\sigma}{\Delta}{\Xi}}$
...
\begin{theorem}[Completeness (Terms)]
...
\begin{theorem}[Completeness (Regions)]
  ...
  \begin{enumerate}
    ...
    \item Likewise $e \mapsto \letstmt{\square}{e}{\ms{ret}\;\square}$ is a

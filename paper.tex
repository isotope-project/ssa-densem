...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
Fixing a distinguished label ``\ms{r}'', we can define a ``packing'' operation on regions as
follows:
\begin{equation}
  \haslb{\Gamma}{r}{\ms{L}} \implies 
  \haslb{\Gamma}{[r]^+ := [\ms{pack}_{\ms{r}}(\ms{L})]r}{\ms{r}([\ms{L}])}
\end{equation}
...
is possible to relate the semantics of the substituted term to the original more precisely. To do
this, we first need to give a denotational semantics to substitutions themselves, which we interpret
as morphisms as in Figure~\ref{fig:ssa-subst-sem}. We can then state the following soundness

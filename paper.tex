...
\newcommand{\tmseq}[4]{\issubst{#1 \approx #2}{#3}{#4}}
\newcommand{\lbseq}[5]{\lbsubst{#1 \approx #2}{#3}{#4}{#5}}
...
\begin{itemize}
  ...
  any of the other expression constructors, reflecting the path that this does not effect the
  expected operational behaviour of our expression.
...
\begin{itemize}
  ...
  $$
  (\letexpr{(x, y)}{(a, b)}{c}) 
  \approx (\letexpr{x}{a}{\letexpr{y}{b}{c}})
  \approx ([a/x](\letexpr{y}{b}{c}))
  \approx ([a/x][b/y]c)
  $$
  ...
  \ms{let}-expression into its own unary \ms{let}-expression
\end{itemize}
...
\begin{align*}
  (\letexpr{(x, y)}{f\;a}{b})
  &\approx (\letexpr{z_f}{f\;a}{\letexpr{(x, y)}{z}{b}}) \\
  &\approx (\letexpr{z_a}{a}{\letexpr{z_f}{f\;z_a}{\letexpr{(x, y)}{z}{b}}}) \\
  &\approx (\letexpr{z_a}{a}{\letexpr{(x, y)}{f\;z_a}{b}})
\end{align*}
% TODO: "it is enough to give?" Let's try removing rules from the formalization...
Similarly, we give $\eta$, $\beta$, and binding rules for \brle{case} expressions,
as in Figure \ref{fig:ssa-case-expr}. In particular
\begin{itemize}
  \item \todo{left, right}
  \item \todo{eta}
  \item \todo{binding}
...
\TODO{deal with case distributivity}

\TODO{rules for inl, inr, op for free}

\TODO{unit: $\mb{1}$ is terminal}

\TODO{abort: $\mb{0}$ is initial}

...
\newcommand{\envinr}[1]{\iota_{{#1}r}}
\newcommand{\envinl}[1]{\iota_{{#1}l}}
...
Our first step is to extend the work of \citet{chakravarty-functional-ssa-2003}, by
providing an algorithm for converting between \isotopessa{} and ANF in an equivalence
preserving way. We begin by defining a typing judgement $\ahasty{\Gamma}{\epsilon}{a}{A}$ for \emph{atomic
expressions} in Figure~\ref{fig:ssa-ops}. An atomic expression is either a single variable, a
function application, an injection, a pair of variables, or the unit expression. We specifically
forbid \ms{case}-expressions and \ms{let}-bindings at this level; they are required to appear only
at the region level. It is obvious that $\ahasty{\Gamma}{\epsilon}{a}{A} \implies
\hasty{\Gamma}{\epsilon}{a}{A}$. 
...
\end{equation}
\TODO{check etc.}
We may adapt some of the Elgot equations to $\rfix{\cdot}$ as follows:
\begin{itemize}
  \item \emph{Fixpoint}: given $f : R \otimes A \to B + A$,
  \begin{equation}
    \rfix{f} = \rcase{f} ; [ \pi_r, \rfix{f} ]
  \end{equation}
  \item \emph{Naturality}: given $f : R \otimes A \to B + A$, $g : B \to C$, we have
  \TODO{this...}
  \item \emph{Dinaturality}: given $f : R \otimes A \to B + A'$, $g : R \otimes A' \to A$, we have
    \begin{equation}
      \rfix{\rcase{f} ; \pi_r + g}
      = \rcase{f} ; [\pi_r, \rfix{\lmor{g} ; \pi_l \otimes A ; f}]
    \end{equation}
\end{itemize}
One particularly useful property we may derive is that
\begin{equation}
  \begin{aligned}
  \lmor{\rfix{f}} ; \pi_l \otimes B 
...
\TODO{think about pulling down to appendix...}

\TODO{this is \emph{much} clearer as a string diagram}
...
\begin{theorem}[name=Completeness (Expressions), label=completenessexpr]
...
\begin{theorem}[name=Completeness (Regions), label=completenessregions]
  We have that, for all pure $\ms{eff}(\Gamma) = \bot$,
  $$
    \lbeq{\Gamma}{r}{r'}{\ms{L}} 
...
\end{definition}
%
Given $f : R \otimes A \to B$ will introduce the syntax sugar
\begin{equation}
  \rlmor{f} := \dmor{R} ; \alpha ; R \otimes f : R \otimes A \to R \otimes B
\end{equation}
In particular, we have that $\rseq{}{f}{g} = \rlmor{f} ; g$. It is trivial to verify (for example,
by drawing string diagrams) that
\begin{equation}
  \begin{gathered}
...
We can define
\begin{itemize}
  \item Associators $\alpha^R_{A, B, C} := \pi_r ; \alpha_{A, B, C}$
  \item Unitors $\lambda^R_A := \pi_r ; \lambda_A$ and $\rho^R_A := \pi_r ; \rho_A$
  \item Symmetries $\sigma^R_{A, B} := \pi_r ; \sigma_{A, B}$
  \item Projections $\pi_l^R := \pi_r ; \pi_l$ and $\pi_r^R := \pi_r ; \pi_r$
  \item Terminal morphisms $!^R_A := \pi_r ; !_A = !_{R \otimes A}$
  \item Diagonals $\Delta^R_A := \pi_r ; \Delta_A$
  \item Pure morphisms ${\envcom{\mc{C}}{R}}_\bot(A, B) = \mc{C}_{\bot}(R \otimes A, B)$
\end{itemize}
Since $\pi_r ; \cdot$ is a functor into the co-Kleisli category, to check we indeed have a Freyd
category, it suffices to show that:
\begin{itemize}
  \item $\alpha^R_{A, B, C}$ is natural: \todo{this}
  \item $\lambda^R_{A}$ is natural: \todo{this}
  \item $\rho^R_{A}$ is natural: \todo{this}
  \item $\sigma^R_{A, B}$ is natural: \todo{this}
  \item Pure morphisms are central: given $f : R \otimes A \to B$ and $g : R \otimes A' \to B'$
  pure, we have that
  \begin{equation}
    \begin{aligned}
    \rseq{}{f \otimes_R A'}{B \otimes_R g}
    & = \Delta_R ; \alpha ; R \otimes (\alpha^{-1} ; f \otimes A' ; \sigma) 
      ; \alpha^{-1} ; g \otimes B ; \sigma \\
    & = \Delta_R ; \alpha ; R \otimes (R \otimes \sigma ; \alpha^{-1} ; g \otimes A ; \sigma) 
    ; \alpha^{-1} ; f \otimes B' \\
    & = \rseq{}{A \otimes_R g}{f \otimes_R B'}
    \end{aligned}
  \end{equation}
  since both sides correspond to the diagram in Figure~\ref{fig:env-slide}. We will write this
  morphism as $f \otimes_R g$.
  \item $!^R_A$ is terminal for pure morphisms: yes, since $!^R_A$ is just $!_{R \otimes A}$ which
  is the terminal morphism in $\mc{C}$
  \item $\Delta^R_A$ duplicates pure morphisms: we have
  \begin{equation}
    \begin{aligned}
    \rseq{}{f}{\Delta^R_B} 
      = \Delta_R ; \alpha ; R \otimes f ; \pi_r ; \Delta_B
      = f ; \Delta_B = \Delta_{R \otimes A} ; f \otimes f 
      = \Delta_R ; \alpha ; R \otimes (\pi_r ; \Delta_A) ; 
    \end{aligned}
  \end{equation}
\end{itemize}
...
former may not be the case without distributivity! To do so, given $f : R \otimes A \to B$ and $g :
R \otimes A \to C$, we may define the coproduct and injections
\begin{equation}
...
\end{equation}
\TODO{show this is actually a coproduct}
\TODO{show this is distributive}
\TODO{segue to some more $\lmor{f}$ and $\rcase{f}$ equations}
\TODO{show $\rfix{f}$ gives $\envcop{R}{f}{g}$ an Elgot structure}
...
\begin{figure}
  ...
  \Description{Justification for sliding in co-Kleisli category as a string diagram}
  \label{fig:env-slide}
\end{figure}

\section{Proofs}

\TODO{organize this into subsections or somesuch}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \end{equation}
    as desired.
    \item If $r = \where{s}{(\wbranch{\ell_i}{x_i}{t_i},)_i}$, then we have by induction that,
    for all $i$,
    \todo{convert to use new formulation, or nah?}
    \begin{equation}
      \begin{aligned}
      & \dnt{\issubst{\gamma}{\Gamma}{\Delta}} \otimes \dnt{A_i} 
      ; \dnt{\haslb{\Delta, \bhyp{x_i}{A_i}}{t_i}{\ms{L}}}
      \\ &= \dnt{\issubst{(\gamma, x_i \mapsto x_i)}  
                  {(\Gamma, \bhyp{x_i}{A_i})}{(\Delta, \bhyp{x_i}{A_i})}}
      ; \dnt{\haslb{\Delta, \bhyp{x_i}{A_i}}{t_i}{\ms{L}}}
      \\ &= \dnt{\haslb{\Gamma, \bhyp{x_i}{A_i}}{[\gamma, x_i \mapsto x_i]t_i}{\ms{L}}}
          = \dnt{\haslb{\Gamma, \bhyp{x_i}{A_i}}{[\gamma]t_i}{\ms{L}}}
      \end{aligned}
    \end{equation}
    and therefore that
    \begin{equation}
      \begin{aligned}
        & \dnt{\issubst{\gamma}{\Gamma}{\Delta}} \otimes \Sigma_i\dnt{A_i} ; \delta^{-1}_{\Sigma} ;
        \\ & \qquad
          [ 
            \Delta_{\dnt{\Delta}} \otimes \dnt{A_i} ; \alpha ;
            \dnt{\Delta} \otimes 
            \dnt{\haslb{\Delta, \bhyp{x_i}{A_i}}{t_i}{\ms{L}, (\lhyp{\ell_j}{A_j},)_j}}
          ]_i ; \dnt{\Delta} \otimes \alpha^+ ; \delta^{-1} \\
        &= \delta^{-1}_{\Sigma} ; [ 
          \Delta_{\dnt{\Gamma}} \otimes \dnt{A_i} ; \alpha ;
          \dnt{\Gamma} \otimes 
          \dnt{\haslb{\Gamma, \bhyp{x_i}{A_i}}{[\gamma]t_i}{\ms{L}, (\lhyp{\ell_j}{A_j},)_j}}
        ]_i ; \dnt{\issubst{\gamma}{\Gamma}{\Delta}} \otimes \alpha^+  ; \delta^{-1} \\
        &= \delta^{-1}_{\Sigma} ; [ 
          \Delta_{\dnt{\Gamma}} \otimes \dnt{A_i} ; \alpha ;
          \dnt{\Gamma} \otimes 
          \dnt{\haslb{\Gamma, \bhyp{x_i}{A_i}}{[\gamma]t_i}{\ms{L}, (\lhyp{\ell_j}{A_j},)_j}}
        ]_i ; 
        \\ & \qquad \dnt{\Gamma} \otimes \alpha^+ ; \delta^{-1} ; 
        \dnt{\issubst{\gamma}{\Gamma}{\Delta}} \otimes \dnt{\ms{L}} +
        \dnt{\issubst{\gamma}{\Gamma}{\Delta}} \otimes \Sigma_i\dnt{A_i}
      \end{aligned}
    \end{equation}
    By uniformity, it follows that
    \begin{equation}
      \begin{aligned}
          & \dnt{\issubst{\gamma}{\Gamma}{\Delta}} ; \Delta_{\dnt{\Delta}}
          ; \dnt{\Delta} \otimes (\dnt{\haslb{\Delta}{r}{\ms{L}, (\lhyp{\ell_i}{A_i},)_i}} 
          ...
          \\ & \quad [\pi_r, 
            (\delta^{-1}_{\Sigma} ;
              [ 
                ...
                \dnt{\Delta} \otimes 
                \dnt{\haslb{\Delta, \bhyp{x_i}{A_i}}{t_i}{\ms{L}, (\lhyp{\ell_j}{A_j},)_j}}
              ]_i ; \dnt{\Delta} \otimes \alpha^+ ; \delta^{-1})^\dagger ; \pi_r ] \\ 
          & = \Delta_{\dnt{\Gamma}}
          ; \dnt{\Gamma} \otimes \dnt{\haslb{\Gamma}{[\gamma]r}{\ms{L}, (\lhyp{\ell_i}{A_i},)_i}}
          ; \alpha^+_{\dnt{L} + \Sigma_i \dnt{A_i}}) ; \delta^{-1} ; [
          \\ & \qquad \dnt{\issubst{\gamma}{\Gamma}{\Delta}} \otimes \dnt{\ms{L}} ; \pi_r, 
          \\ & \qquad \dnt{\issubst{\gamma}{\Gamma}{\Delta}} \otimes \Sigma_i\dnt{A_i} ;
          \\ & \qquad\quad
          (\delta^{-1}_{\Sigma} ;
            [ 
              \Delta_{\dnt{\Delta}} \otimes \dnt{A_i} ; \alpha ;
          ...
          & = \Delta_{\dnt{\Gamma}}
          ; \dnt{\Gamma} \otimes \dnt{\haslb{\Gamma}{[\gamma]r}{\ms{L}, (\lhyp{\ell_i}{A_i},)_i}}
          ; \alpha^+_{\dnt{L} + \Sigma_i \dnt{A_i}}) ; \delta^{-1} ; [ \pi_r,
          \\ & \qquad
          (\delta^{-1}_{\Sigma} ;
            [ 
              \Delta_{\dnt{\Gamma}} \otimes \dnt{A_i} ; \alpha ;
              \dnt{\Gamma} \otimes 
              \dnt{\haslb{\Gamma, \bhyp{x_i}{A_i}}{[\gamma]t_i}{\ms{L}, (\lhyp{\ell_j}{A_j},)_j}}
            ]_i ; \dnt{\Gamma} \otimes \alpha^+ ; \delta^{-1})^\dagger ; 
          \\ & \qquad \dnt{\issubst{\gamma}{\Gamma}{\Delta}} \otimes \dnt{\ms{L}} ; \pi_r ] \\
          & = \Delta_{\dnt{\Gamma}}
          ; \dnt{\Gamma} \otimes \dnt{\haslb{\Gamma}{[\gamma]r}{\ms{L}, (\lhyp{\ell_i}{A_i},)_i}}
          ; \alpha^+_{\dnt{L} + \Sigma_i \dnt{A_i}}) ; \delta^{-1} ; [ \pi_r,
          ...
          (\delta^{-1}_{\Sigma} ;
            [ 
              ...
              \dnt{\Gamma} \otimes 
              \dnt{\haslb{\Gamma, \bhyp{x_i}{A_i}}{[\gamma]t_i}{\ms{L}, (\lhyp{\ell_j}{A_j},)_j}}
            ]_i ; \dnt{\Gamma} \otimes \alpha^+ ; \delta^{-1})^\dagger ; \pi_r ] \\
          & = \dnt{\haslb{\Gamma}{[\gamma](\where{s}{(\wbranch{\ell_i}{x_i}{t_i},)_i})}{\ms{L}}}
      \end{aligned}
    \end{equation}
    as desired.
  \end{itemize}
...
\begin{lemma}[Label Substitution Extension]
  For $\lbsubst{\Gamma}{\sigma}{\ms{L}}{\ms{K}}$, we have
  \begin{equation}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item \brle{dinat}: we have by label substitution that, where $\ms{R} = (\ell_i(A_i),)_i$ and
    $\ms{R}' = (\kappa_j(B_j),)_j$, $S = \dnt{\lbsubst{\Gamma}{\sigma}{\ms{R}}{\ms{R}'}}$, and
    $S' = \alpha^+_{\dnt{R}} ; S ; \alpha^+_{\Sigma_j\dnt{B_j}}$,
    \begin{equation}
      \begin{aligned}
    ...
    \begin{equation}
      \begin{aligned}
        ...
        & = \lmor{
          ...
          ; \pi_l \otimes \alpha^+_{\dnt{\ms{L}} + \dnt{\ms{R}}}
          ; \delta^{-1}
          ; \pi_r + (
            S ; \alpha^+_{\Sigma_j\dnt{B_j}}
          )
        ...
        & = \lmor{
            ...
            ; [ \dnt{\haslb{\Gamma, \bhyp{x_i}{B_i}}{t_i}{\ms{L}, \ms{R}}},]_i
            ; \alpha^+_{\dnt{\ms{L}} + \Sigma_j\dnt{B_j}}}
          ; \pi_l \otimes (\dnt{\ms{L}} + \Sigma_j\dnt{B_j})
          ; \delta^{-1} ; \pi_r + S'
          \\
        & = \rcase{\loopmor{(\wbranch{\ell_i}{x_i}{t_i},)_i}} ; \pi_r + S'
          \\
      \end{aligned}
    \end{equation}
    ...
    \begin{equation}
      \begin{aligned}
        ...
        & = \lmor{\lmor{\entrymor{r}} ; \delta^{-1} ; \pi_r + S'} ; \delta^{-1}
          ; [
              \pi_r,
              \rfix{\rcase{\loopmor{(\wbranch{\ell_i}{x_i}{t_i},)_i}} ; \pi_r + S'}
          ] \\
        & = \text{\todo{wat}}
      \end{aligned}
    \end{equation}
    \TODO{rest}
    \TODO{nice co-Kleisli edition?}
    % \begin{equation}
    %   \begin{aligned}
...
\TODO{seq lore and ltimes lore...}

\TODO{destringify correctness of equational theory? OR stringify correctness of substitution?}

\TODO{introduce $\delta$-syntax sugar to reduce suffering?}

\TODO{ 
  formalization pointers? completeness explanation? separate soundness and completeness
  sections? 
}

\TODO{three-address lore? named lore?}

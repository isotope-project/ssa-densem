...
code}, referring to the typical three variables $x, y, z$ in an instruction like $x = y + z$) followed by a \emph{terminator} $\tau$.
A terminator is either a return, a jump to another block, or a conditional branch. We give a grammar
for 3-address code in
Figure~\ref{fig:3addr-grammar}; note that we permit nested conditionals to allow jumping to 
more than two possible target labels (eg, the C switch statement). 

\TODO{add $\ms{ret}\;a$ to terminator grammar, and change case to if-then-else}
...
\TODO{remove lets on the LHS and put both pieces of code in vertical form}
\begin{multline}

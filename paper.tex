...
It turns out that this generalization is essential, as it allows us to relate unary and $n$-ary
control-flow graphs and, in particular, prove a form of the B\"ohm-Jacopini theorem
\cite{bohm-jacopini} for our language, which we will discuss in Section~\ref{ssec:bohm-jacopini}.
...
\label{ssec:bohm-jacopini}

We now come to one of the major results of this paper: that the above rules on \isotopessa{} are
enough to prove a form of the B\"ohm-Jacopini theorem, which states that all programs can be
converted to programs with equivalent semantics using only structured control-flow constructs, i.e.
if-then-else, while, and sequence.

We first need to define what structured control-flow means in the context of \isotopessa{} regions.
Since a region $\haslb{\Gamma}{r}{\ms{L}}$ can jump to any of the exit-points in $\ms{L}$, it does
not quite make sense to restrict the top-level of such a region to have only a single exit-point.

\TODO{rest...}

We begin by defining a ``packing'' operation $[\cdot]$ on label-contexts yielding a sum type
encoding what branch of the label-context we are on as follows:
\begin{equation}
...
\begin{equation}
  \begin{aligned}
  \ms{unpack}_\kappa(\cdot) &= \kappa(x) \mapsto \ms{loop} \\
  \ms{unpack}_\kappa(\ms{L}, \ell(A)) &= \kappa(x) \mapsto 
    \caseexpr{x}{l}{[\ms{unpack}_\kappa(\ms{L})]\brb{\kappa}{l}}{r}{\brb{\ell}{r}}
  \end{aligned}
\end{equation}
We can further show that these substitutions are mutually inverse, i.e., that
\begin{equation}
  \begin{aligned}
  \lbsubst{\Gamma}
    {[\ms{unpack}_\kappa(\ms{L})]\ms{pack}_\kappa(\ms{L}) &\teqv \ms{id}_{\ms{L}}}{\ms{L}}{\ms{L}}
  \\
  \lbsubst{\Gamma}
  ...
  \end{aligned}
\end{equation}
Fixing a distinguished label ``$\outlb$'', we can define a ``packing'' operation on regions as
follows:
\begin{equation}
...
\begin{proof}
  Our proof is by rule induction on the equational theory. We sketch the cases as follows:
  \begin{itemize}
    \item \emph{Congruence}: \todo{trivially by induction}
    \item \emph{Initial, Terminal}: \todo{trivial}
    \item \emph{Binding}, \brle{let$_2$-$\beta$}: \todo{follows trivially from string diagrammatic
    reasoning}
    \item \brle{let$_1$-$\beta$}: \todo{single substitution and let binding look the same by
    soundness of substitution}
    \item \brle{let$_2$-$\eta$}, \brle{case-$\eta$}, \brle{case-inl}, \brle{case-inr}: \todo{by
    universal property}
  \end{itemize}
\end{proof}
...
\begin{proof}
  ...
  \begin{itemize}
    \item \emph{Congruence}: \todo{trivially by induction}
    \item \emph{Initial}: \todo{trivial}
    \item \emph{Binding}, \brle{let$_2$-$\beta$}: \todo{like before, follows trivially from string
    diagrammatic reasoning}
    \item \brle{let$_1$-$\beta$}: \todo{exactly analogous to expressions}
    \item \brle{let$_2$-$\eta$}, \brle{case-$\eta$}, \brle{case-inl}, \brle{case-inr}: \todo{exactly
    analogous to expressions}
...
\begin{enumerate}
  ...
  \item We then show that the category of expressions has the structure of a distributive Freyd
  category, whereas the category of regions has the structure of an \isotopessa{} model, and hence
  that expressions may be interpreted in the former and regions in the latter.
  ...
  \begin{equation}
    \hasty{\Gamma, \invar : [\Delta]}{\epsilon}{\dnt{\hasty{\Delta}{\epsilon}{a}{A}}}{A}
    \qquad
    \haslb{\Gamma, \invar : [\Delta]}{\dnt{\haslb{\Delta}{r}{\ms{K}}}}{\ms{L}, \outlb([\ms{K}])}
  \end{equation}
  where packing of contexts is defined as in Section~\ref{ssec:bohm-jacopini}.
  
  \item Finally, we refine this result to show that
  \begin{equation}
    \tmeq{\Gamma, \invar : [\Delta]}{\epsilon}{\dnt{\hasty{\Delta}{\epsilon}{a}{A}}}{[a]}{A}
    \qquad
    \lbeq{\Gamma \invar : [\Delta]}{\dnt{\haslb{\Delta}{r}{\ms{K}}}}{[r]}{\ms{L}, \outlb([\ms{K}])}
  \end{equation}
  Since the packing operator $[\cdot]$ on terms and regions from Section~\ref{ssec:bohm-jacopini} is
  injective for pure contexts $\ms{eff}(\Gamma) = \bot$, and hence in particular for $\Gamma =
  \cdot$, $\ms{L} = \cdot$, it follows that in this case the category of expressions and the
  category of regions are the initial distributive Freyd category and distributive Freyd-Elgot
  category respectively.
\end{enumerate}
Each of these steps has been formalized in Lean 4.
...
\begin{itemize}
  ...
  \item Morphisms $\ms{Th}^\otimes(\Gamma)_\epsilon(A, B) = \{e \mid \hasty{\Gamma,
    \bhyp{\invar}{A}}{\epsilon}{e}{A}\}$ quotiented by $\tmeq{\Gamma,
    \bhyp{\invar}{A}}{\epsilon}{e}{e'}{A}$
...
\TODO{tensor functors + codiagonal + projections + pure subcategory $\bot$}
...
%   \item Injections: $\iota_l\;\invar$, $\iota_r\;\invar$
%   \item Sums: $
%     [a, b] = \caseexpr{\invar}{\invar}{a}{\invar}{b}
%   $
%   \item Distributor
%     $\delta = \caseexpr{\invar}{(x, y)}{(x, \iota_l\;y)}{(x, y)}{(x, \iota_r\;y)}$
%     with inverse
%     $\delta^{-1} = \ms{let}\;(x, y) = \invar; 
%       \caseexpr{y}{z}{\iota_l(x, z)}{z}{\iota_r(x, z)}$
% \end{itemize}

\TODO{pack typing}

\TODO{pack equality lore}
...
\begin{theorem}[Completeness (Expressions)]
  ...
  $$
  In particular, this implies that $\ms{Th}(\cdot)$ is the initial distributive Freyd category
\end{theorem}
...
\begin{theorem}[Completeness (Regions)]
  ...
  \begin{itemize}
    ...
    \item Injections: $\ms{ret}\;\iota_l\;\invar$, $\ms{ret}\;\iota_r\;\invar$
    \item Sums: $
      [r, r'] = \casestmt{\invar}{\invar}{r}{\invar}{r'}
    $
    \item Distributor
      $\delta = \casestmt{\invar}{(x, y)}{\ms{ret}\;(x, \iota_l\;y)}{(x, y)}{\ms{ret}\;(x, \iota_r\;y)}$
      with inverse
      $\delta^{-1} = \ms{let}\;(x, y) = \invar; 
        \casestmt{y}{z}{\ms{ret}\;\iota_l(x, z)}{z}{\ms{ret}\;\iota_r(x, z)}$

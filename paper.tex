...
being given by an immutable \ms{let}-binding. One immediate advantage of SSA is that it makes
dominance-based scoping. In particular, given a variable $x$, which we can now associate to its
unique definition $D_x$, $x$ is in scope at a point $P$ if and only if $D_x$ strictly dominates $P$.
...
\begin{multline}
  ...
  \\ \equiv \letexpr{x_0}{3y + 5}{\letexpr{x_1}{3x_0 + 2}{\letexpr{x_2}{3x_1 + 1}{\ms{ret}\;x_2}}}
\end{multline}
...
\begin{figure}
  ...
  \begin{align*}
    ...
    \dnt{\haslb{\Gamma}{\where{r}{(\wbranch{\ell_i}{x_i}{t_i},)_i}}{\ms{L}}}
      &=  
      \Delta_{\dnt{\Gamma}}
      ; \dnt{\Gamma} \otimes \ms{entry}(r) ; \delta^{-1} ; [\pi_r, 
      \\ & \qquad 
        (\Delta_{\dnt{\Gamma}} \otimes \Sigma_i\dnt{A_i} ; \alpha 
        ; \dnt{\Gamma} \otimes \ms{loop}((\wbranch{\ell_i}{x_i}{t_i},)_i) ; \delta^{-1})^\dagger 
        ; \pi_r ]
  \end{align*} 
  \begin{align*}
    \text{where} \qquad 
    & \boxed{\iota_{\ms{L}, \ell} : \dnt{A} \to_\bot \dnt{\ms{L}}} \qquad
    \iota_{(\ms{L}, \ell(A)), \ell} = \iota_r \qquad
    \iota_{(\ms{L}, \kappa(B)), x} = \iota_l ; \iota_{\ms{L}, \ell} \\
    & \boxed{\ms{entry}(r) : \dnt{\Gamma} \to \dnt{L} + \Sigma_i\dnt{A_i}} \\
    & \ms{entry}(r) = \dnt{\haslb{\Gamma}{r}{\ms{L}, (\lhyp{\ell_i}{A_i},)_i}} 
    ; \alpha^+_{\dnt{\ms{L}} + \Sigma_i \dnt{A_i}} \\
    & \boxed{\ms{loop}((\wbranch{\ell_i}{x_i}{t_i},)_i) 
      : \dnt{\Gamma} \otimes \Sigma_i\dnt{A_i} \to \dnt{\ms{L}} + \Sigma_i\dnt{A_i}}
    \\
    & \ms{loop}(r) = \delta^{-1}_{\Sigma} 
...
\begin{theorem}[name=Soundness (Substitution), restate=soundnesssubst]
  Given $\dnt{\issubst{\gamma}{\Gamma}{\Delta}} : \dnt{\Gamma} \to \dnt{\Delta}$ where
  $\ms{eff}(\Delta) = \bot$, we have that
  \begin{enumerate}[label=(\alph*)]
...
\begin{lemma}[Label Substitution Extension]
  ...
  \begin{equation}
    ...
    = \dnt{\Gamma} \otimes \alpha^+_{\dnt{L} + \dnt{R}} ; \delta^{-1} ; [
      \pi_r ; \iota_r, \dnt{\lbsubst{\Gamma}{\sigma}{\ms{L}}{\ms{K}}} ; \iota_l
    ] ; \alpha^+_{\dnt{\ms{L}, \ms{R}}}
  \end{equation}
\end{lemma}
...
\begin{proof}
  Fix $\lbsubst{\sigma}{\ms{L}}{\ms{K}}$. We begin by proving the soundness of label substitution
  for regions as follows: assuming $\haslb{\Gamma}{r}{\ms{L}}$, we proceed by induction on $r$
  as follows:
  \begin{itemize}
    \item If $r = \brb{\ell}{a}$, then we have that
    \begin{equation}
      \begin{aligned}
        ...
        & = \Delta_{\dnt{\Gamma}} \otimes \dnt{\hasty{\Gamma}{\bot}{a}{A}}
          ; \dnt{\haslb{\Gamma, \bhyp{x}{A}}{[\sigma](\brb{\ell}{x})}{\ms{K}}} \\
        & = \Delta_{\dnt{\Gamma}} \otimes (\dnt{\hasty{\Gamma}{\bot}{a}{A}} ; \iota_{\ms{L}, \ell})
          ; \dnt{\lbsubst{\Gamma}{\sigma}{\ms{L}}{\ms{K}}} \\
        & = \Delta_{\dnt{\Gamma}} \otimes \dnt{\haslb{\Gamma}{\brb{\ell}{a}}{\ms{L}}}
          ; \dnt{\lbsubst{\Gamma}{\sigma}{\ms{L}}{\ms{K}}}
      \end{aligned}
    \end{equation}
    as desired.
    \item If $r = (\letstmt{x}{a}{t})$, then we have that
    \begin{equation}
      \begin{aligned}
        & \dnt{\haslb{\Gamma}{[\sigma](\letstmt{x}{a}{t})}{\ms{K}}} 
        = \dnt{\haslb{\Gamma}{\letstmt{x}{a}{[\sigma]t}}{\ms{K}}} \\
        & = \Delta_{\dnt{\Gamma}} ; \dnt{\Gamma} \otimes \dnt{\hasty{\Gamma}{\epsilon}{a}{A}}
          ; \dnt{\haslb{\Gamma, \bhyp{x}{A}}{[\sigma]t}{\ms{K}}} \\
        & = \Delta_{\dnt{\Gamma}} ; \dnt{\Gamma} \otimes \dnt{\hasty{\Gamma}{\epsilon}{a}{A}}
          ; \Delta_{\dnt{\Gamma}} 
          ; \dnt{\Gamma} \otimes \dnt{\haslb{\Gamma, \bhyp{x}{A}}{t}{\ms{L}}}
          ; \dnt{\lbsubst{\Gamma}{\sigma}{\ms{L}}{\ms{K}}} \\
        & = \Delta_{\dnt{\Gamma}}
          ; \dnt{\Gamma} \otimes (\Delta_{\dnt{\Gamma}}
            ; \dnt{\Gamma} \otimes \dnt{\hasty{\Gamma}{\epsilon}{a}{A}}
            ; \dnt{\haslb{\Gamma, \bhyp{x}{A}}{t}{\ms{L}}})
          ; \dnt{\lbsubst{\Gamma}{\sigma}{\ms{L}}{\ms{K}}} \\
        & = \Delta_{\dnt{\Gamma}} 
          ; \dnt{\Gamma} \otimes \dnt{\haslb{\Gamma}{\letstmt{x}{a}{t}}{\ms{L}}}
          ; \dnt{\lbsubst{\Gamma}{\sigma}{\ms{L}}{\ms{K}}} \\
      \end{aligned}
    \end{equation}
    as desired.
    \item If $r = (\letstmt{(x, y)}{a}{t})$, then we have that \todo{this}
    \item If $r = \casestmt{a}{x}{s}{y}{t}$, then we have that \todo{this}
    \item If $r = \where{s}{(\wbranch{\ell_i}{x_i}{t_i},)_i}$, then we have that
    \begin{equation}

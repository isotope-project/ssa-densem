...
Conceptually, what we're doing is replacing variables $x$ with arbitrary expressions $a$. While
we're at it, it makes sense to allow the pure control-flow combinators, namely sequencing and
branching, to appear in expressions, by allowing let-expressions and case-expressions, giving us the
grammar for expressions $a$ in Figure~\ref{fig:ssa-grammar}.

\TODO{this adds no expressive power}

\TODO{we now get a Cranelift-like language; we can do E-graphs; this is cool and good}

\TODO{do the same thing for label substitution}

...
$$
\letstmt{x}{\caseexpr{a}{x}{b}{y}{c}}{r}
$$
...
\begin{itemize}
  ...
  into standard SSA
  \item Compiling $a$ to SSA and, binding it's value to $t_a$, inserting a branch to either:
  \begin{itemize}
    \item A new label $\ell_{\ms{left}}$, which computes $b$, binding its value to $t_b$, and jumps
    to $\ell_{\ms{rest}}(t_b)$, or
    \item A new label $\ell_{\ms{right}}$, which computes $c$, binding its value to $t_c$, and jumps
    to $\ell_{\ms{rest}}(t_c)$
  \end{itemize}
\end{itemize}
...
$$
\where{\casestmt{e}{t}{\brb{\ell_1}{t}}{\brb{\ell_2}{y}}{t}}{
  \wbranch{\ell_1}{x}{s},
  \wbranch{\ell_2}{y}{t}
}
$$
...
\TODO{morally, what we're doing is allowing anonymous labels at the ends of our terminators}

\TODO{the other change: a where-block can have lets pulled out of it}

\TODO{our complex transformation above then becomes the simple rewrite rule}

...
\begin{definition}[Distributive category]
  ...
  %
  We will say a Freyd category $\mc{C}$ is distributive if the subcategory of pure morphisms
  $\mc{C}_\bot$ is distributive, contains all injections, and is closed under coproducts (which
  implies, in particular, that $\mc{C}$ is distributive when taken as a premonoidal category).
\end{definition}
...
\begin{proposition}
  If $(-)^\dagger$ is an iteration operator which is satisfies naturality, codiagonal, and is
  $\mc{K}$-uniform for $\mc{K}$ containing all injections and closed under coproducts, then it also
  satisfies dinaturality.
\end{proposition}
Since in a distributive Freyd category $\mc{C}_\bot$ contains all injections and is closed under
coproducts, dinaturality follows from the other axioms of an Elgot category.
%
...
\begin{enumerate}
  ...
  regions from Section~\ref{ssec:records-enums} is injective for pure contexts $\ms{eff}(\Gamma) =

...
control-flow, to be able to perform, for example, state-machine optimizations. In this section, we
will build up some of the machinery to do this, and demonstrate how it can be used by stating and
proving a form of the B\"ohm-Jacopini theorem \cite{bohm-jacopini} for SSA. Of course, all this has
the ulterior motive of laying the groundwork for Section~\ref{ssec:completeness}, where we will
rigorously prove our equational theory is \emph{complete} w.r.t. a categorical model of SSA.

We begin by describing how to convert control-flow into data-flow; since our equational theory is
symmetric, this machinery allows us us to interconvert between the two. We begin by defining a
``packing'' operation $[\cdot]$ on label-contexts yielding a sum type encoding what branch of the
label-context we are on as follows:
\begin{equation}
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
Fixing a distinguished label ``$\outlb$'', we can define a ``\emph{label packing}'' operation on
regions as follows:
\begin{equation}
  \haslb{\Gamma}{r}{\ms{L}} \implies 
  \haslb{\Gamma}{[r]^+ := [\ms{pack}_{\outlb}^+(\ms{L})]r}{\outlb([\ms{L}])}
\end{equation}
In particular the packing substitution has an inverse, it follows that label packing is an injection
w.r.t. the equational theory, i.e.
\begin{equation}
...
composed only of conditional branching, sequencing, and looping.

To adapt this result to SSA, we need to figure out how we should write these combinators for SSA
regions $\haslb{\Gamma}{r}{\ms{L}}$ so that we can build up an inductive set of structured regions.
We will be maximally strict, and allow branching to an exit label in $\ms{L}$ only as the terminal
statement in a structured program (and, in particular, not from within a loop!). It is obvious that
we can represent conditional branching using \ms{case}-statements, but we need convenient primitives
for sequencing and looping. Sequencing can be quite trivially expressed using a \ms{where}-block as
follows:
\begin{equation}
  \prftree[r]{\rle{seq}}
...
\subsection{\texorpdfstring{$\phi$}{phi}-node elimination}
...
% explicit argument. This section will describe the machinery needed to do that, which, combined
% with uniformity to reshuffle variables, is enough to justify the elimination of $\phi$-nodes. 

We begin by defining \emph{packing} of variable contexts as follows:
\begin{equation}
...
\end{equation}
We may similarly define have a ``variable packing'' operation on regions:
\begin{equation}
  \haslb{\Gamma}{r}{\ms{L}} \implies 
...
expressions and regions w.r.t. the equational theory.

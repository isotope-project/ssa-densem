...
$\phi$-nodes can be quite unintuitive because they lack an obvious operational interpretation.
\citet{kelsey-95-cps} establishes a correspondence between SSA and a subset of
\emph{continuation-passing style (CPS)}, a common intermediate representation for functional
compilers. In particular, while not all CPS programs can be directly converted to SSA
form—particularly those using non-local returns like \texttt{longjmp} or \texttt{call/cc}—the
...
therefore be dominated by it; otherwise, there would be no way to call it.
...
\TODO{The work we have covered so far...}
...
\begin{itemize}
  ...
  \item Static Single Assignment (SSA) form was first introduced as a compiler intermediate
  representation by \citet{alpern-ssa-original-88} and \citet{rosen-gvn-1988} to address data-flow
  problems such as \emph{common subexpression elimination (CSE)}, \emph{global value numbering
  (GVN)}, and \emph{constant propagation}; \citet{cytron-ssa-intro-91} provided a foundational
  algorithm for lowering programs from \emph{3-address code}, or \emph{register transfer language
  (RTL)}, into SSA form with a minimal number of $\phi$-nodes, based on the graph-theoretic notion
  of a \emph{dominance frontier}. Since then, SSA has become a cornerstone of modern compiler
  design, forming the basis of intermediate representations in production-grade compilers, notably
  the LLVM compiler infrastructure~\cite{llvm}. 

  \item The key property that makes SSA effective for compiler implementation is that each variable
  is assigned exactly once, simplifying the tracking of variable values throughout the program. For
  example, if we know that $x$ and $y$ are constant, and the (single) definition of $z$ is $z = x +
  y$, then we can safely assume that $z$ is constant as well without further analysis, even if this
  definition appears within a loop. A quintessential example of an SSA-enabled optimization is
  \emph{sparse conditional constant propagation (SCCP)}, introduced by \citet{wegman-sccp-91}, which
  leverages SSA to efficiently combine constant propagation with dead code elimination through a
  lattice-based data-flow analysis.

  \item \citet{appel-ssa} showed informally how SSA can be seen as a collection of mutually
  tail-recursive procedures. In this interpretation, $\phi$-nodes correspond to function arguments,
  and dominance-based scoping corresponds to the lexical scoping of the procedures themselves, with
  dominated procedures appearing inside the bodies of the procedures that dominate them. This
  perspective implies that SSA effectively exposes the functional program hidden inside the
  imperative one. This functional interpretation of SSA highlights why it is so effective for
  optimization: since, just like in functional programming, variables are immutable in SSA, it
  enables algebraic reasoning techniques traditionally used in functional programming. Optimizations
  that are natural in the functional paradigm, such as inlining, dead code elimination, and constant
  propagation, become more accessible in SSA form.

  \item The relationship between SSA and Continuation-Passing Style (CPS) was explored by
  \cite{kelsey-95-cps}. He noted that while not all CPS programs can be directly converted to SSA
  form—particularly those using non-local returns like longjmp or call/cc—the typical outputs of CPS
  transformations avoid such features. Kelsey observed that many optimizations requiring flow
  analysis in CPS could be performed directly in SSA, largely because CPS programs are predominantly
  functional in nature.

  \item Building on these insights, \citet{chakravarty-functional-ssa-2003} formalized a mapping from SSA
  to A-normal form (ANF). They argued that ANF provides a clearer operational interpretation than
  general lambda calculus, facilitating advanced functional optimizations like 0CFA
  (Zero-Control-Flow Analysis). Their work also referenced \citet{flanagan-93-anf}, who advocated
  for ANF over CPS as an intermediate representation, citing its efficiency and the reduced need for
  complex flow analyses.

  \TODO{note: use different argument here}

  \item Despite these advancements, earlier works primarily focused on establishing correspondences
  between SSA and functional representations without providing explicit semantics or comprehensive
  equational theories. \citet{benton-kennedy-99} addressed this gap by introducing the Monadic
  Intermediate Language (MIL). MIL is equipped with an operational semantics from which a rich
  equational theory is derived, allowing for refinement through observational equivalence. Their
  approach differs from Moggi's equational metalanguage \cite{moggi-91-monad} by emphasizing values
  and computations without nesting computations within computations, akin to the distinction between
  Kleisli and Freyd categories.

  \item Further exploring the theoretical underpinnings of SSA, \citet{garbuzov-structural-cfg-2018}
  established a correspondence between an operational semantics for SSA and \emph{Call-By-Push-Value
  (CBPV)} \cite{levy-cbpv-99}. Utilizing normal-form bisimulations from \citet{lassen-bisim}, they
  derived an equational theory to justify compiler optimizations. However, their operational
  semantics is effect-free, which means it cannot justify optimizations involving side effects, such
  as eliminating redundant writes.
  
  \item The work by \citet{goncharov-metalang-21} is particularly relevant as it presents a
  metalanguage for \emph{guarded iteration} using guarded Elgot monads, closely resembling our own
  approach. Their metalanguage, based on labeled iteration concepts from \citet{geron-iteration-16},
  employs fine-grained call-by-value semantics within a distributive category and a strong guarded
  pre-iterative monad for denotational semantics. 

...
\begin{itemize}
  \item We have moved from structured control flow in terms of \ms{while} to
  unstructured jumps between the basic blocks \ms{start} (the entry block),
  \ms{loop}, and \ms{body}. 
  \item We have replaced subexpressions like $i + 1$ in $a * (i + 1)$ with let-bindings,
  so that every expression in our program is atomic. 
\end{itemize}
\TODO{Add a IMP$\to$3ADR, 3ADR$\to$SSA, cite SSA-intro}
...
throughout the execution of the program. To improve our ability to reason about
...
\ms{let}-binding. \todo{Explain why SSA is useful -- i.e., say substitution, even
though it is not totally true\ldots} Unfortunately, it is difficult to express programs with loops
...
\textit{previously} came from. For example, in basic block \ms{loop} in Figure~\ref{fig:ten-fact-ssa},
$i_0$ evaluates to 1
...
blocks with arguments (BBA), as illustrated in Figure \ref{fig:ten-fact-bba}. In this
...
\TODO{cite A Correspondence between Continuation Passing Style and
  Static Single Assignment Form by Richard A. Kelsey }
...
\begin{figure}
  ...
  \begin{subfigure}[t]{.35\textwidth}
    ...
    \end{align*}
    \caption{
      Programs \ref{fig:fact-anf} and \ref{fig:fully-substituted-fact} after substitution;
...
\begin{figure}
  \begin{minipage}{.5\textwidth}
    \begin{subfigure}{\textwidth}
      \begin{align*}
        ...
        & \}
      \end{align*}
      \caption{Substituted program from Figure \ref{fig:fully-substituted-fact}}
      \label{fig:fully-substituted-fact2}
    \end{subfigure}
    \begin{subfigure}{\textwidth}
      \begin{align*}
        & \ms{let}\;n = 10; \\
        ...
        & \}
      \end{align*}
      \caption{Equivalent to Figure \ref{fig:fully-substituted-let} by \textit{dinaturality}}
      \label{fig:suboptimal-fact2}
    \end{subfigure}
  \end{minipage}%
  \begin{subfigure}[c]{.5\textwidth}
    \begin{align*}
      ...
      & \}
    \end{align*}
    \caption{Equivalent to Figure \ref{fig:fully-substituted-fact2} by substitution}
    \label{fig:fully-substituted-let}
  \end{subfigure}
  \caption{
    Decomposing multi-block rewrites (from \ref{fig:suboptimal-fact2} to
    \ref{fig:fully-substituted-fact2}, and therefore to the more optimal program 
...
\TODO{metatheory: substitution; substitution composes and weakens}

...
$\Delta$''.
%
We give the (standard) formal rules for weakening $\Gamma \leq \Delta$ in the first part of Figure
\ref{fig:ssa-meta-rules}. 
\begin{itemize}
  \item \brle{wk-nil} says that the empty context weakens itself,
  \item \brle{wk-skip} says that if $\Gamma$ weakens $\Delta$, then $\Gamma, x: A$ also weakens
  $\Delta$ for arbitrary (fresh) $x$, and
  \item \brle{wk-cons} says that if $\Gamma$ weakens $\Delta$, then $\Gamma$ with $\bhyp{x}{A}$
  added weakens $\Delta, \bhyp{x}{A}$.
\end{itemize}
It is easy to see that weakening defined in this manner induces a partial order on contexts.
Weakening for label-contexts $\ms{L} \leq \ms{K}$ is precisely the dual of this: $\ms{L} \leq
\ms{K}$ precisely when $\ms{L}$ has \emph{fewer} labels than $\ms{K}$, but they otherwise agree. In
particular,
\begin{itemize}
  \item \brle{lwk-nil} says that the empty label-context weakens itself,
  \item \brle{lwk-skip} says that if $\ms{L}$ weakens $\ms{K}$, then $\ms{L}$ also weakens $\ms{K},
  \lhyp{\ell}{A}$ for arbitrary (fresh) $\ell$, and
  \item \brle{lwk-cons} says that if $\ms{L}$ weakens $\ms{K}$, then $\ms{L}$ with $\lhyp{\ell}{A}$
  added weakens $\ms{K}, \lhyp{\ell}{A}$.
\end{itemize}

Likewise, we proceed to define weakening for \emph{label contexts} $\ms{L} \leq \ms{K}$ analogously,
except that we will flip the ordering such that $\ms{L}$ weakens $\ms{K}$ if it contains
\emph{fewer}, rather than \emph{more}, labels than $\ms{K}$. Note that $\ms{L}$ types \emph{fewer}
regions than $\ms{K}$. This makes the metatheory come out more clearly, since label-contexts are
``on the right'' (with variable contexts ``on the left'').
...
%
We may now state weakening formally as follows:

...
axiomatization, and a variety of interesting concrete models.
...
This is exactly what we provide in this paper. 
...
features designed to improve readability and programmer productivity, such as syntactic sugar. Early
...
normalize programs into a form more suitable for analysis and optimization. 
...
Figure~\ref{fig:3addr-grammar}; note that we interpret \ms{if}-statements as \ms{case}-expressions
branching on $\mb{1} + \mb{1}$, and constants as nullary instructions $c()$.
...
\begin{figure}[H]
  \begin{center}
    \begin{grammar}
      ...
      <\(\tau\)> ::= \(\brb{\ell}{o}\) \;|\; $\caseexpr{x}{y}{\tau}{z}{\tau'}$
...
about optimizations. To deal with scoping, it suffices to observe that the function corresponding to
a given basic block $B$ can only be called by other blocks $B'$ having that basic block's parent $P
= \ms{parent}(B)$ in the dominance tree as an ancestor (as, otherwise, the parent would not actually
...
\TODO{we can use this observation to introduce \emph{lexical SSA}; grammar in
Figure~\ref{fig:lex-ssa}}
...
\TODO{new object: regions, which are subtrees of the dominance tree}

\TODO{think of a region like a basic block carrying the subtree of blocks which it dominates;
single-entry-multiple-exit style}

\TODO{new object: control-blocks: a terminator, and where the terminator can jump to}

\TODO{the data is that in \ref{fig:ssa-data}}

\TODO{no need for a distinguished entry block; we now just pass in a region, which has an implicit
entry block}

It follows that by topologically sorting the basic blocks in the CFG according to the dominance
order and inserting brackets based on the dominance tree, we can convert dominance-based scoping to
lexical scoping, where the immediate children of a node in the dominator tree are placed in its
lexical scope immediately after the definitions. We give the resulting grammar for \emph{lexical
SSA} in Figure~\ref{fig:lex-ssa}:
\begin{figure}[H]
  \begin{center}
    \begin{grammar}
      <\(v\)> ::= \(x\) \;|\; \((v, v')\) \;|\; \(()\)
...
\begin{figure}
  \begin{lstlisting}[language=C++]
    struct BasicBlock {
      ...
      ControlBlock terminator;                      // control-block
    }
...
\TODO{pull up definition of strict SSA to introduction}

\TODO{push down definition of ANF transformation to appendix}

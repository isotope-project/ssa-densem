...
which are not pure but nontheless relevant or affine abound; for example,
\begin{itemize}
  \item In a programming language with nondeterminism, nondeterministic functions are impure (since
  $\letexpr{y}{f(x)}{(y, y)}$ is a strict refinement of $(f(x), f(x))$) but are affine, assuming
  ...
  duplicable, and, depending on the language's other effects, may be relevant
\end{itemize}
All three of these concepts still make perfect sense when interpreted as-is in a Freyd category, and
are particularly useful when reasoning about models of probablistic programming, such as Markov

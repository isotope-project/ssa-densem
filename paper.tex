...
\TODO{reforge to following plan:}
\begin{enumerate}
  \item (generalized) SSA restricts to ANF
  \item ANF restricts to strict SSA
  \item Strict SSA erases to standard SSA
  \item Equational theory says semantics is invariant up to label re-ordering and label fusion
  \item There is a single maximal label fusion tree, given by the ``uses variable'' order with the
  entry block at the root (which is a subset of the ``dominates'' order)
  \item Everyone which erases to the same SSA has the same maximal label fusion tree
  \item Therefore everyone which erases to the same SSA has the same semantics, by equational theory
\end{enumerate}
...
will give a subset of \isotopessa{} regions, which we will call \emph{strict} regions, about which we make the
following claims:
\begin{enumerate}
  \item Every \isotopessa{} region can be converted to an equivalent strict region using our
  equational theory \label{claim:anf-ssa-conv}
  \item Every strict region can be erased to a well-formed SSA program purely by removing
  \ms{where}-block annotations \label{claim:anf-erase}
  \item Every well-formed SSA program can be typed as an strict region purely by adding
  \ms{where}-blocks to correspond to dominance \label{claim:anf-wf}
  \item Two strict regions which erase to the same SSA program are equivalent according to our
  equational theory
\end{enumerate}
Thus, we make the claim that strict regions correspond exactly to SSA, with the
\ms{where}-bracketing being merely extra syntactic information added to make our typing rules and
denotational semantics a bit more syntax-directed. In particular, \ms{where}-blocks can be added
purely algorithmically by computing dominance between basic blocks.
...
unit level; in particular, it is obvious that $\ahasty{\Gamma}{\epsilon}{a}{A} \implies
\hasty{\Gamma}{\epsilon}{a}{A}$. Since SSA consists of \emph{basic blocks}, that is, linear
sequences of instructions followed by a terminator, we will introduce two judgements:
\begin{itemize}
  \item \todo{terminators}
  \item \todo{regions: block + dominated sub-blocks}
\end{itemize}
...
$\thaslb{\Gamma}{r}{\ms{L}} \implies \haslb{\Gamma}{r}{\ms{L}}$ and $\chaslb{\Gamma}{r}{\ms{L}}
\implies \haslb{\Gamma}{r}{\ms{L}}$.  We may now state Claim~\ref{claim:anf-ssa-conv} above formally
as follows:
% \begin{lemma}[A-normalization]
%   For all $\haslb{\Gamma}{r}{\ms{L}}$, there exists a region $\ms{ANF}(r)$ such that
%   $\ahaslb{\Gamma}{\ms{ANF}(r)}{\ms{L}}$ and $\lbeq{\Gamma}{r}{\ms{ANF}(r)}{\ms{L}}$
% \end{lemma}
% \begin{proof}
%   We begin by proving the following case: if $\ahaslb{\Gamma, \bhyp{x}{A}}{r}{\ms{L}}$ is in ANF,
%   and $\hasty{\Gamma}{\epsilon}{a}{A}$, then there exists a region
%   $\ahaslb{\Gamma}{\ms{ANF}(\letstmt{x}{a}{r})}{\ms{L}}$ in ANF such that
%   $\lbeq{\Gamma}{(\letstmt{x}{a}{r})}{\ms{ANF}(\letstmt{x}{a}{r})}{\ms{L}}$. We proceed by induction
%   on $a$:
%   \begin{itemize}
%     \item If $\ahasty{\Gamma}{\epsilon}{a}{A}$ is atomic, we are done; otherwise
%     \item If $a = f\;e$, then we may rewrite \TODO{this}
...
%     \end{equation}
%     Since $r$ is in ANF, by induction (since $b, c$ are subterms of $a$), this is equivalent to some
%     \begin{equation}
%       \letstmt{w}{e}
%         {\casestmt{w}{y}{\ms{ANF}(\letexpr{b}{x}{r})}{z}{\ms{ANF}(\letexpr{c}{x}{r}})}
%     \end{equation}
%     As both branches of this case statement are in ANF and the discriminat is a variable, the
%     case-statement itself is in ANF, and hence by induction again (since $e$ is a subterm of $a$)
...
%   \end{itemize}
%   We may now prove the lemma by a straightforward induction on $r$:
%   \begin{itemize}
%     \item If $r = \brb{\ell}{a}$, by $\beta$-reduction, we may rewrite this as $r \teqv
%     \letstmt{x}{a}{\brb{\ell}{x}}$; the result then follows from a direct application of the above
%     \item If $r = \letstmt{x}{a}{r'}$, then by induction we have $r \teqv
%     \letstmt{x}{a}{\ms{ANF}(r')}$, and a direct application of the above allows us to
%     define $\ms{ANF}(\letstmt{x}{a}{r'}) = \ms{ANF}(\letstmt{x}{a}{\ms{ANF}(r')}) \teqv r$ as
%     desired.
%     \item If $r = \letstmt{(x, y)}{a}{r'}$, we may first rewrite this by induction as 
%     \begin{equation}
%       r \teqv \letstmt{z}{a}{\letstmt{(x, y)}{z}{r'}} 
%         \teqv \letstmt{z}{a}{\letstmt{(x, y)}{z}{\ms{ANF}(r')}}
%     \end{equation}
%     Since $z$ is a variable, $\letstmt{(x, y)}{z}{\ms{ANF}(r')}$ is also in ANF, so we can apply the
%     above to define $\ms{ANF}(\letstmt{(x, y)}{a}{r'}) = \ms{ANF}(\letstmt{z}{a}{\letstmt{(x,
%     y)}{z}{\ms{ANF}(r')}}) \teqv r$. The proof for case-statements is analogous.
%     \item The case for control-flow graphs is simply a straightforward application of the inductive
%     hypotheses
%   \end{itemize}
% \end{proof}
...
\begin{figure}
  ...
  \end{gather*}
  \caption{Typing rules for SSA operations}
  \Description{}
  \label{fig:ssa-ops}
\end{figure}
...
\begin{figure}
  ...
  \end{gather*}
  \begin{equation*}
    \boxed{\chaslb{\Gamma}{r}{\ms{L}}}
  \end{equation*}
  \begin{gather*}
    \prftree[r]{\rle{cfg}}
      {\chaslb{\Gamma}{r}{\ms{L}, (\lhyp{\ell_i}{A_i},)_i}}
      {\forall i. \chaslb{\Gamma, \bhyp{x_i}{A_i}}{t_i}{\ms{L}, (\lhyp{\ell_j}{A_j},)_j}}
      {\chaslb{\Gamma}{\where{r}{(\wbranch{\ell_i}{x_i}{t_i},)_i}}{\ms{L}}}
  \end{gather*}
...
To substantiate Claim~\ref{claim:anf-erase}, we give a grammar for SSA programs in
Figure~\ref{fig:ssa-standard}. It is easy to see that removing braces and \ms{where}-blocks from an
arbitrary region gives a flat SSA program; this program will be well-formed with respect to
dominance scoping if it is well-typed.

\TODO{to show equality, we need}
\begin{itemize}
  \item Re-ordering within a CFG is trivial
  \item \emph{Merging} CFGs
  \item There is a unique, maximally merged normal-form CFG for each SSA program, and everyone is
  equal to it
  \item Therefore, everyone erasing to the same thing is equal, since this is unique per-program
\end{itemize}
...
\begin{figure}[H]
  \begin{center}
    \begin{grammar}
      ...
      <\(s, t\)> ::= \(\beta\) 
      \;|\;  \(\where{t}{(\wbranch{\ell_i}{x_i}{t_i},)_i}\)

      <\(G\)> ::= \(\beta\) \;|\; \(G; \wbranch{\ell}{x}{\beta}\)
    \end{grammar}
  \end{center}
  \caption{ 
    Grammar for standard SSA, parametrized over an \isotopessa{} signature. A standard SSA
    program is made up of \emph{basic blocks} \(\beta\), and can be thought of as a \emph{region}
    \(r\) or a \emph{control-flow graph} \(G\) (with a distinguished entry block $\beta$). 
  }
  \Description{}
...
\TODO{use folk B\"ohm-Jacopini to argue for arbitrary bracketing theorem (??). Or is it just useful
on its own?}
...
In general, the idea of generalizing traces to represent true concurrency leads us to our other
approach: swapping out sets of \emph{linear} traces with (sets of) \emph{partially ordered}
structures which directly represent the concurrency of their component operations. This, in
particular, directly represents that executing two things ``at the same time'' (i.e., not
postulating an order relationship between them) is not the same as executing them in some particular
order. The most basic such data structure is a \emph{partially ordered multiset}, or \emph{pomset},
which we describe in Section~\ref{ssec:tso} of our paper. Pomsets can be used to model sequential
consistency quite naturally, and, like for traces, can be augmented with additional structure to
model relaxed memory models such as TSO weak memory, as in \citet{sparky}.

One issue, however, is that it can be very difficult to figure out \emph{which} structures to
augment pomsets with to model more advanced memory models. For example, \citet{jagadeesan-pwp-20}
introduce \emph{pomsets with preconditions} (PwP), which augment pomsets with logical formulae, to
model weak memory behaviors. However, \citet{leaky-semicolon} demonstrate that sequential
composition (the semicolon operator) in PwP is not associative, which undermines many common program
optimizations and breaks the monadic structure necessary for compositional reasoning. To address
this issue, \citet{leaky-semicolon} propose \emph{pomsets with predicate transformers} (PwT), which
restore the associativity of sequential composition. Despite this improvement, their semantics still
do not support loops, and it remains future work to develop a monadic structure based on PwT that
fully supports recursion and iterative constructs.

Another potential generalization of pomsets inspired by game semantics is \emph{event structures},
which introduce a \emph{conflict relation} which allows representing many potentially conflicting
executions as a single mathematical object. \citet{castellan-16} shows how to use event structures
to represent concurrent executions w.r.t. a \emph{memory model} -- they study a simple language
supporting parallel composition of $n$ simple, linear programs defined as lists of loads, stores,
and arithmetic operations, for which event structures give a denotational semantics.

...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
Fixing a distinguished label ``\ms{o}'', we can define a ``packing'' operation on regions as
follows:
\begin{equation}
  \haslb{\Gamma}{r}{\ms{L}} \implies 
  \haslb{\Gamma}{[r]^+ := [\ms{pack}_{\ms{o}}(\ms{L})]r}{\ms{o}([\ms{L}])}
\end{equation}
...
\begin{enumerate}
  ...
  that, for a distinguished variable $\ms{i}$ and label $\ms{o}$,
  \begin{equation}
    \hasty{\ms{i} : [\Gamma]}{\epsilon}{\dnt{\hasty{\Gamma}{\epsilon}{a}{A}}}{A}
    \qquad
    \haslb{\ms{i} : [\Gamma]}{\dnt{\haslb{\Gamma}{r}{L}}}{\ms{o}([\ms{L}])}
  \end{equation}
  where packing of contexts is defined as in Section~\ref{ssec:bohm-jacopini}.
  
  \item Finally, we refine this result to show that
  \begin{equation}
    \tmeq{\ms{i} : [\Gamma]}{\epsilon}{\dnt{\hasty{\Gamma}{\epsilon}{a}{A}}}{[a]}{A}
    \qquad
    \lbeq{\ms{i} : [\Gamma]}{\dnt{\haslb{\Gamma}{r}{L}}}{[r]}{\ms{o}([\ms{L}])}
  \end{equation}
  Since the packing operator $[\cdot]$ on terms and regions from Section~\ref{ssec:bohm-jacopini} is
  injective for pure contexts $\ms{eff}(\Gamma) = \bot$, it follows that in this case the category
  of expressions and the category of regions are the initial distributive Freyd category and
  distributive Freyd-Elgot category respectively.
\end{enumerate}

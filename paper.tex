...
\TODO{Reflow to B\"ohm-Jacopini + elimination of $\phi$-nodes}
...
Instead, we'll define a predicate $\ms{structured}(\ms{L})$ parametrized by a \emph{set} of
acceptable targets.
...
\begin{equation}
  ...
  = [y \mapsto \pi_l\;y]\ms{pack}_y^\otimes(\Gamma), x \mapsto \pi_r\;y 
\end{equation}
\begin{equation}
...
\begin{enumerate}
  ...
  from Section~\ref{ssec:bohm-jacopini} is injective for pure contexts $\ms{eff}(\Gamma) = \bot$,
...
\begin{definition}[Trace Monad Transformer]
  ...
  \begin{itemize}
    \item $\ms{Traces}?\;M\;I = \ms{TraceT}\;M\;I\;\mc{P}$, with Elgot operation
    \TODO{elgot operation}
    \TODO{subsingleton traces, deterministic functions}
    \TODO{nonempty traces, total functions}
    \item $\ms{Traces}\;M\;I = \ms{TraceT}\;M\;I\;\mc{P}^+$, which can be viewed as an Elgot submonad
    of $\ms{Traces}?\;M\;I$ since the fixpoint of a total function is total
    \item $\ms{Trace}?\;M\;I = \ms{TraceT}\;M\;I\;\ms{Option}$, which can be viewed as an Elgot 
    submonad of $\ms{Traces}?\;M\;I$ since the fixpoint of a deterministic function is deterministic
    \item $\ms{Trace}\;M\;I = \ms{TraceT}\;M\;I\;\ms{Id}$, which can be viewed as an Elgot submonad
    of $\ms{Traces}?\;M\;I$, and in particular the intersection of $\ms{Trace}?$ and $\ms{Traces}$.
  \end{itemize}
\end{definition}

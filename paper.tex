...
\begin{figure}
  \begin{gather*}
    ...
    \prftree[r]{\rle{sb-id}}{\issubst{\gamma}{\Gamma}{\Delta}}{x \notin \gamma}
      {\issubst{\gamma}{\Gamma, \thyp{x}{A}{\epsilon}}{\Delta}, \thyp{x}{A}{\epsilon}}
       \\
    ...
    \qquad
    \prftree[r]{\rle{ls-id}}
      {\lbsubst{\Gamma}{\sigma}{\ms{L}}{\ms{K}}}{\ell \notin \sigma}
      {\lbsubst{\Gamma}{\sigma}{\ms{L}, \lhyp{\ell}{A}}{\ms{K}, \lhyp{\ell}{A}}}
    \\
...
\begin{itemize}
  ...
  while \brle{sb-skip} says that we can ignore variables defined in a substitution which do not
  appear in the right-hand side of the substitution well-typedness judgement. Together, these say
  that \emph{every} substitution can be interpreted as taking every context to the empty context,
  ...
  \item \brle{sb-id} says that, for any variables $x$ not appearing in a substitution $\gamma$, we
  can type $\gamma$ using the fact that the action of $\gamma$ on $x$ is the identity. This is
  useful as it allows us to implicitly extend substitutions to larger contexts without defining
  unwieldy operators. \todo{clean this up}
\end{itemize}
...
\TODO{deal with skip rule causing problems for rewriting...}
...
\ref{fig:ssa-subst-def}, we may then define label-substitution recursively in the obvious manner,
mapping $\ms{br}\;\ell\;a$ to $[a/x]r$ as a base case. We can similarly define the composition of
label substitutions $[\sigma']\sigma$ in the expected manner. This allows us to state \emph{label
      ...
      \prftree[r]{\rle{dinat}}
        {
          ...
          \lbsubst{\kappa}{\Gamma}{(\lhyp{\ell_i}{A_i},)_i}{(\lhyp{\kappa_j}{A_j},)_j}
        }{
          \forall i. \haslb{\Gamma, \bhyp{x_i}{A_i}}{t_i}{\ms{L}, (\lhyp{\ell_j}{A_j},)_j}
        }{
          \lbeq{\Gamma}
            {\where{([\sigma]r)}{(\wbranch{\kappa_i}{x_i}{[\sigma]t_i},)_i}}
...
\end{multline*}
Rewrites like this are an instance of the principle we call \emph{dinaturality}, which

\TODO{explain, give for-loop example or something similar}

\TODO{but, what about side-effecting things}

\TODO{dinaturality should allow this specifically}

\TODO{we also want to natively handle cfgs in general}

\TODO{which is going to turn out to be Very Useful (TM)}
...
single input wire, as in Figure~\todo{this}. Continuing this analogy, we draw the codiagonal
morphism the codiagonal morphism $[\ms{id}_A, \ms{id}_A]$ as joining two wires, and the morphism $0$
from the initial object as a wire coming from nowhere. 
...
wires around. For example, we have \todo{these}

String diagrams also give us an elegant way to represent and reason about Elgot structures. It turns
out that \todo{cite Hyland and Hasegawa} Elgot structures induce a \emph{trace} on the coproduct:
given $f : A + C \to B + C$, we can define 
\begin{equation}
  \ms{Tr}_{A, B}^C(f) 
...
therefore the Elgot operator, as in Figure~\todo{this}. Continuing with the control-flow diagram
analogy, such traces can be interpreted as \emph{loops}, with the Elgot axioms, now drawn as
diagrams in Figure~\todo{this}, suddenly seeming a lot more obvious.
...
do not in general validate \emph{sliding}, as in Figure~\todo{this}. However, this is easy enough to
fix: we can postulate a ``state'' wire, drawn in red, which all impure morphisms require as an input
and output, as in Figure~\todo{this}. Since the state wire linearly threads through all impure
boxes, it establishes a unique order in which they must be executed; this construction is shown to
be sound in \citet{promonad}. Pure morphisms do not have a state wire, but, if necessary, can be
drawn between them. This gives us a convenient way to distinguish between string diagrams using the
monodial structure induced by the coproduct and those using the premonoidal structure induced by the
tensor product in a category having both (such as a distributive premonoidal category): the latter
will have a state wire, while the former will not.
...
%
We will model types in the obvious manner: 
\begin{itemize}
...
We can now interpret \isotopessa{} expressions $\hasty{\Gamma}{\epsilon}{a}{A}$ over a given signature
in a model intepreting that signature as morphisms $\dnt{\Gamma} \to_\epsilon \dnt{A}$ using the
rules in Figure~\ref{fig:ssa-expr-sem}. Up to this point, both our syntax and semantics are quite
...
Similarly, we can interpret \isotopessa{} regions $\haslb{\Gamma}{r}{\ms{L}}$ as morphisms
$\dnt{\Gamma} \to \dnt{\ms{L}}$; note that we don't assume anything about the effect of these
morphisms. As $\ms{L}$ is a coproduct, we can view the result object of a region $r$ as encoding
both \emph{data} and \emph{control-flow} information. In particular, we interpret a branch
$\brb{\ell}{a}$ as simply the injection of the (pure) expression $\hasty{\Gamma}{\bot}{a}{A}$, our
\emph{data}, into the element of the coproduct corresponding to $\ell$, which encodes the point in
control-flow the rest of the program should jump to next. This is in contrst to \emph{expressions},
which purely encode data, with no particular instructions on how to use it afterwards. Our
interpretation of \ms{let}-statements and \ms{case}-statements, given in
...
coproduct $\dnt{\ms{K}}$; in particular, in both cases, the morphisms are pure. We can state a
weakening lemmas as follows:
\begin{lemma}[(Label) Weakening]
  ...
  \end{enumerate}
\end{lemma}
The proof for this proceeds mostly as a straightforward induction.

\TODO{discuss uniformity lore}.
...
(Label) substitutions can also be modeled analogously to weakenings, except that their
corresponding substitutions are not always pure.
...
\begin{figure}
  ...
  \end{gather*}
  \begin{align*}
    \dnt{\issubst{\gamma}{\Gamma, \thyp{x}{A}{\epsilon}}{\Delta, \thyp{x}{A}{\epsilon}}} 
  ...
  \end{gather*}
  \begin{align*}
    \dnt{\lbsubst{\kappa}{\Gamma}{\ms{L}, \ell(A)}{\ms{K}, \ell(A)}}
      &= \delta ; [\dnt{\lbsubst{\kappa}{\Gamma}{\ms{L}}{\ms{K}}}, \iota_r]
    \\
...
\TODO{factor out packing lore and initiality lore, actually describe category with text}
...
\begin{theorem}[Completeness (Expressions)]
  ...
  $
  i.e. $\ms{Th}(\cdot)$ is the initial model
\end{theorem}
...
\begin{theorem}[Completeness (Regions)]
  ...
  $
  i.e. $\ms{Th}(\cdot, \cdot)$ is the initial model.
\end{theorem}
...
\TODO{more general semicategory lore?}

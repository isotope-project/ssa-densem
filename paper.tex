...
of iterations. Our final rewriting rule, \brle{dinat}, generalises the
...
\end{figure}
\TODO{work on wording here, point back to reviews}
...
give a denotational proof. However, the completeness of our equational theory (proved in Section~\ref{ssec:completeness})
means that the semantic equality implies the existence of the requisite derivation tree. A proof can be
found in Lemma~\ref{lem:where-fusion} in the appendix. This is one of the benefits of having a completeness
result: it lets us switch freely between equational and denotational modes of reasoning. 
...
\end{equation}
\TODO{reword}
We note the \ms{case} branch is a little more complex than strictly necessary: to get to ANF, we
don't actually need the new label, and can instead replace each branch with $\letanf{x}{a}{r}$.
However, in the worst case, this can cause exponential blowup in the size of our program text,
complicating our claim of equivalence between ANF and \isotopessa{}.

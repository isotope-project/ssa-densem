...
major components:
\begin{itemize}
  \item The \emph{terminator} $\entrymor{r} : \dnt{\Gamma} \to \dnt{\ms{L}} + \Sigma_i\dnt{A_i}$,
  ...
  \end{itemize}
  \item The ``\emph{loop}'' $\loopmor{(\wbranch{\ell_i}{x_i}{t_i},)_i} : \dnt{\Gamma} \otimes
  \Sigma_i\dnt{A_i} \to \dnt{\ms{L}} + \Sigma_i\dnt{A_i}$ which, given as input the context
  $\dnt{\Gamma}$ and an input $\dnt{A_i}$ for the $i^{th}$ basic block $t_i$, executes $t_i$ and
  then re-associates the output. The output type $\dnt{\ms{L}} + \Sigma_i\dnt{A_i}$ again expresses
  that control-flow may either exit the \ms{where}-block (via $\dnt{\ms{L}}$) or jump to some other
  basic block (via $\Sigma_i\dnt{A_i}$).
\end{itemize}
We glue these together in the obvious manner to get the semantics for a where-block:
\begin{itemize}
  \item Compute $\lmor{\entrymor{r}}$, which, given as input the context $\dnt{\Gamma}$, compies the
  context, executes $\entrymor{r}$, and then returns the copied context and the output as
  $\dnt{\Gamma} \otimes (\dnt{\ms{L}} + \Sigma_i\dnt{A_i})$
  ...
  immediately
  \item Otherwise, compute $\loopmor{(\wbranch{\ell_i}{x_i}{t_i},)_i}$ in a loop, passing in a
  fresh copy of the context each time. This is implemented with $\ms{rfix}$.
\end{itemize}
...
\begin{proof}
  ...
  \begin{itemize}
    \item \emph{Fixpoint:} \TODO{this}
    \item \emph{Naturality:} \TODO{this}
    \item \emph{Codiagonal:} \TODO{this}
    \item \emph{Uniformity:} \TODO{this}
  \end{itemize}
\end{proof}
  ...
  \begin{itemize}
    ...
    \begin{equation}
      ...
      & = \dnt{\Gamma} \otimes \alpha^+_{\Sigma_i\dnt{A_i}} 
        ; \rfix{\loopmor{(\wbranch{\ell_i}{x_i}{t_i},)_i}}
    ...
    \begin{equation}
      \begin{aligned}
        ...
        & = \lmor{\dnt{\haslb{\Gamma}{\topwhile{\ms{L}}{r}}{\outlb([\ms{L}, \ms{R}])}}}
          ; \dnt{\Gamma} \otimes \alpha^+_{\dnt{\ms{L, R}}} \\ & \qquad
          ; \dnt{\haslb 
          {\Gamma, \bhyp{\invar}{[\ms{L}, \ms{R}]}}
          {\caseexpr{\ms{ua}\;\invar}{x}{\brb{\outlb}{x}}{y}{L}}
          {\outlb([\ms{L}])}} \\

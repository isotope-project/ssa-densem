...
The result of doing so looks a lot like the correspondence between CPS and SSA described in
\citet{kelsey-95-cps}. 

\TODO{why are these similar? explain?}

\TODO{talk about type-theoretic benefits of lexical scoping}
...
\begin{itemize}
  ...
  \item Can generalize to \isotopessa (like MNF but not higher-order); by adding an
  \textit{expression language}
  \item Again, obviously no additional expressive power
  \item This lets us state \textit{substitutions}, like, in Figure~\ref{fig:fact-cse}, which can be
  used to build up optimizations such as \textit{common-subexpression elimination}.
\end{itemize}
...
\begin{itemize}
  \item Substitution, in particular, lets us do algebra \textit{algebra}, for example, we know that:
  \item \begin{itemize}
  ...
  \end{itemize}
  \item So therefore we can rewrite \ref{fig:fact-subst-2} to \ref{fig:fact-dinat} by substitution
  \item Which we can combine with control-flow rewrites, such as \textit{dinaturality}
  (\todo{explain}, cite Elgot lore?) to build up complex optimizations, such as rewriting
  \ref{fig:fact-zero} to \ref{fig:fact-opt}.
